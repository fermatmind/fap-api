...
class BigFiveAuthorityV228Test extends TestCase
{
    ...
    {
        parent::setUp();
        $this->packagePath = base_path('../generated/big-five-authority-v2/big5-authority-v2-article-learning-habits-28');
    }

    public function test_final_package_has_exact_batch_28_bilingual_inventory(): void
    {
        ...
        $this->assertSame([28], array_values(array_unique(array_column($package['assets'], 'batch'))));
        $this->assertCount(5, array_unique(array_column($package['assets'], 'unique_intent_key')));
    }

    public function test_every_asset_has_complete_structure_and_ability_boundary(): void
    {
        $package = $this->readJson('final-package.json');
        ...
        foreach ($package['assets'] as $asset) {
            ...
            $terms = $asset['locale'] === 'en' ? ['learning style', 'creativity', 'decision quality', 'unknown'] : ['学习风格', '创造力', '决策质量', 'unknown'];
    ...
    public function test_qa_proves_task_level_scope_and_zero_release_mutations(): void
    {
        ...
        $this->assertTrue($qa['checks']['consumes_only_pr21_batch_28']);
        $this->assertTrue($qa['checks']['task_level_experiments_not_trait_prescriptions']);
        $this->assertTrue($qa['checks']['ability_and_outcome_inference_prohibited']);
