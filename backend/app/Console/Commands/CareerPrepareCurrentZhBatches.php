...
final class CareerPrepareCurrentZhBatches extends Command
{
    protected $signature = 'career:prepare-current-zh-batches
    ...
    protected $description = 'Prepare deterministic read-only zh-CN Career Current candidate projections and batches';

    public function handle(CareerCurrentZhBatchPreparer $preparer): int
    {
        ini_set('memory_limit', '1024M');
        try {
            if (! (bool) $this->option('dry-run')) {
                throw new CareerTenBlockCompileFailure('CURRENT_ZH_DRY_RUN_REQUIRED');
            }
            ...
            $this->line((string) json_encode([
                'status' => 'PASS_CURRENT_ZH_BATCH_PREPARE',
            ...
            $this->line((string) json_encode([
                'status' => 'FAIL_CURRENT_ZH_BATCH_PREPARE',
                'safe_error_code' => $throwable instanceof CareerTenBlockCompileFailure
                    ? $throwable->safeCode : 'CURRENT_ZH_UNEXPECTED_FAILURE',
                'current_package_writes' => 0,
                'database_writes' => 0,
                'cache_writes' => 0,
                'cms_writes' => 0,
