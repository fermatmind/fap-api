...
const locked = matrix.themes.filter((theme) => theme.batch === 27);
...
assert(finalPackage.assets.every((asset) => asset.batch === 27));
...
for (const asset of finalPackage.assets) {
  const theme = locked.find((candidate) => candidate.topic_id === asset.topic_id);
  const locale = theme.locales.find((candidate) => candidate.locale === asset.locale);
  assert.equal(asset.slug, locale.slug);
  assert.equal(asset.path, locale.path);
  assert.equal(asset.title_intent, locale.title_intent);
  assert.equal(asset.primary_question, locale.primary_question);
  assert.equal(asset.audience, locale.audience);
  assert.equal(asset.user_task, locale.user_task);
  assert.deepEqual(asset.keywords, locale.keywords);
  assert.deepEqual(asset.internal_link_targets, locale.internal_link_targets);
  assert.deepEqual(asset.source_mapping.map((source) => source.source_id), locale.source_requirements);
  assert(asset.source_mapping.every((source) => sourceIds.has(source.source_id) && (source.public_url || source.repository_path)));
  assert.deepEqual(asset.sections.map((section) => section.key), sections);
  assert(asset.sections.every((section) => section.body_md.length >= 30));
  const boundary = asset.sections.find((section) => section.key === 'method_product_boundary').body_md.toLowerCase();
  const terms = asset.locale === 'en' ? ['consent', 'compatibility', 'relationship outcomes', 'unknown'] : ['同意', '匹配', '关系结局', 'unknown'];
  for (const term of terms) assert(boundary.includes(term));
...
for (const forbidden of ['/attempt/', '/report/', '/orders/', 'clinically validated', 'guaranteed compatible', 'relationship will']) assert(!serialized.includes(forbidden));
console.log('Big Five batch 27 validation passed: exact 5 relationship themes / 10 bilingual Article candidates / consent and outcome boundaries');
