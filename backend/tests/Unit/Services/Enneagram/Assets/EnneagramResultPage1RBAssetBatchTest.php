...
final class EnneagramResultPage1RBAssetBatchTest extends TestCase
{
    public function test_1r_b_batch_assets_are_runner_safe_and_not_runtime(): void
    {
        $root = base_path('content_assets/enneagram/result_page/batch_1r_b/v0_1');
        $manifest = $this->readJson($root.'/asset_batch_manifest.json');
        $assetsDoc = $this->readJson($root.'/assets.json');

        $this->assertSame('fap.enneagram.result_page.batch_1r_b.manifest.v0.1', $manifest['schema_version'] ?? null);
        ...
        $this->assertSame(423, (int) data_get($manifest, 'candidate_contract_reference.this_batch_asset_count'));
        $this->assertSame(423, (int) ($assetsDoc['asset_count'] ?? 0));
        $this->assertSame(423, count((array) ($assetsDoc['assets'] ?? [])));
        $this->assertSame('d34a6951a7198a7af38c1c76bdf13c4f5374992e0904c403f7e302f2882d5b39', $manifest['source_asset_sha256'] ?? null);
        ...
        $this->assertSame(423, (int) ($sourceMapping['payload_count'] ?? 0));
        ...
        $this->assertSame(423, (int) ($runnerReport['payload_count'] ?? 0));
        $this->assertSame(0, (int) ($runnerReport['expected_runner_failure_count'] ?? -1));
        $this->assertCount(423, (array) ($runnerReport['payload_hashes'] ?? []));

        $runner = new EnneagramResultPageAgentBatchRunner;
        $sourceLedgerRow = $this->sourceLedgerRow('batch_1r_b_asset_stream');
        $assetKeys = [];
        ...
        foreach ((array) ($assetsDoc['assets'] ?? []) as $asset) {
            ...
            $this->assertSame('1R-B', $asset['batch_scope'] ?? null);
            $this->assertSame('batch_1r_b_asset_stream', data_get($asset, 'source_trace.primary_source_id'));
        ...
        $this->assertCount(423, array_unique($assetKeys));
