...
{
    protected $signature = 'seo:council-runtime {operation=status : status, pause or resume} {--mission=* : Complete target set of formal Mission IDs}';
    ...
    public function handle(Platform12RuntimeControl $runtime): int
    {
        ...
        if (! in_array($operation, ['status', 'pause', 'resume'], true) || ($operation !== 'resume' && $this->option('mission') !== [])) {
            $this->line('{"state":"OPERATION_DENIED","business_write_enabled":false}');

            return self::FAILURE;
        }
        $result = $operation === 'status' ? $runtime->status() : $runtime->change($operation === 'pause', $this->option('mission'));
