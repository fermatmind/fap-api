...
ART_DIR="${ART_DIR:-backend/artifacts/pr41}"
if [[ "${ART_DIR}" != /* ]]; then
...
ANON_ID="${ANON_ID:-pr41-verify-anon}"
...
http_code="$(curl -sS -o "${SCALES_JSON}" -w "%{http_code}" \
  -H "Accept: application/json" \
  "${API_BASE}/api/v0.3/scales" || true)"
if [[ "${http_code}" != "200" ]]; then
  echo "scales_failed http=${http_code}" >&2
  cat "${SCALES_JSON}" >&2 || true
  fail "scales endpoint failed"
fi

RESOLVED_SCALE_META="$(SCALE_CODE="${SCALE_CODE}" php -r '
$requested = strtoupper(trim((string) getenv("SCALE_CODE")));
$data = json_decode((string) file_get_contents($argv[1]), true);
if (!is_array($data)) {
    fwrite(STDERR, "scales_json_invalid\n");
    exit(1);
}
$items = $data["items"] ?? $data["data"] ?? $data;
if (!is_array($items) || count($items) === 0) {
    fwrite(STDERR, "scales_items_invalid\n");
    exit(1);
}
$first = null;
$matched = null;
foreach ($items as $item) {
    ...
    $slug = trim((string) ($item["primary_slug"] ?? ""));
    if ($slug === "") {
        $slug = $code;
    }
    $candidate = ["code" => $code, "slug" => $slug];
    if ($first === null) {
        $first = $candidate;
    }
    if ($requested !== "" && strtoupper($code) === $requested) {
        $matched = $candidate;
        break;
    }
}
$resolved = $matched ?? $first;
if (!is_array($resolved)) {
    fwrite(STDERR, "scale_code_missing\n");
    exit(1);
}
echo $resolved["code"]."|".$resolved["slug"];
' "${SCALES_JSON}")"

SCALE_CODE="${RESOLVED_SCALE_META%%|*}"
QUESTIONS_SCALE="${RESOLVED_SCALE_META#*|}"
if [[ -z "${SCALE_CODE}" ]]; then
  fail "resolved scale code is empty"
fi
if [[ -z "${QUESTIONS_SCALE}" ]]; then
  QUESTIONS_SCALE="${SCALE_CODE}"
fi
echo "${SCALE_CODE}" > "${ART_DIR}/scale_code.txt"
echo "${QUESTIONS_SCALE}" > "${ART_DIR}/questions_scale.txt"
echo "resolved_scale_code=${SCALE_CODE}"
echo "resolved_questions_scale=${QUESTIONS_SCALE}"
...
http_code="$(curl -sS -o "${QUESTIONS_JSON}" -w "%{http_code}" \
  -H "Accept: application/json" \
  "${API_BASE}/api/v0.3/scales/${QUESTIONS_SCALE}/questions" || true)"
echo "questions_http_primary=${http_code} path=${QUESTIONS_SCALE}"
if [[ "${http_code}" != "200" ]] && [[ "${QUESTIONS_SCALE}" != "${SCALE_CODE}" ]]; then
  http_code="$(curl -sS -o "${QUESTIONS_JSON}" -w "%{http_code}" \
    -H "Accept: application/json" \
    "${API_BASE}/api/v0.3/scales/${SCALE_CODE}/questions" || true)"
  echo "questions_http_fallback=${http_code} path=${SCALE_CODE}"
fi
if [[ "${http_code}" != "200" ]]; then
...
php artisan test --filter PaymentWebhookStripeSignatureTest > "${ART_DIR}/payment_webhook_stripe_signature_test.log"
php artisan test --filter PaymentWebhookLockBusyTest > "${ART_DIR}/payment_webhook_lock_busy_test.log"
php artisan test --filter PaymentWebhookIdempotencyTest > "${ART_DIR}/payment_webhook_idempotency_test.log"
php artisan test --filter CommerceWebhookIdempotencyTest > "${ART_DIR}/commerce_webhook_idempotency_test.log"
cd "${REPO_DIR}"
