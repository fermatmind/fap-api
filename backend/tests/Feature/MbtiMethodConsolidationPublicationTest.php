...
class MbtiMethodConsolidationPublicationTest extends TestCase
{
    private const FIXTURE_CONNECTION = 'mbti_method_consolidation_fixture';
    ...
    protected function setUp(): void
    {
        ...
        $connection['prefix'] = 'mbti_method_consolidation_'.bin2hex(random_bytes(6)).'_';
    ...
    {
        return json_decode(file_get_contents(database_path('data/assessment_mbti_method_consolidation_zh_20260920.json')), true, 512, JSON_THROW_ON_ERROR);
    }
    ...
    private function insert(string $table, array $zh): void
    {
    ...
    {
        (require database_path('migrations/2026_09_20_090000_consolidate_mbti_method_content_zh.php'))->up();
    }
    ...
    public function test_updates_only_exact_fields_in_both_registries_and_is_idempotent(): void
    {
        ...
        foreach (['scales_registry', 'scales_registry_v2'] as $table) {
            ...
            $this->assertSame(['why_choose' => ['intro' => 'Keep English']], $content['en']);
            $this->assertSame($expected, $content['zh']);

            $validity = collect($content['zh']['faq'])->firstWhere('id', 'faq-validity');
            $this->assertCount(2, $validity['references']);
            $this->assertSame(['#method-and-evidence', '/zh/reliability-validity'], array_column($validity['related_links'], 'href'));
        }
    }
    ...
    public function test_rejects_cross_registry_mixed_state_without_writing(): void
    {
        ...
        $this->assertSame($before['why_choose'], $actual['zh']['why_choose']);
    }
    ...
    public function test_rejects_partial_or_unknown_target_state_without_writing(): void
    {
        $before = $this->baseline();
        $partial = $before;
        $partial['why_choose']['items'][0]['body'] = $this->package()['updates'][0]['value'];
        foreach (['scales_registry', 'scales_registry_v2'] as $table) {
        ...
        $actual = json_decode(DB::table('scales_registry')->value('content_i18n_json'), true, 512, JSON_THROW_ON_ERROR);
        $this->assertSame($partial['why_choose'], $actual['zh']['why_choose']);
    }
    ...
    public function test_rejects_incomplete_registry_rows_and_seeder_uses_the_same_package(): void
    {
        ...
        $this->assertStringContainsString('assessment_mbti_method_consolidation_zh_20260920.json', $source);
    }
}
