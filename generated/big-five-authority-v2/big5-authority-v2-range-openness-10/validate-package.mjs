...
const DIR = "generated/big-five-authority-v2/big5-authority-v2-range-openness-10";
const LEDGER = "generated/big-five-authority-v2/big5-authority-v2-source-ledger-05/source-ledger.json";
const EXPECTED_PAGES = [
  { locale: "en", path: "/en/personality/big-five/openness-high", level: "high", family: "v2_range", key: "range:openness:high", legacyIntent: null },
  { locale: "zh-CN", path: "/zh/personality/big-five/openness-high", level: "high", family: "v2_range", key: "range:openness:high", legacyIntent: null },
  { locale: "en", path: "/en/personality/big-five/openness-mid", level: "middle", family: "v2_range", key: "range:openness:middle", legacyIntent: null },
  { locale: "zh-CN", path: "/zh/personality/big-five/openness-mid", level: "middle", family: "v2_range", key: "range:openness:middle", legacyIntent: null },
  { locale: "en", path: "/en/personality/big-five/openness-low", level: "low", family: "v2_range", key: "range:openness:low", legacyIntent: null },
  { locale: "zh-CN", path: "/zh/personality/big-five/openness-low", level: "low", family: "v2_range", key: "range:openness:low", legacyIntent: null },
  { locale: "en", path: "/en/personality/big-five/high-openness", level: "high", family: "legacy_canonical", key: "legacy:high-openness", legacyIntent: "idea_portfolio_to_decision_protocol" },
  { locale: "en", path: "/en/personality/big-five/low-openness", level: "low", family: "legacy_canonical", key: "legacy:low-openness", legacyIntent: "evidence_threshold_for_change_adoption" },
];
...
const validateRaw = (raw) => {
  ...
  if (raw.schema_version !== "big5_authority_v2_range_package.v1" || raw.train_id !== "BIG5-AUTHORITY-V2-RANGE-OPENNESS-10" || raw.stage !== "raw" || raw.framework !== "big_five" || raw.domain_code !== "openness") add("schema", "raw_identity_invalid", "raw", "Raw package identity is invalid.");
  ...
  if (JSON.stringify(sorted(pages.filter(isObject).map((page) => page.canonical_path))) !== JSON.stringify(sorted(expectedPaths()))) add("schema", "raw_paths_invalid", "raw.pages", "Raw paths must equal PR10 coverage.");
...
const validateRepaired = (repaired, sourceLedger) => {
  const issues = [];
  const add = (gate, code, path, message) => issues.push({ gate, code, path, message });
  if (repaired.schema_version !== "big5_authority_v2_range_repair.v1" || repaired.train_id !== "BIG5-AUTHORITY-V2-RANGE-OPENNESS-10" || repaired.stage !== "repaired" || repaired.framework !== "big_five" || repaired.domain_code !== "openness") add("schema", "repaired_identity_invalid", "repaired", "Repaired package identity is invalid.");
  if (JSON.stringify(sorted(repaired.required_intents ?? [])) !== JSON.stringify(sorted(REQUIRED_INTENTS))) add("schema", "repaired_intents_invalid", "repaired.required_intents", "Repaired intents must equal PR10 scope.");
  ...
  if (JSON.stringify(sorted(pages.filter(isObject).map((page) => page.canonical_path))) !== JSON.stringify(sorted(expectedPaths()))) add("coverage", "repaired_paths_invalid", "repaired.pages", "Repaired paths must equal PR10 coverage.");
  for (const [index, page] of pages.entries()) {
    const expected = expectedForPath(page?.canonical_path);
    ...
    if (page?.legacy_intent !== expected?.legacyIntent) add("duplicate", "repaired_legacy_intent_invalid", `repaired.pages.${index}.legacy_intent`, "Legacy intent must match frozen PR10 identity.");
...
const validateFinal = (candidate, sourceLedger) => {
  const issues = [];
  const add = (gate, code, path, message) => issues.push({ gate, code, path, message });
  if (candidate.schema_version !== "big5_authority_v2_range_package.v1" || candidate.train_id !== "BIG5-AUTHORITY-V2-RANGE-OPENNESS-10" || candidate.stage !== "final" || candidate.framework !== "big_five" || candidate.domain_code !== "openness") add("schema", "final_identity_invalid", "final", "Final package identity is invalid.");
  const pages = Array.isArray(candidate.pages) ? candidate.pages : [];
  if (candidate.expected_page_count !== 8 || pages.length !== 8) add("coverage", "page_count_mismatch", "pages", "PR10 must contain exactly eight final pages.");
  if (JSON.stringify(sorted(pages.filter(isObject).map((page) => page.canonical_path))) !== JSON.stringify(sorted(expectedPaths()))) add("coverage", "canonical_coverage_mismatch", "pages", "Final canonical coverage must equal PR10's eight paths.");
  ...
  pages.forEach((page, pageIndex) => {
    ...
    if (!expected || page.content_key !== expected.key || page.locale !== expected.locale || page.route_family !== expected.family || page.range_level !== expected.level || page.legacy_intent !== expected.legacyIntent) add("coverage", "range_identity_mismatch", path, "Page identity must match the frozen PR10 route matrix.");
    if (page.page_family !== "range" || page.framework !== "big_five" || page.domain_code !== "openness") add("framework", "range_framework_invalid", path, "Page must remain an openness range page.");
    ...
    if (CROSS_FRAMEWORK_PATTERN.test(serialized)) add("framework", "cross_framework_leakage", path, "A different framework appears outside PR10 authority.");
  ...
  for (const level of ["high", "middle", "low"]) {
    const en = pageByPath.get(`/en/personality/big-five/openness-${level === "middle" ? "mid" : level}`);
    const zh = pageByPath.get(`/zh/personality/big-five/openness-${level === "middle" ? "mid" : level}`);
  ...
  const v2High = pageByPath.get("/en/personality/big-five/openness-high");
  const legacyHigh = pageByPath.get("/en/personality/big-five/high-openness");
  const v2Low = pageByPath.get("/en/personality/big-five/openness-low");
  const legacyLow = pageByPath.get("/en/personality/big-five/low-openness");
...
const validateWorkflow = (raw, skeptical, repaired, final, ledger) => {
  ...
  if (skeptical.schema_version !== "big5_authority_v2_skeptical_review.v1" || skeptical.train_id !== "BIG5-AUTHORITY-V2-RANGE-OPENNESS-10" || skeptical.reviewed_artifact !== raw.artifact || skeptical.review_status !== "repair_required") add("skeptical_review_invalid", "Skeptical review identity is invalid.");
  ...
  return {
    train_id: "BIG5-AUTHORITY-V2-RANGE-OPENNESS-10",
