...
class BigFiveAuthorityV227Test extends TestCase
{
    ...
    {
        parent::setUp();
        $this->packagePath = base_path('../generated/big-five-authority-v2/big5-authority-v2-article-relationships-27');
    }

    public function test_final_package_has_exact_batch_27_bilingual_inventory(): void
    {
        ...
        $this->assertSame([27], array_values(array_unique(array_column($package['assets'], 'batch'))));
        $this->assertCount(5, array_unique(array_column($package['assets'], 'unique_intent_key')));
    }

    public function test_every_asset_has_complete_structure_and_relationship_boundary(): void
    {
        $package = $this->readJson('final-package.json');
        ...
        foreach ($package['assets'] as $asset) {
            ...
            $terms = $asset['locale'] === 'en' ? ['consent', 'compatibility', 'relationship outcomes', 'unknown'] : ['同意', '匹配', '关系结局', 'unknown'];
    ...
    public function test_qa_proves_consent_primacy_and_zero_release_mutations(): void
    {
        ...
        $this->assertTrue($qa['checks']['consumes_only_pr21_batch_27']);
        $this->assertTrue($qa['checks']['consent_and_boundary_primacy']);
        $this->assertTrue($qa['checks']['compatibility_and_outcome_prediction_prohibited']);
