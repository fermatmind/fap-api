...
3. Before any CMS apply preflight, capture the fixed 2026-07-13 through 2026-08-09 production product baseline defined by `product_funnel_baseline.json` through read-only reporting only. Freeze the exact landing/start/completion/result/failure aggregates, each raw report SHA-256, capture timestamp, and active backend revision. Do not continue while its status is `pending_preapply_production_read`, any required source is pending, or its repository hash is not bound by `measurement_plan.json` and `manifest.json`.
