...
Before creating or committing either pointer candidate, apply acquires one bounded, server-enforced MySQL `READ` table lock over `occupations` and `index_states` on the same connection used by the Eloquent authority queries. This lock blocks every other session's ordinary writes to either authority table without requiring writer-process cooperation. Apply first completes the final 1016/1016 read under that lock, then holds it across both pointer candidate writes, the immutable pointer commit/readback, the last process and active-pointer drift guards, the root `rename`, and activated-pointer readback before releasing it in `finally`. Unsupported drivers, timeout, acquisition failure, final authority drift, or release failure fail closed; lock/revalidation failure therefore cannot strand a committed immutable pointer.
