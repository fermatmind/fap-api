...
ART_DIR="${BACKEND_DIR}/artifacts/pr44"
SERVE_PORT="${SERVE_PORT:-1844}"
DB_PATH="/tmp/pr44.sqlite"
...
SERVE_PORT="${SERVE_PORT}" ART_DIR="${ART_DIR}" bash "${BACKEND_DIR}/scripts/pr44_verify.sh"
...
PR44 Acceptance Summary
- pass_items:
  - event_payload_limiter: OK
  - event_payload_feature: OK
  - pack_seed_config_consistency: OK
  - dynamic_answers_submit: OK
  - phpunit(EventPayloadLimiterTest, EventPayloadLimitsTest): PASS
- key_outputs:
...
bash "${BACKEND_DIR}/scripts/sanitize_artifacts.sh" 44

bash -n "${BACKEND_DIR}/scripts/pr44_accept.sh"
bash -n "${BACKEND_DIR}/scripts/pr44_verify.sh"
