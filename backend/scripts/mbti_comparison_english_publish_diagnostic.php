...
set_exception_handler(static function (Throwable $throwable): never {
    ...
    fwrite(STDERR, "comparison_english_publish_diagnostic_failed:$errorCode\n");
    exit(1);
});
...
if (! is_file($backendRoot.'/artisan') || ! is_file($backendRoot.'/vendor/autoload.php')
    || ! is_file($sourceBackendRoot.'/app/Services/Cms/MbtiComparisonEnglishPublishDiagnosticService.php')
    || ! is_file($sourceBackendRoot.'/app/Services/ContentImport/MbtiComparisonEnglishPackageImporter.php')) {
    throw new RuntimeException('source_or_backend_not_bootstrapable');
}

require $backendRoot.'/vendor/autoload.php';
require $sourceBackendRoot.'/app/Services/ContentImport/MbtiComparisonEnglishPackageImporter.php';
require $sourceBackendRoot.'/app/Services/Cms/MbtiComparisonEnglishPublishDiagnosticService.php';
$app = require $backendRoot.'/bootstrap/app.php';
$app->make(Kernel::class)->bootstrap();

$diagnostic = $app->make(MbtiComparisonEnglishPublishDiagnosticService::class)->diagnose(
    $sourceBackendRoot.'/content_assets/en-content-parity/CONTROL-approvals/W1-MBTI-COMPARISONS/publish-diagnostic-readback-approval-2026-08-02.json',
    MbtiComparisonEnglishPublishDiagnosticService::APPROVAL_SHA256,
);
$diagnostic['control_plane_sha'] = $options['control-plane-sha'];
$diagnostic['active_revision'] = $options['active-revision'];
fwrite(STDOUT, json_encode($diagnostic, JSON_UNESCAPED_SLASHES | JSON_THROW_ON_ERROR).PHP_EOL);
