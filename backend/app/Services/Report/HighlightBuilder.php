...
class HighlightBuilder
{
    ...
    public function finalize(
        ...
        foreach ($normalized as $h) {
            ...
            if ($k === 'strength') {
                $strength[] = $h;
            } elseif ($k === 'blindspot' && ! $blindspot) {
                $blindspot = $h;
            } elseif ($k === 'action' && ! $action) {
                $action = $h;
            } else {
                $others[] = $h;
            }
    ...
    private function normalizeDoc($doc): ?array
    {
        ...
        if (isset($doc['doc'])) {
            $inner = $doc['doc'];
            if (is_object($inner)) {
                $inner = json_decode(json_encode($inner, JSON_UNESCAPED_UNICODE), true);
            }
            if (is_array($inner)) {
                $doc = $inner;
            }
