...
return new class extends Migration
{
    ...
    {
        $package = json_decode(
            file_get_contents(__DIR__.'/../data/assessment_riasec_parity_en_20260921.json'),
            true,
            512,
            JSON_THROW_ON_ERROR,
        );
        if (($package['schema_version'] ?? null) !== 'assessment.exact-field-copy.v2'
            || ($package['scale_code'] ?? null) !== 'RIASEC'
            || ($package['locale'] ?? null) !== 'en'
            || count($package['updates'] ?? []) !== 2) {
            throw new RuntimeException('RIASEC English parity package identity is invalid.');
        }
        ...
        DB::transaction(function () use ($package, $expectedTables): void {
            ...
            if ($availableTables !== $expectedTables) {
                throw new RuntimeException('RIASEC registry table set is incomplete; refusing partial update.');
            }
            ...
            if (count($presentRows) !== count($expectedTables)) {
                throw new RuntimeException('RIASEC registry row set is incomplete; refusing partial update.');
            }
            ...
            foreach ($rows as $table => $row) {
                ...
                if (! is_array($localized)) {
                    throw new RuntimeException($table.' RIASEC English content is not an object.');
                }
                $states[$table] = $this->classifyState($localized, $package['updates'], $table);
            }

            if (count(array_unique($states)) !== 1) {
                throw new RuntimeException('RIASEC registry targets are in a mixed state; refusing partial update.');
            }
            if (reset($states) === 'new') {
                return;
            }
    ...
    private function classifyState(array $localized, array $updates, string $table): string
    {
        ...
        foreach ($updates as $update) {
            ...
            $matchesExpected = $expectedExists
                ? $exists && $current === ($update['expected'] ?? null)
                : ! $exists;
            $matchesNew = $exists && $current === $update['value'];
        ...
        throw new RuntimeException($table.' RIASEC target fields are mixed or changed since review; refusing to overwrite.');
    }
    ...
    private function readTarget(array $localized, array $update, string $table): array
    {
        if (isset($update['path'])) {
            ...
            $value = data_get($localized, $update['path'], $missing);

            return [$value !== $missing, $value === $missing ? null : $value];
        }
