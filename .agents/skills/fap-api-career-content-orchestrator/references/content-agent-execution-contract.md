...
Gate inputs are canonical hashes, not a lossy previous-output chain. Gate 1 binds request, Current inventory, and source-policy version/hash. Gate 2 binds request, research candidate and Gate 1 output. Gate 3 binds request, research candidate, editorial output and the fixed C3.6A-R adapter version. Gate 4 binds request, evidence package, adapter output and the adapter-produced dimension-binding digest. Gate 5 binds request, the sorted per-slug dry-compile aggregate and Gate 4 output. Research, adapter and dry-compile aggregate artifact hashes equal their corresponding gate output hashes.
