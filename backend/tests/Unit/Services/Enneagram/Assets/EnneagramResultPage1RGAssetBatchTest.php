...
final class EnneagramResultPage1RGAssetBatchTest extends TestCase
{
    public function test_1r_g_batch_assets_are_runner_safe_and_not_runtime(): void
    {
        $root = base_path('content_assets/enneagram/result_page/batch_1r_g/v0_1');
        $manifest = $this->readJson($root.'/asset_batch_manifest.json');
        $assetsDoc = $this->readJson($root.'/assets.json');

        $this->assertSame('fap.enneagram.result_page.batch_1r_g.manifest.v0.1', $manifest['schema_version'] ?? null);
        ...
        $this->assertSame(162, (int) data_get($manifest, 'candidate_contract_reference.this_batch_asset_count'));
        $this->assertSame(162, (int) ($assetsDoc['asset_count'] ?? 0));
        $this->assertSame(162, count((array) ($assetsDoc['assets'] ?? [])));
        $this->assertSame('c399a93a35f8c769b93b4ab4040a62b59980b34cf8cac351da48af3f91efbf46', $manifest['source_asset_sha256'] ?? null);
        ...
        $this->assertCount(18, (array) data_get($manifest, 'coverage_counts.by_scene_axis'));
        ...
        $this->assertSame(162, (int) ($runnerReport['payload_count'] ?? 0));
        $this->assertSame(0, (int) ($runnerReport['expected_runner_failure_count'] ?? -1));
        $this->assertCount(162, (array) ($runnerReport['payload_hashes'] ?? []));

        $runner = new EnneagramResultPageAgentBatchRunner;
        $sourceLedgerRow = $this->sourceLedgerRow('batch_1r_g_asset_stream');
        $assetKeys = [];
        ...
        foreach ((array) ($assetsDoc['assets'] ?? []) as $asset) {
            ...
            $this->assertSame('1R-G', $asset['batch_scope'] ?? null);
            $this->assertSame('scene_localization_response', $asset['category'] ?? null);
            $this->assertNotSame('', (string) data_get($asset, 'routing_metadata.branch_metadata.scene_axis'));
            $this->assertNotSame('', (string) data_get($asset, 'routing_metadata.branch_metadata.scene_domain'));
            $this->assertNotSame('', (string) data_get($asset, 'routing_metadata.branch_metadata.scene_label_zh'));
            $this->assertSame('batch_1r_g_asset_stream', data_get($asset, 'source_trace.primary_source_id'));
        ...
        $this->assertCount(162, array_unique($assetKeys));
