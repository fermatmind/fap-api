...
final class CareerCurrentEnBatchMaterializer
{
    public const CONTRACT_VERSION = 'career.current.en_batch_materialization.v1';

    public const EXPECTED_SOURCE_AGGREGATE_SHA256 = CareerCurrentEnBatchPreparer::EXPECTED_SOURCE_AGGREGATE_SHA256;
    ...
    public function __construct(
        ...
        private readonly CareerCurrentEnBatchPreparer $preparer,
    ) {}
    ...
    public function materialize(
        ...
        if (preg_match('/\Abatch-[0-9]{3}\z/', $batchId) !== 1
            ...
            throw new CareerTenBlockCompileFailure('CURRENT_EN_MATERIALIZE_INPUT_INVALID');
        }
        if ($write) {
            if (! in_array($environment, ['local', 'testing'], true) || ! $this->isLinkedGitWorktree($backendRoot)) {
                throw new CareerTenBlockCompileFailure('CURRENT_EN_MATERIALIZE_WRITE_NOT_ALLOWED');
            }
            if ($outputRoot !== null && trim($outputRoot) !== '') {
                throw new CareerTenBlockCompileFailure('CURRENT_EN_MATERIALIZE_OUTPUT_FORBIDDEN');
            }
        } else {
        ...
        if (! is_string($baseAssetsSha256) || ! hash_equals($expectedBaseAssetsSha256, $baseAssetsSha256)) {
            throw new CareerTenBlockCompileFailure('CURRENT_EN_BASE_ASSETS_HASH_MISMATCH');
        }
        ...
        foreach ($targetSlugs as $slug) {
            if ($slug === 'software-developers' || ! isset($baseline['rows'][$slug])) {
                throw new CareerTenBlockCompileFailure('CURRENT_EN_TARGET_SET_INVALID');
            }
            $before = $baseline['rows'][$slug];
            $candidate = $this->preparer->candidateRowForSource($sourceRoot, $slug, $before);
            $candidateHash = $this->projectionHash($candidate, 'en');
            if (! hash_equals((string) ($plan['manifest']['per_slug'][$slug]['en_projection_sha256'] ?? ''), $candidateHash)) {
                throw new CareerTenBlockCompileFailure('CURRENT_EN_PLAN_PROJECTION_HASH_MISMATCH');
            }
            $beforeZh = $this->projectionHash($before, 'zh-CN');
            $afterZh = $this->projectionHash($candidate, 'zh-CN');
            if (! hash_equals($beforeZh, $afterZh)) {
                throw new CareerTenBlockCompileFailure('CURRENT_EN_ZH_PROJECTION_CHANGED');
            }
            if (hash_equals($this->projectionHash($before, 'en'), $candidateHash)) {
                $alreadyCurrent[] = $slug;
            }
            $candidateRows[$slug] = $candidate;
            $targetProjectionHashes[$slug] = $candidateHash;
            $zhBefore[$slug] = $beforeZh;
        }
        ...
        try {
            ...
            foreach (['presentation-source-registry.json', 'supporting-evidence-v1.json'] as $auxiliary) {
                ...
                if (is_file($sourcePath) && ! is_link($sourcePath)) {
                    ...
                    $this->writeFile($candidatePackageRoot.'/'.$auxiliary, $bytes);
                }
            }
            ...
            if (! is_string($candidateManifestBytes)) {
                throw new CareerTenBlockCompileFailure('CURRENT_EN_CANDIDATE_PACKAGE_INVALID');
            }

            $diff = $this->exactDiff($baseline['rows'], $candidate['rows'], $targetSlugs);
            if ($diff['changed_zh_locale_pages'] !== 0
                || $diff['non_target_row_changes'] !== 0
                || $diff['changed_en_locale_pages'] !== count($targetSlugs) - count($alreadyCurrent)) {
                throw new CareerTenBlockCompileFailure('CURRENT_EN_PACKAGE_DIFF_MISMATCH');
            }

            $report = [
                'status' => 'PASS_CURRENT_EN_BATCH_MATERIALIZE',
            ...
            if ($write) {
                ...
                if (! hash_equals($candidate['summary']['assets_sha256'], $written['summary']['assets_sha256'])
                    ...
                    throw new CareerTenBlockCompileFailure('CURRENT_EN_WRITTEN_PACKAGE_MISMATCH');
                }
                $report['write'] = [
    ...
    {
        if (($source['career_count'] ?? null) !== CareerCurrentEnBatchPreparer::EXPECTED_CAREERS
            || ($source['file_count'] ?? null) !== CareerCurrentEnBatchPreparer::EXPECTED_FILES
            || ! hash_equals(self::EXPECTED_SOURCE_AGGREGATE_SHA256, (string) ($source['aggregate_sha256'] ?? ''))) {
            throw new CareerTenBlockCompileFailure('CURRENT_EN_SOURCE_HASH_MISMATCH');
        }
    }
    ...
    private function selectBatch(array $plan, string $batchId): array
    {
        ...
        throw new CareerTenBlockCompileFailure('CURRENT_EN_BATCH_UNKNOWN');
    }
    ...
    private function assertSequence(array $rows, array $plan, int $requestedOrdinal): array
    {
        ...
        foreach ($plan['batches'] as $batch) {
            ...
            foreach ($batch['target_slugs'] as $slug) {
                $expected = (string) ($plan['manifest']['per_slug'][$slug]['en_projection_sha256'] ?? '');
                if (isset($rows[$slug]) && hash_equals($expected, $this->projectionHash($rows[$slug], 'en'))) {
                    $current++;
                }
            }
            ...
            if ($current !== 0) {
                if ((int) $batch['ordinal'] !== $requestedOrdinal) {
                    throw new CareerTenBlockCompileFailure('CURRENT_EN_BATCH_SEQUENCE_INVALID');
                }
                break;
            }
            break;
        ...
        if ($requestedOrdinal <= $completed) {
            throw new CareerTenBlockCompileFailure('CURRENT_EN_BATCH_ALREADY_MATERIALIZED');
        }
    ...
    {
        $path = tempnam(sys_get_temp_dir(), 'career-current-en-');
        if (! is_string($path) || ! unlink($path) || ! mkdir($path, 0700)) {
            throw new CareerTenBlockCompileFailure('CURRENT_EN_TEMP_WRITE_FAILED');
        }
    ...
    {
        $temporary = tempnam(dirname($path), '.career-current-en-');
        if (! is_string($temporary)) {
            throw new CareerTenBlockCompileFailure('CURRENT_EN_CURRENT_WRITE_FAILED');
        }
        try {
            $mode = fileperms($path);
