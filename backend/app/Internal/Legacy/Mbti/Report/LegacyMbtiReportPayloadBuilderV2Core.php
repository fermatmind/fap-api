...
class LegacyMbtiReportPayloadBuilderV2Core
{
    ...
    private function buildHighlights(array $scoresPct, array $axisStates, string $typeCode, string $contentPackageVersion): array
    {
        ...
        if (! is_array($allowedLvls)) {
            $allowedLvls = ['clear', 'strong', 'very_strong'];
        }
        if (! is_array($levelOrder)) {
            $levelOrder = ['very_weak', 'weak', 'moderate', 'clear', 'strong', 'very_strong'];
        }
    ...
    private function buildRoleCard(string $contentPackageVersion, string $typeCode): array
    {
        ...
        if (! is_array($out['theme'] ?? null)) {
            $out['theme'] = ['color' => ''];
        }
        if (! is_string($out['theme']['color'] ?? '')) {
            $out['theme']['color'] = '';
        }
        if (! is_array($out['tags'] ?? null)) {
            $out['tags'] = [];
        }
    ...
    private function buildRecommendedReads(string $contentPackageVersion, string $typeCode, array $scoresPct, int $max = 8): array
    {
        ...
        if (! is_array($forbidTags)) {
            $forbidTags = [];
        }

        $requireAnyTags = $rules['require_any_tags'] ?? [];
        if (! is_array($requireAnyTags)) {
            $requireAnyTags = [];
        }

        $requireAllTags = $rules['require_all_tags'] ?? [];
        if (! is_array($requireAllTags)) {
            $requireAllTags = [];
        }
        ...
        $dupCheck = function (array $it) use (
            ...
            foreach ($softBy as $k) {
                ...
                if ($k === 'canonical_url') {
                    ...
                    if ($key !== '' && isset($seenCUrl[$key])) {
                        return ['by' => 'canonical_urlKey', 'key' => $key];
                    }
                ...
                if ($k === 'url') {
                    $key = $normalizeUrlKey($v);
                    if ($key !== '' && isset($seenUrl[$key])) {
                        return ['by' => 'urlKey', 'key' => $key];
                    }
        ...
        $normalize = function (array $it) use ($defaults, $getNonEmptyString): array {
            ...
            if (array_key_exists('cta', $it)) {
                $out['cta'] = (string) $it['cta'];
            }
            if (array_key_exists('estimated_minutes', $it)) {
                $out['estimated_minutes'] = (int) $it['estimated_minutes'];
            }
            if (array_key_exists('locale', $it)) {
                $out['locale'] = (string) $it['locale'];
            }
            if (array_key_exists('access', $it)) {
                $out['access'] = (string) $it['access'];
            }
            if (array_key_exists('channel', $it)) {
                $out['channel'] = (string) $it['channel'];
            }
            if (array_key_exists('status', $it)) {
                $out['status'] = (string) $it['status'];
            }
            if (array_key_exists('published_at', $it)) {
                $out['published_at'] = (string) $it['published_at'];
            }
            if (array_key_exists('updated_at', $it)) {
                $out['updated_at'] = (string) $it['updated_at'];
            }
        ...
        $resolveCap = function ($capRaw, int $remaining): int {
            if (is_string($capRaw)) {
                ...
                if ($s === 'remaining' || $s === '*' || $s === 'all') {
                    return $remaining;
                }
                if (is_numeric($capRaw)) {
                    return (int) $capRaw;
                }

                return 0;
            }
            if (is_int($capRaw) || is_float($capRaw)) {
                return (int) $capRaw;
            }
        ...
        foreach ($fillOrder as $bucketName) {
            ...
            foreach ($list as $it) {
                ...
                if ($taken >= $cap) {
                    if ($debugReads) {
                        $debug['buckets'][$bucketName]['stop_cap'] = true;
                    }
                    break;
                }
                ...
                if (! is_array($it)) {
                    if ($debugReads) {
                        $debug['buckets'][$bucketName]['skip_invalid']++;
                    }
                ...
                if ($id === '') {
                    if ($debugReads) {
                        $debug['buckets'][$bucketName]['skip_no_id']++;
                    }
                ...
                if ($debugReads) {
                    $debug['buckets'][$bucketName]['taken']++;
                }
            }
        }
        ...
        if ($minItems > 0 && count($out) < min($minItems, $maxItems)) {
            ...
            if ($need > 0 && is_array($bucketLists['fallback'] ?? null)) {
                foreach ($bucketLists['fallback'] as $it) {
                    ...
                    if (! is_array($it)) {
                        if ($debugReads) {
                            $debug['min_items_fill']['skip_invalid']++;
                        }
                    ...
                    if ($id === '') {
                        if ($debugReads) {
                            $debug['min_items_fill']['skip_no_id']++;
                        }
    ...
    private function pickTraitsB(array $matched, string $typeCode, array $axisInfo, int $max = 4): array
    {
        ...
        $strict = array_values(array_filter($matched, function ($c) {
            ...
            foreach ($tags as $t) {
                ...
                if ($t === 'kind:intro' || $t === 'kind:axis_intro') {
                    return false;
                }
            }
        ...
        if (count($top) < $max) {
            ...
            foreach ($more as $c) {
                ...
                // 去重：id + dim
                if ($this->containsId($top, (string) ($c['id'] ?? ''))) {
                    continue;
                }
                $d = $this->cardAxisDim($c);
                if ($d && isset($hasDim[$d])) {
    ...
    private function cardMatches(array $card, string $typeCode, array $axisInfo): bool
    {
        ...
        if (isset($match['type'])) {
            ...
            if ($t && isset($t['type_code']) && (string) $t['type_code'] !== $typeCode) {
                return false;
            }
        }
    ...
    private function buildSectionCardsFromAssets(
        ...
        if ($maxCards < $targetCards) {
            $maxCards = max($targetCards, 6);
        }

        $fallbackTags = $rules['fallback_tags'] ?? ['kind:core'];
        if (! is_array($fallbackTags)) {
            $fallbackTags = ['kind:core'];
        }
