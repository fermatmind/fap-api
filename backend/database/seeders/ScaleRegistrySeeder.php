...
final class ScaleRegistrySeeder extends Seeder
{
        ...
        $scale = $this->upsertMbtiPreservingPublishedContent($writer, [
            ...
            'content_i18n_json' => $this->catalogContent(
                ...
                zhFaq: [
                    [
                        'q' => 'MBTI 测试免费吗？',
                        'a' => '可以免费完成 MBTI 主测试，并查看 16 型人格结果、偏好维度与后续探索建议。页面不会把基础结果说明作为付费前置条件。',
                    ],
                    [
                        'q' => 'MBTI 完整结果能看到什么？',
                        'a' => '完成后会看到你的 16 型人格类型、E/I、S/N、T/F、J/P 偏好方向，以及用于沟通、自我观察和职业探索的解释。结果是结构化参考，不是能力、健康或未来结果判断。',
                    ],
                    [
                        'q' => 'MBTI 测试一般多久？',
                        'a' => '不同版本题量不同，大多数用户会在一次连续作答中完成；短版适合快速了解，长版适合更细致地观察偏好。完成时间不代表结果质量。',
                    ],
                    [
                        'q' => 'MBTI 能决定职业吗？',
                        'a' => '不能。MBTI 可以帮助你描述沟通、决策和精力偏好，作为职业探索的一个参考，但不能单独决定职业选择、录用结果或发展上限。',
                    ],
                    [
                        'q' => 'MBTI 是心理诊断吗？',
                        'a' => '不是。MBTI 测试用于自我了解和讨论起点，不能替代心理诊断、治疗建议或医疗专业意见。',
                    ],
                    [
                        'q' => '16 型人格结果会变吗？',
                        'a' => '可能会。不同时间、处境和作答理解会影响结果表达；建议把变化看作自我观察线索，而不是固定身份标签。',
                    ],
                    [
                        'q' => 'MBTI 和大五人格有什么区别？',
                        'a' => 'MBTI 用 4 组偏好组合成 16 型人格；大五人格更像连续维度评分。两者都适合做自我观察，但解释方式不同。',
                    ],
                    [
                        'q' => '做完 MBTI 后下一步看什么？',
                        'a' => '可以先阅读结果里的偏好解释，再结合沟通、学习、职业探索场景做记录；如果想交叉观察，也可以继续了解 MBTI 基础文章或大五人格测试。',
                    ],
                ],
            ),
    ...
    private function upsertAssessmentPreservingFaq(ScaleRegistryWriter $writer, array $attributes): \App\Models\ScaleRegistry
    {
        ...
        $copyPackage = json_decode(file_get_contents(database_path('data/assessment_riasec_enneagram_copy_zh_20260908.json')), true, 512, JSON_THROW_ON_ERROR);
        foreach ($copyPackage['scales'][$attributes['code']]['updates'] ?? [] as $update) {
            if (isset($update['path'])) {
                data_set($attributes['content_i18n_json']['zh'], $update['path'], $update['value']);
