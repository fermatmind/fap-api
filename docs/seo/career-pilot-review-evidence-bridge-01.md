...
Rejected, exception, partial, duplicate, malformed, missing, cold, and stale evidence projects `unknown` with no review timestamp for the governed pilot slugs. Any visible content, SEO, structured-data, claim-permission, warning, source, or truth-layer drift changes the target/package SHA and invalidates the whole batch.
