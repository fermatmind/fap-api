...
use App\Domain\Career\Compilation\CareerCurrentEnBatchMaterializer;
...
final class CareerCurrentEnBatchMaterializerTest extends TestCase
{
    ...
    public function test_full_batch_contract_is_deterministic_and_ordered_when_canonical_source_is_available(): void
    {
        ...
        $roots = [];
        try {
            $planRoot = $roots[] = $this->temporaryDirectory('career-en-plan-');
            $plan = app(CareerCurrentEnBatchPreparer::class)->prepare(
                self::SOURCE,
                $planRoot,
                50,
                str_repeat('a', 40),
            ...
            self::assertSame(CareerCurrentEnBatchMaterializer::EXPECTED_SOURCE_AGGREGATE_SHA256, $plan['manifest']['source_aggregate_sha256']);
            ...
            $materializer = app(CareerCurrentEnBatchMaterializer::class);
            ...
            foreach ($plan['manifest']['per_slug'] as $slug => $entry) {
                ...
                $hash = CareerCurrentAuthorityPackage::hashValue($package->publicProjection($current['rows'][$slug], 'en'));
                if (hash_equals((string) $entry['en_projection_sha256'], $hash)) {
            ...
            $firstOutput = $roots[] = $this->temporaryDirectory('career-en-next-batch-a-');
            ...
            self::assertSame($firstTargetCount, $first['diff']['changed_en_locale_pages']);
            self::assertSame(0, $first['diff']['changed_zh_locale_pages']);
            ...
            $secondOutput = $roots[] = $this->temporaryDirectory('career-en-next-batch-b-');
            ...
            $batchOneBackend = $roots[] = $this->temporaryDirectory('career-en-next-backend-');
            ...
            if ($firstOrdinal < 21) {
                ...
                $batchTwoOutput = $roots[] = $this->temporaryDirectory('career-en-following-batch-');
                ...
                self::assertSame((int) $plan['report']['batch_sizes'][$firstOrdinal], $batchTwo['diff']['changed_en_locale_pages']);
                self::assertSame(0, $batchTwo['diff']['changed_zh_locale_pages']);
                self::assertSame(0, $batchTwo['diff']['non_target_row_changes']);
            }

            $wrongBaseOutput = $roots[] = $this->temporaryDirectory('career-en-wrong-base-');
            $this->expectFailure('CURRENT_EN_BASE_ASSETS_HASH_MISMATCH', fn () => $materializer->materialize(
                self::SOURCE,
                $planRoot,
                'batch-001',
                str_repeat('0', 64),
            ...
            $skippedOutput = $roots[] = $this->temporaryDirectory('career-en-skipped-');
            $this->expectFailure('CURRENT_EN_BATCH_SEQUENCE_INVALID', fn () => $materializer->materialize(
                self::SOURCE,
                $planRoot,
            ...
            $duplicateOutput = $roots[] = $this->temporaryDirectory('career-en-duplicate-');
            $this->expectFailure('CURRENT_EN_BATCH_ALREADY_MATERIALIZED', fn () => $materializer->materialize(
                self::SOURCE,
                $planRoot,
                $firstBatchId,
                $batchOneSha,
            ...
            self::assertSame($sourceBefore, app(CareerCurrentEnBatchPreparer::class)->inspectSource(self::SOURCE)['aggregate_sha256']);
        } finally {
    ...
    {
        $source = (string) file_get_contents(base_path('app/Console/Commands/CareerMaterializeCurrentEnBatch.php'));

        self::assertStringNotContainsString('--target=', $source);
        self::assertStringContainsString("['local', 'testing']", (string) file_get_contents(
            base_path('app/Domain/Career/Compilation/CareerCurrentEnBatchMaterializer.php'),
        ));
    ...
    {
        $sourceRoot = $this->temporaryDirectory('career-en-wrong-source-');
        $planRoot = $this->temporaryDirectory('career-en-unused-plan-');
        $outputRoot = $this->temporaryDirectory('career-en-unused-output-');
        try {
            $this->expectFailure('CURRENT_EN_AUTHORITY_HASH_MISMATCH', fn () => app(CareerCurrentEnBatchMaterializer::class)->materialize(
                $sourceRoot,
                $planRoot,
                'batch-001',
                str_repeat('0', 64),
