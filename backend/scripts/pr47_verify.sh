...
ART_DIR="${ART_DIR:-${BACKEND_DIR}/artifacts/pr47}"
SERVE_PORT="${SERVE_PORT:-1847}"
HOST="127.0.0.1"
API_BASE="http://${HOST}:${SERVE_PORT}"
SCALE_CODE="${SCALE_CODE:-MBTI}"
ANON_ID="${ANON_ID:-pr47-verify-anon}"
WEBHOOK_SECRET="${INTEGRATIONS_WEBHOOK_MOCK_SECRET:-pr47_mock_secret}"
...
cd "${BACKEND_DIR}"
php artisan test --filter ProviderWebhookSignatureTest > "${ART_DIR}/phpunit.txt"
cd "${REPO_DIR}"
