...
final class PersonalityDesktopCloneBaselineReader
{
    public function resolveSourceDir(?string $sourceDir = null): string
    {
        ...
        if ($candidate === '') {
            $candidate = base_path('../content_baselines/personality_clone');
        ...
        if ($resolved === false || ! is_dir($resolved)) {
            throw new RuntimeException(sprintf(
                'Personality desktop clone baseline source directory not found: %s',
                $candidate,
            ));
        }
    ...
    public function read(string $sourceDir, array $selectedLocales = []): array
    {
        ...
        $files = $locales === []
            ? glob(rtrim($sourceDir, DIRECTORY_SEPARATOR).DIRECTORY_SEPARATOR.'mbti_desktop_clone.*.json') ?: []
            : array_map(
                static fn (string $locale): string => rtrim($sourceDir, DIRECTORY_SEPARATOR)
                    .DIRECTORY_SEPARATOR
                    .sprintf('mbti_desktop_clone.%s.json', $locale),
                $locales,
            );

        if ($files === []) {
            throw new RuntimeException(sprintf(
                'No personality desktop clone baseline files found in %s.',
                $sourceDir,
            ));
        }
        ...
        foreach ($files as $file) {
            if (! is_file($file)) {
                throw new RuntimeException(sprintf(
                    'Personality desktop clone baseline file missing: %s',
                    $file,
                ));
            }
            ...
            if (! is_string($raw) || trim($raw) === '') {
                throw new RuntimeException(sprintf(
                    'Personality desktop clone baseline file is empty: %s',
                    $file,
                ));
            }
            ...
            if (! is_array($decoded)) {
                throw new RuntimeException(sprintf(
                    'Personality desktop clone baseline file is not valid JSON: %s',
                    $file,
                ));
            }
