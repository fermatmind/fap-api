...
final class RiasecResultPageProductionRolloutGate
{
    ...
    public function decide(Attempt $attempt): RiasecResultPageProductionRolloutDecision
    {
        ...
        if ((bool) config('riasec_result_page_v2.production_emergency_disabled', false)) {
            return $this->deny('production_rollout_emergency_disabled', $context);
        }

        if (! (bool) config('riasec_result_page_v2.production_runtime_enabled', false)) {
            return $this->deny('production_runtime_disabled', $context);
        }

        if (! (bool) config('riasec_result_page_v2.production_rollout_enabled', false)) {
            return $this->deny('production_rollout_disabled', $context);
        }

        if (! (bool) config('riasec_result_page_v2.production_rollout_configured', false)) {
            return $this->deny('production_rollout_not_configured', $context);
        }

        if (! (bool) config('riasec_result_page_v2.production_rollout_manual_approval_granted', false)) {
            return $this->deny('production_rollout_manual_approval_missing', $context);
        }

        if (! (bool) config('riasec_result_page_v2.production_import_gate_passed', false)) {
            return $this->deny('production_rollout_import_gate_missing', $context);
        }

        $releaseFailure = $this->releaseFailureReason();
        if ($releaseFailure !== null) {
            return $this->deny($releaseFailure, $context);
        }

        $configErrors = $this->configErrors();
        if ($configErrors !== []) {
            return new RiasecResultPageProductionRolloutDecision(
                false,
        ...
        if ($scopeFailure !== null) {
            return $this->deny($scopeFailure, $context);
        }

        $mode = (string) config('riasec_result_page_v2.production_rollout_mode', 'disabled');
        if ($mode === 'disabled') {
            return $this->deny('production_rollout_mode_disabled', $context);
        }
        ...
        if ($mode === 'allowlist_only') {
            return $this->deny('production_rollout_allowlist_denied', $context);
        }
        ...
        return $this->deny('production_rollout_percentage_denied', $context);
    }
    ...
    /** @param array<string,string|int> $context */
    private function deny(string $reason, array $context): RiasecResultPageProductionRolloutDecision
    {
        return new RiasecResultPageProductionRolloutDecision(false, $reason, null, $context);
    }
}
