...
- The candidate's full coverage inspection before each offline batch follows the same read-only retry boundary: retry exactly once only for an explicitly classified transient database read, preserve a safe pre-batch stage/category/attempt receipt, and stop without a write on permanent or unclassified failure. Candidate-runtime initialization failures are classified but never retried.
