...
Approval requires a fresh MFA/TOTP or recovery code submitted for that exact high-risk action, a non-empty reason, a UUID correlation ID, and a supported registered operation type. The normal login-session TOTP marker is not accepted as high-risk step-up proof, and disabling the general admin TOTP switch does not bypass this R3 gate. Approve, legacy evidence binding, execute, reject, and retry each verify their own time-bound TOTP or one-time recovery code before mutation or dispatch. The adapter computes a deterministic fingerprint over the exact approval ID, operation surface, organization, requester, reason, correlation ID, and business payload. The approval record stores only versioned private governance metadata, the target fingerprint, reason hash, evidence SHA-256, actor IDs, review mode, approval timestamp, and the fact that step-up succeeded. TOTP values, recovery codes, tokens, secrets, passwords, authorization headers, cookies, and API keys are never stored in governance metadata or written to audit/error output.

The `AdminApproval` model rejects credential-bearing reasons before an Eloquent request row can be created or updated, so request-side audit code cannot receive a successfully persisted raw secret reason. The approval service repeats the same validation for legacy or direct-database rows and redacts separator-prefixed and camelCase credential labels in flat errors, nested audit metadata, and stored failure messages.

Approve and execute are separate transitions. Approving records evidence and never dispatches the operation. A later explicit execute action binds the freshly stepped-up execution actor, authorization timestamp, execution-attempt number, and exact approval evidence SHA into a separate hashed private authorization record before queueing the existing worker. A repeat execute by the same actor requires another fresh step-up and refreshes the short-lived timestamp and authorization hash for that unchanged attempt; a different actor cannot replace an active authorization. Before that binding or dispatch, grant, revoke, and payment-event reprocess require `admin.ops.write`; refund requires both `admin.finance.write` and the order mutation gate's `admin.ops.write`; rollback requires `admin.content.release` or `admin.owner`. The worker repeats the same domain permission check, validates the authorization, performs the domain action as that execution actor, and attributes success/failure audit records to the same actor. It also recomputes and verifies the exact target fingerprint and approval evidence SHA before changing the approval to `EXECUTING`. Any payload, reason, correlation, approval actor, execution actor, mode, schema, timestamp, attempt, permission, or evidence drift fails closed before action dispatch. Duplicate approval is idempotent only when the existing immutable evidence still matches.

Legacy `APPROVED` or `FAILED` rows created before this adapter may bind missing governance evidence through a separate explicit action. Binding requires current step-up, the original approver, the current configured review policy, a valid original approval timestamp, and a completely absent governance key; it preserves status and queues no operation. Existing malformed or drifted governance evidence is never overwritten. Execution or retry remains a later separate action after successful binding.
