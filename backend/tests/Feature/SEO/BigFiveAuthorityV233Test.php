...
class BigFiveAuthorityV233Test extends TestCase
{
    ...
    {
        parent::setUp();
        $this->packagePath = base_path('../generated/big-five-authority-v2/big5-authority-v2-article-research-briefings-33');
    }

    public function test_final_package_has_exact_batch_33_bilingual_inventory(): void
    {
        ...
        $this->assertSame([33], array_values(array_unique(array_column($package['assets'], 'batch'))));
        $this->assertCount(5, array_unique(array_column($package['assets'], 'unique_intent_key')));
    }

    public function test_every_asset_has_complete_structure_and_research_boundary(): void
    {
        $package = $this->readJson('final-package.json');
        ...
        $requiredSources = ['academic.goldberg-1990-big-five-structure', 'academic.soto-john-2017-bfi2', 'internal.public-claim-boundary-matrix'];
        foreach ($package['assets'] as $asset) {
            $this->assertSame($required, array_column($asset['sections'], 'key'));
            $this->assertSame($requiredSources, array_column($asset['source_mapping'], 'source_id'));
            $this->assertCount(3, $asset['internal_link_targets']);
            $this->assertTrue($asset['research_verification']['time_sensitive_facts_web_verified']);
            $this->assertTrue($asset['research_verification']['primary_papers_preferred']);
            $this->assertSame('2026-07-14', $asset['research_verification']['access_date']);
            foreach ($asset['source_mapping'] as $source) {
                $this->assertNotEmpty($source['publication_year']);
                $this->assertSame('2026-07-14', $source['access_date']);
            }
            $boundary = strtolower(collect($asset['sections'])->firstWhere('key', 'method_product_boundary')['body_md']);
            $terms = $asset['locale'] === 'en' ? ['primary-paper', 'publication year', 'access date', 'unknown'] : ['原始论文', '出版年份', '访问日期', 'unknown'];
    ...
    public function test_qa_proves_web_verification_and_zero_release_mutations(): void
    {
        ...
        $this->assertTrue($qa['checks']['consumes_only_pr21_batch_33']);
        $this->assertTrue($qa['checks']['primary_paper_metadata_web_verified']);
        $this->assertTrue($qa['checks']['publication_year_and_access_date_visible']);
        $this->assertTrue($qa['checks']['evidence_to_claim_boundary_preserved']);
