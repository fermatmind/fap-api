...
final class RiasecResultPageProductionRolloutGate
{
    ...
    public function decide(Attempt $attempt): RiasecResultPageProductionRolloutDecision
    {
        $context = $this->context($attempt);

        if ((bool) config('riasec_result_page_v2.production_emergency_disabled', false)) {
            return new RiasecResultPageProductionRolloutDecision(false, 'production_rollout_emergency_disabled', null, $context);
        }

        if (! (bool) config('riasec_result_page_v2.production_rollout_enabled', false)) {
            return new RiasecResultPageProductionRolloutDecision(false, 'production_rollout_disabled', null, $context);
        }

        if (! (bool) config('riasec_result_page_v2.production_rollout_configured', false)) {
            return new RiasecResultPageProductionRolloutDecision(false, 'production_rollout_not_configured', null, $context);
        }

        if (! (bool) config('riasec_result_page_v2.production_rollout_manual_approval_granted', false)) {
            return new RiasecResultPageProductionRolloutDecision(false, 'production_rollout_manual_approval_missing', null, $context);
        }

        if (! (bool) config('riasec_result_page_v2.production_import_gate_passed', false)) {
            return new RiasecResultPageProductionRolloutDecision(false, 'production_rollout_import_gate_missing', null, $context);
        }

        $releaseFailure = $this->releaseFailureReason();
        if ($releaseFailure !== null) {
            return new RiasecResultPageProductionRolloutDecision(false, $releaseFailure, null, $context);
        }

        $configErrors = $this->configErrors();
        if ($configErrors !== []) {
            return new RiasecResultPageProductionRolloutDecision(false, 'production_rollout_invalid_config', null, $context, $configErrors);
        }

        $scopeFailure = $this->scopeFailureReason($context);
        if ($scopeFailure !== null) {
            return new RiasecResultPageProductionRolloutDecision(false, $scopeFailure, null, $context);
        }

        $mode = (string) config('riasec_result_page_v2.production_rollout_mode', 'disabled');
        if ($mode === 'disabled') {
            return new RiasecResultPageProductionRolloutDecision(false, 'production_rollout_mode_disabled', null, $context);
        }

        $allowlistMatch = $this->allowlistMatch($context);
        if ($allowlistMatch !== null) {
            return new RiasecResultPageProductionRolloutDecision(true, 'production_rollout_allowed', $allowlistMatch, $context);
        }

        if ($mode === 'allowlist_only') {
            return new RiasecResultPageProductionRolloutDecision(false, 'production_rollout_allowlist_denied', null, $context);
        }

        if ($this->percentageAllows($context)) {
            return new RiasecResultPageProductionRolloutDecision(true, 'production_rollout_allowed', 'rollout_percentage', $context);
        }

        return new RiasecResultPageProductionRolloutDecision(false, 'production_rollout_percentage_denied', null, $context);
    }
