...
class BigFiveAuthorityV231Test extends TestCase
{
    ...
    {
        parent::setUp();
        $this->packagePath = base_path('../generated/big-five-authority-v2/big5-authority-v2-article-research-methods-31');
    }

    public function test_final_package_has_exact_batch_31_bilingual_inventory(): void
    {
        ...
        $this->assertSame([31], array_values(array_unique(array_column($package['assets'], 'batch'))));
        $this->assertCount(5, array_unique(array_column($package['assets'], 'unique_intent_key')));
    }

    public function test_every_asset_has_complete_structure_and_unknown_boundary(): void
    {
        $package = $this->readJson('final-package.json');
        ...
        foreach ($package['assets'] as $asset) {
            ...
            $terms = $asset['locale'] === 'en' ? ['instrument', 'reliability', 'percentiles', 'unknown'] : ['工具', '信度', '百分位', 'unknown'];
    ...
    public function test_qa_proves_exact_instrument_scope_and_zero_release_mutations(): void
    {
        ...
        $this->assertTrue($qa['checks']['consumes_only_pr21_batch_31']);
        $this->assertTrue($qa['checks']['exact_instrument_evidence_only']);
        $this->assertTrue($qa['checks']['no_coefficient_transfer_to_fermatmind']);
        $this->assertTrue($qa['checks']['all_product_metrics_unknown']);
