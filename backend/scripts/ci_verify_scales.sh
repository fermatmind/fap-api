...
echo "[CI][scales] scale_scope=${SCALE_SCOPE} run_big5_ocean_gate=${RUN_BIG5_OCEAN_GATE} run_sds_20_gate=${RUN_SDS_20_GATE} run_sds_norms_gate=${RUN_SDS_NORMS_GATE} run_full_scale_regression=${RUN_FULL_SCALE_REGRESSION} run_scale_identity_contract=${RUN_SCALE_IDENTITY_CONTRACT} run_scale_identity_hard_cutover=${RUN_SCALE_IDENTITY_HARD_CUTOVER} run_scale_identity_gate=${RUN_SCALE_IDENTITY_GATE}"
echo "[CI][scales] running MBTI baseline gate"
RUN_SCALE_IDENTITY_CONTRACT="${RUN_SCALE_IDENTITY_CONTRACT}" \
RUN_SCALE_IDENTITY_HARD_CUTOVER="${RUN_SCALE_IDENTITY_HARD_CUTOVER}" \
