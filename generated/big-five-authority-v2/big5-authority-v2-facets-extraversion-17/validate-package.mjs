...
const DIR = "generated/big-five-authority-v2/big5-authority-v2-facets-extraversion-17";
const LEDGER = "generated/big-five-authority-v2/big5-authority-v2-source-ledger-05/source-ledger.json";
const TRAIN_ID = "BIG5-AUTHORITY-V2-FACETS-EXTRAVERSION-17";
const DOMAIN = "extraversion";
const TAXONOMY = "ipip_neo_30_facet_navigation";
const FACETS = ["warmth", "gregariousness", "assertiveness", "activity", "excitement-seeking", "positive-emotions"];
...
const validateRaw = (raw) => {
  ...
  if (raw.train_id !== TRAIN_ID || raw.stage !== "raw" || raw.framework !== "big_five" || raw.domain_code !== DOMAIN || raw.taxonomy !== TAXONOMY || raw.expected_page_count !== 12 || pages.length !== 12) add("schema", "raw_identity_invalid", "raw", "Raw package identity and page count must match PR17.");
  ...
  pages.forEach((page, index) => {
    ...
    if (!FACETS.includes(page?.facet_code) || !LOCALES.includes(page?.locale)) add("coverage", "raw_facet_identity_invalid", path, "Raw facet identity is outside PR17.");
...
const validateRepaired = (repaired) => {
  ...
  if (repaired.train_id !== TRAIN_ID || repaired.stage !== "repaired" || repaired.framework !== "big_five" || repaired.domain_code !== DOMAIN || repaired.taxonomy !== TAXONOMY || repaired.expected_page_count !== 12 || pages.length !== 12) add("schema", "repaired_identity_invalid", "repaired", "Repaired package identity and page count must match PR17.");
  if (JSON.stringify(sorted(repaired.required_intents ?? [])) !== JSON.stringify(sorted(REQUIRED_INTENTS))) add("schema", "repaired_intents_invalid", "repaired.required_intents", "Repaired intents must equal PR17 scope.");
...
const validateFinal = (candidate, ledger) => {
  ...
  if (candidate.train_id !== TRAIN_ID || candidate.stage !== "final" || candidate.framework !== "big_five" || candidate.domain_code !== DOMAIN || candidate.taxonomy !== TAXONOMY || candidate.expected_page_count !== 12 || pages.length !== 12) add("schema", "final_identity_invalid", "final", "Final package identity and page count must match PR17.");
  if (JSON.stringify(sorted(pages.filter(isObject).map((page) => page.canonical_path))) !== JSON.stringify(sorted(expectedPaths()))) add("coverage", "canonical_coverage_mismatch", "pages", "Final canonical coverage must equal PR17's twelve paths.");
  ...
  pages.forEach((page, index) => {
    ...
    if (!identityOk || page.page_family !== "facet_detail" || page.route_family !== "facet_canonical" || page.framework !== "big_five" || page.domain_code !== DOMAIN || page.taxonomy !== TAXONOMY) add("coverage", "facet_identity_mismatch", path, "Page identity must match the locked PR17 facet matrix.");
    ...
    if (CROSS_FRAMEWORK_PATTERN.test(serialized)) add("framework", "cross_framework_leakage", path, "A different framework appears outside PR17 authority.");
