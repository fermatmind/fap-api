...
final class EnneagramPublicAuthorityV210Type1FamilyTest extends TestCase
{
    private const PACKAGE = 'docs/seo/personality/enneagram-authority-v2/enneagram-public-authority-v2-type-1-family-10/type-1-family-draft.json';

    private const QA_REPORT = 'docs/seo/personality/enneagram-authority-v2/enneagram-public-authority-v2-type-1-family-10/qa-report.json';
    ...
    private const FAMILY_IDENTITIES = [
        ...
        'wing:1w9',
        'wing:1w2',
        'instinctual_subtype:type-1/self-preservation',
        'instinctual_subtype:type-1/social',
        'instinctual_subtype:type-1/one-to-one',
    ];

    public function test_package_contains_exactly_the_twelve_frozen_type_one_family_targets(): void
    {
        ...
        $this->assertSame('ENNEAGRAM-PUBLIC-AUTHORITY-V2-TYPE-1-FAMILY-10', $package['artifact']);
        ...
        $this->assertSame(
            ['core_type' => 2, 'instinctual_subtype' => 6, 'wing' => 4],
        ...
        foreach ($assets as $key => $asset) {
            ...
            foreach ($map['limitations'] as $limitation) {
                $this->assertContains($limitation, $asset['visible_evidence']['limitations'], "{$key}.visible_evidence.limitations");
            }
    ...
    public function test_locales_are_independent_faqs_are_unique_and_release_truth_is_closed(): void
    {
        $package = $this->package();
        ...
        $this->assertCount(36, $faqQuestions);
        $this->assertCount(36, $faqQuestions->map(fn (string $value): string => $this->normalize($value))->unique());
        $this->assertCount(36, $faqAnswers->map(fn (string $value): string => $this->normalize($value))->unique());
        $this->assertSame([false], array_values(array_unique($package['execution_boundaries'])));
    }

    public function test_core_pages_cover_the_required_type_one_editorial_dimensions(): void
    {
        $cores = collect($this->package()['assets'])->where('entity_type', 'core_type');
        $requiredKinds = [
            'motivation_attention',
            'behavior_motive',
            'decision_feedback_conflict',
            'strengths_tradeoffs',
            'work_relationship_examples',
            'wings_mistypes',
            'counterexample',
            'use_boundary',
        ];
        ...
        foreach ($cores as $asset) {
            $key = $this->key($asset);
            $kinds = array_column($asset['sections'], 'kind');
            foreach ($requiredKinds as $kind) {
                $this->assertContains($kind, $kinds, "{$key}.{$kind}");
            }
            $text = json_encode([
                $asset['answer_first'],
                $asset['sections'],
                $asset['observation_exercise'],
                $asset['visible_evidence'],
            ], JSON_UNESCAPED_UNICODE) ?: '';
            foreach (['1w9', '1w2'] as $wing) {
                $this->assertStringContainsString($wing, $text, $key);
            }
    ...
    public function test_wing_pages_are_core_first_comparative_and_evidence_limited(): void
    {
        ...
        $requiredKinds = [
            'core_first_modifier',
            'sibling_core_comparison',
            'observable_situations',
            'counterexample',
    ...
    public function test_subtype_pages_use_matched_comparisons_and_reject_biological_certainty(): void
    {
        ...
        foreach ($subtypes as $asset) {
            ...
            $this->assertMatchesRegularExpression(
                $asset['locale'] === 'en'
                    ? '/(?:not a biological|not a measurable biological|not a biological drive)/i'
                    : '/(?:不是生物|不是已测得的生物)/u',
                $boundary['body'],
                $key,
            );
    ...
    public function test_positive_review_release_and_biological_mutations_fail_closed(): void
    {
        $package = $this->package();
    ...
    public function test_qa_report_records_codex_native_skeptical_review_and_zero_writes(): void
    {
        ...
        $this->assertSame(15, $report['raw_draft_audit']['asset_specific_issue_count']);
        $this->assertSame(15, array_sum($report['raw_draft_audit']['issues']));
