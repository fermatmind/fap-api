...
final class RiasecContentPromotionAdapter implements ExactPackagePromotionAdapter
{
    public function __construct(private readonly RiasecContentPromotionAuthority $authority, private readonly PromotionRollbackSnapshotService $snapshots) {}

    public function id(): string
    {
        return 'w4_riasec_content_release_v2';
    }
    ...
    {
        return $lane === 'W4' && $subscope === 'riasec';
    }
    ...
    public function publish(PromotionContext $context): array
    {
        ...
        $snapshot = $this->snapshots->resolve($context, $this->targets($package), 'riasec-content-release', 'before_publication', $reference);
        $previous = $this->previousReleaseId($snapshot, $context);
        $result = $this->authority->publish($context, $previous);
    ...
    {
        $package = $this->authority->inspect($context);
        $snapshot = $this->snapshots->resolve($context, $this->targets($package), 'riasec-content-release', 'before_publication', $rollbackReference);
    ...
    private function capture(PromotionContext $context, array $package, string $phase): string
    {
        ...
        return $this->snapshots->capture($context, $targets, 'riasec-content-release', $phase, $rows, $targets->identities(), ['previous_release_id' => $previous]);
    }
    ...
    private function previousReleaseId(ContentReleaseSnapshot $snapshot, PromotionContext $context): ?string
    {
        ...
        foreach ((array) data_get($snapshot->meta_json, 'rows', []) as $row) {
            if (! is_array($row) || (string) ($row['package_sha256'] ?? '') !== $context->packageSha256 || ($row['previous_release_id'] ?? null) !== $previous) {
                throw new DomainException('riasec_promotion_rollback_snapshot_invalid');
            }
        }
