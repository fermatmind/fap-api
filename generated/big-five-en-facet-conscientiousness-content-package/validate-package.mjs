...
const expectedCodes = ["competence", "order", "dutifulness", "achievement-striving", "self-discipline", "deliberation"];
const visibleWords = (body) => body.replace(/\[[^\]]+\]\([^\)]+\)/g, "").replace(/[#*`-]/g, " ").trim().split(/\s+/).filter(Boolean).length;

const [ledger, raw, review, repaired, seed] = await Promise.all([
  readJson("source_ledger.json"), readJson("raw_codex_draft.json"), readJson("skeptical_review.json"),
  readJson("repaired_draft.json"), readJson("big_five_en_facet_conscientiousness_seed.json"),
]);
...
assert(expectedCodes.every((code) => seed.assets.some((asset) => asset.entity_key === code)), "Conscientiousness Facet coverage mismatch");
...
for (const asset of seed.assets) {
  ...
  assert(asset.locale === "en", `${asset.entity_key}: wrong locale`);
  assert(asset.slug === `big-five/facets/${asset.entity_key}`, `${asset.entity_key}: slug drift`);
  assert(asset.canonical_path === `/en/personality/big-five/facets/${asset.entity_key}`, `${asset.entity_key}: canonical drift`);
  assert(asset.hreflang?.en === asset.canonical_path && asset.hreflang?.["zh-CN"] === `/zh/personality/big-five/facets/${asset.entity_key}`, `${asset.entity_key}: hreflang drift`);
  ...
  for (const section of asset.sections) {
    assert(typeof section.body_md === "string" && visibleWords(section.body_md) >= 42, `${asset.entity_key}.${section.key}: thin section`);
  ...
  assert(asset.internal_links.every((item) => item.href.startsWith("/en/personality/big-five/")), `${asset.entity_key}: IA drift`);
  const body = asset.sections.map((section) => section.body_md).join("\n");
  for (const phrase of ["your result shows", "absolutely accurate", "most accurate", "guaranteed hire", "guaranteed success", "official six personality types"]) {
    assert(!body.toLowerCase().includes(phrase), `${asset.entity_key}: forbidden claim ${phrase}`);
  }
...
assert(ledger.taxonomy.length === 1 && ledger.taxonomy[0].facets.length === 6, "Conscientiousness ledger taxonomy mismatch");
assert(Object.keys(ledger.facet_boundaries).length === 6, "facet boundaries incomplete");

const qaReport = {
  package: seed.package, outcome: "pass", generated_at: "2026-07-11T00:00:00Z",
  model_sessions: [
    { id: "codex-native-en-conscientiousness-raw-2026-07-11", mode: "codex_native_draft", external_model: false },
    { id: "codex-skeptical-review-en-conscientiousness-2026-07-11", mode: "codex_skeptical_self_review", external_model: false },
    { id: "codex-repair-en-conscientiousness-2026-07-11", mode: "codex_repair", external_model: false },
  ],
  coverage: { assets: 6, locale: "en", entity_type: "facet_detail", facet_codes: expectedCodes, sections_per_asset: 9, faq_per_asset: 5, internal_links_per_asset: 7 },
  checks: {
    v1_assets_envelope: "pass", body_md_only: "pass", substantive_sections: "pass",
    source_traceability: "pass_shared_ledger", independently_authored_english: "pass",
    facet_non_equivalence_boundaries: "pass", private_result_boundary: "pass",
    duplicate_template_risk: "pass_no_exact_section_duplicates", forbidden_claims: "pass",
    raw_draft_audited: "pass", repaired_draft_matches_seed: "pass", publish_indexability: "blocked_noindex_package_only",
  },
  indexability: { robots: "noindex,follow", launch_state: "content_ready", index_eligible: false, sitemap_eligible: false, llms_eligible: false },
  blockers: [],
  deferred: [
    "The remaining twenty-four English Facet pages retain their current thin content until their four domain PRs complete.",
    "CMS write/import, combined dry-run, runtime readback, indexability, sitemap, llms, JSON-LD release, search submission, and deployment remain outside this PR.",
  ],
};
await writeFile(resolve(outputDir, "qa_report.json"), `${JSON.stringify(qaReport, null, 2)}\n`);
console.log(JSON.stringify({ outcome: qaReport.outcome, assets: 6, sections_per_asset: 9, faq_per_asset: 5, internal_links_per_asset: 7, robots: "noindex,follow" }, null, 2));
