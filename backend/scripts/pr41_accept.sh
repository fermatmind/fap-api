...
SERVE_PORT="${SERVE_PORT}" ART_DIR="${ART_DIR}" bash "${BACKEND_DIR}/scripts/pr41_verify.sh"
...
PR41 Acceptance Summary
- result: PASS
- serve_port: ${SERVE_PORT}
- pass_items:
  - php artisan test --filter ContentLoaderServiceMtimeTest: PASS
  - php artisan test --filter PaymentWebhookStripeSignatureTest: PASS
  - php artisan test --filter PaymentWebhookLockBusyTest: PASS
  - php artisan test --filter PaymentWebhookIdempotencyTest: PASS
  - php artisan test --filter CommerceWebhookIdempotencyTest: PASS
...
bash "${BACKEND_DIR}/scripts/sanitize_artifacts.sh" 41

bash -n "${BACKEND_DIR}/scripts/pr41_accept.sh"
bash -n "${BACKEND_DIR}/scripts/pr41_verify.sh"

echo "[PR41][PASS] acceptance complete"
