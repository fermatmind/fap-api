...
namespace App\Services\Psychometrics\Eq60;
...
final class NormGroupResolver
{
    public function resolve(string $scaleCode, array $ctx = []): array
    {
        ...
        if (!Schema::hasTable('scale_norms_versions') || !Schema::hasTable('scale_norm_stats')) {
            return $this->missingPayload($locale, $region, $gender, $ageBand);
        }

        foreach ($this->buildCandidates($locale, $gender, $ageBand) as $groupId) {
    ...
    private function missingPayload(string $locale, string $region, string $gender, string $ageBand): array
    {
        return [
            'status' => 'MISSING',
    ...
    private function resolveAgeBand(array $ctx): string
    {
        ...
        $default = (string) config('eq60_norms.resolver.default_age_band', '18-60');
        ...
        $bands = (array) config('eq60_norms.resolver.age_bands', []);
    ...
    private function buildCandidates(string $locale, string $gender, string $ageBand): array
    {
        ...
        $templates = (array) config("eq60_norms.resolver.chains.{$chainKey}", []);
