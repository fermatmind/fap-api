...
- Career Current 正文权威仅为 `backend/content_assets/career/current/manifest.json` 绑定的 1046 个 canonical slug 目录与 2092 个 `career.detail.content.v3` 双语页面；契约、manifest schema 和字段 ownership 分别由 `career-content-v3-current.v1.json`、`career-content-v3-current-manifest.v1.schema.json` 和 `career-content-v3-current-field-ownership.v1.json` 约束。旧 10 模块 × 64 分片和 flat projection 均非 Current，DB/cache/API 的 presentation_v1/v2 仅是 manifest hash 绑定的兼容派生层。
- Career Current 的迭代基线仅保留 `accountants-and-auditors` 双语 enhanced 正文；其余 1045 个 canonical slug 必须保留双语文件与 URL 身份，但在被明确选中迭代前保持 `legacy`、`blocks: []` 的空正文占位，运行时不得回退展示旧 DB/cache 正文。
- Career Current 的 2092 页离线 package/codec/容量扫描只在现有 CI Career job 执行；staging 保持其已发布 30×2 cohort 的环境 smoke，不要求镜像 production 1046 条兼容行；真实 1046×2 DB/cache/API/publisher parity 仅在 Career 分类命中的 production 候选 release 中只读执行，且必须先于 migration、cache warm、symlink 和 publisher，失败时不得激活候选或运行 publisher。
- Personality Current 正文权威仅为 `backend/content_assets/personality_public/current/manifest.json` 绑定的 182 个双语公开身份与 364 个 `personality.page.content.v1` 页面文件。激活前它们是只读等价基线；按页面族激活后，DB/cache/旧 batch JSON 与 MBTI comparison overlay 只能提供兼容传输，不能覆盖 Current 正文。历史别名继续仅作 redirect，不得进入 manifest、canonical、hreflang、sitemap 或 llms。
...
- The sole editable private Enneagram result authority is `backend/content_packs/ENNEAGRAM/v2/registry`; its `en/` directory is the English locale of the same package. Do not create a later registry version or maintain E105/FC144 editorial copy separately. Runtime releases remain immutable and hash-addressed.
