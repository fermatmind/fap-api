...
final class ReportArtifactsArchiveService
{
    ...
    private function writeSidecars(array $plan, array $payload): void
    {
        ...
        foreach ($this->resultItems($payload) as $item) {
            ...
            if (! in_array($status, ['copied', 'already_archived'], true)) {
                continue;
            }
        ...
        try {
            $this->accessProjections->refreshAttemptProjection(
                ...
                [
                    ...
                    'source_ref' => trim((string) ($payload['run_path'] ?? data_get($payload, '_meta.plan_path', 'archive_report_artifacts'))),
                    'actor_type' => 'system',
                    'actor_id' => 'archive_report_artifacts',
                ]
            );
        } catch (\Throwable $e) {
            Log::warning('ARCHIVE_ACCESS_PROJECTION_WRITE_FAILED', [
