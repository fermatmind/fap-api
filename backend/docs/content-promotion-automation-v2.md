...
- `W2 / big-five` and `W5 / enneagram`: audit-compatible through the shared `PersonalityPublicContentAsset`, immutable `PersonalityPublicContentAssetRevision`, separately declared `review-evidence.json`, published/working pointer, public read-model cache invalidation, and `ContentReleaseSnapshot` authority. Each package has its own framework, target set, SHA, receipt chain, and rollback lineage. Draft import binds only exact approved operator-supplied review evidence; publication requires that bound evidence and an unchanged pre-publication fingerprint. The adapter accepts complete, type-safe English text-only snapshots only; it rejects Markdown and HTML media forms, never writes hero, inline, OG, or Twitter fields, and preserves all discoverability fields.
- `W3 / articles`, `W3 / career-guides`, `W4 / riasec`, `W6 / iq`, `W7 / eq`, `W8 / career-jobs`: registered but fail closed until their existing authority-specific importer/publisher exposes compatible revision/audit metadata and rollback.
