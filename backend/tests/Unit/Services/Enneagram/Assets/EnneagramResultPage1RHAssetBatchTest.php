...
final class EnneagramResultPage1RHAssetBatchTest extends TestCase
{
    public function test_1r_h_batch_assets_are_runner_safe_and_not_runtime(): void
    {
        $root = base_path('content_assets/enneagram/result_page/batch_1r_h/v0_1');
        $manifest = $this->readJson($root.'/asset_batch_manifest.json');
        $assetsDoc = $this->readJson($root.'/assets.json');

        $this->assertSame('fap.enneagram.result_page.batch_1r_h.manifest.v0.1', $manifest['schema_version'] ?? null);
        ...
        $this->assertSame('74b5db9775792f156eeeb9cc203c888c3c627c8498cfb5c7a16ff521208c9140', $manifest['source_asset_sha256'] ?? null);
        ...
        $this->assertCount(10, (array) data_get($manifest, 'coverage_counts.by_fc144_recommendation_context'));
        $this->assertCount(10, (array) data_get($manifest, 'coverage_counts.by_recommendation_strategy'));
        ...
        $sourceLedgerRow = $this->sourceLedgerRow('batch_1r_h_asset_stream');
        $assetKeys = [];
        ...
        foreach ((array) ($assetsDoc['assets'] ?? []) as $asset) {
            ...
            $this->assertSame('1R-H', $asset['batch_scope'] ?? null);
            $this->assertSame('fc144_recommendation_response', $asset['category'] ?? null);
            $this->assertNotSame('', (string) data_get($asset, 'routing_metadata.branch_metadata.fc144_recommendation_context'));
            $this->assertNotSame('', (string) data_get($asset, 'routing_metadata.branch_metadata.recommendation_strategy'));
            $this->assertSame('batch_1r_h_asset_stream', data_get($asset, 'source_trace.primary_source_id'));
            $this->assertContains('fc144_boundary_policy', (array) data_get($asset, 'source_trace.source_ids', []));
