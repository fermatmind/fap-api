...
- Preflight is read-only and must prove zero pending `ops` jobs and no deploy lock/process. Existing configuration discovery is by the exact `[program:fap-queue-ops]` identity inside bounded Supervisor roots, not by filename; at most one small `.conf`/`.ini` file may match. Preflight may attest a shared/multi-section legacy file using only safe layout counts and hashes, but `apply_supported` must remain false for that layout. Apply may replace/install only an absent or exact single-program dedicated config and restart/start only that worker; duplicate identities and unknown paths fail closed. A shared layout may be changed only through the separately versioned `Backend Production Ops Shared Config Migration` workflow after its immutable successful read-only preflight and exact workflow-defined authorization phrase. That migration must replace only the exact `[program:fap-queue-ops]` section, bind the full config/current ops/foreign projection/foreign status/patched hashes and section counts, prove zero queued jobs, preserve foreign sections and program statuses, restart only `fap-queue-ops`, and perform exact rollback on failed readback. Neither control may deploy code, move the active symlink, run application migrations, or change CMS/database authority, publication, sitemap, llms, search, or PR23 state.
