...
final class CareerMaterializeCurrentEnBatch extends Command
{
    protected $signature = 'career:materialize-current-en-batch
    ...
    protected $description = 'Materialize one ordered English Career Current authority batch without runtime writes';

    public function handle(CareerCurrentEnBatchMaterializer $materializer): int
    {
        ...
        try {
            ...
            if ($dryRun === $write) {
                throw new CareerTenBlockCompileFailure('CURRENT_EN_MATERIALIZE_MODE_INVALID');
            }
            ...
            $this->line((string) json_encode([
                'status' => 'FAIL_CURRENT_EN_BATCH_MATERIALIZE',
                'safe_error_code' => $throwable instanceof CareerTenBlockCompileFailure
                    ? $throwable->safeCode : 'CURRENT_EN_MATERIALIZE_UNEXPECTED_FAILURE',
