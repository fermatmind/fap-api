...
const DIR = "generated/big-five-authority-v2/big5-authority-v2-range-extraversion-12";
const LEDGER = "generated/big-five-authority-v2/big5-authority-v2-source-ledger-05/source-ledger.json";
const EXPECTED_PAGES = [
  { locale: "en", path: "/en/personality/big-five/extraversion-high", level: "high", family: "v2_range", key: "range:extraversion:high", legacyIntent: null },
  { locale: "zh-CN", path: "/zh/personality/big-five/extraversion-high", level: "high", family: "v2_range", key: "range:extraversion:high", legacyIntent: null },
  { locale: "en", path: "/en/personality/big-five/extraversion-mid", level: "middle", family: "v2_range", key: "range:extraversion:middle", legacyIntent: null },
  { locale: "zh-CN", path: "/zh/personality/big-five/extraversion-mid", level: "middle", family: "v2_range", key: "range:extraversion:middle", legacyIntent: null },
  { locale: "en", path: "/en/personality/big-five/extraversion-low", level: "low", family: "v2_range", key: "range:extraversion:low", legacyIntent: null },
  { locale: "zh-CN", path: "/zh/personality/big-five/extraversion-low", level: "low", family: "v2_range", key: "range:extraversion:low", legacyIntent: null },
  { locale: "en", path: "/en/personality/big-five/high-extraversion", level: "high", family: "legacy_canonical", key: "legacy:high-extraversion", legacyIntent: "social_energy_and_airtime_budget" },
  { locale: "en", path: "/en/personality/big-five/low-extraversion", level: "low", family: "legacy_canonical", key: "legacy:low-extraversion", legacyIntent: "quiet_participation_and_recovery_plan" },
];
...
const validateRaw = (raw) => {
  ...
  if (raw.schema_version !== "big5_authority_v2_range_package.v1" || raw.train_id !== "BIG5-AUTHORITY-V2-RANGE-EXTRAVERSION-12" || raw.stage !== "raw" || raw.framework !== "big_five" || raw.domain_code !== "extraversion") add("schema", "raw_identity_invalid", "raw", "Raw package identity is invalid.");
  ...
  if (JSON.stringify(sorted(pages.filter(isObject).map((page) => page.canonical_path))) !== JSON.stringify(sorted(expectedPaths()))) add("schema", "raw_paths_invalid", "raw.pages", "Raw paths must equal PR12 coverage.");
...
const validateRepaired = (repaired, sourceLedger) => {
  const issues = [];
  const add = (gate, code, path, message) => issues.push({ gate, code, path, message });
  if (repaired.schema_version !== "big5_authority_v2_range_repair.v1" || repaired.train_id !== "BIG5-AUTHORITY-V2-RANGE-EXTRAVERSION-12" || repaired.stage !== "repaired" || repaired.framework !== "big_five" || repaired.domain_code !== "extraversion") add("schema", "repaired_identity_invalid", "repaired", "Repaired package identity is invalid.");
  if (JSON.stringify(sorted(repaired.required_intents ?? [])) !== JSON.stringify(sorted(REQUIRED_INTENTS))) add("schema", "repaired_intents_invalid", "repaired.required_intents", "Repaired intents must equal PR12 scope.");
  ...
  if (JSON.stringify(sorted(pages.filter(isObject).map((page) => page.canonical_path))) !== JSON.stringify(sorted(expectedPaths()))) add("coverage", "repaired_paths_invalid", "repaired.pages", "Repaired paths must equal PR12 coverage.");
  for (const [index, page] of pages.entries()) {
    const expected = expectedForPath(page?.canonical_path);
    ...
    if (page?.legacy_intent !== expected?.legacyIntent) add("duplicate", "repaired_legacy_intent_invalid", `repaired.pages.${index}.legacy_intent`, "Legacy intent must match frozen PR12 identity.");
...
const validateFinal = (candidate, sourceLedger) => {
  const issues = [];
  const add = (gate, code, path, message) => issues.push({ gate, code, path, message });
  if (candidate.schema_version !== "big5_authority_v2_range_package.v1" || candidate.train_id !== "BIG5-AUTHORITY-V2-RANGE-EXTRAVERSION-12" || candidate.stage !== "final" || candidate.framework !== "big_five" || candidate.domain_code !== "extraversion") add("schema", "final_identity_invalid", "final", "Final package identity is invalid.");
  const pages = Array.isArray(candidate.pages) ? candidate.pages : [];
  if (candidate.expected_page_count !== 8 || pages.length !== 8) add("coverage", "page_count_mismatch", "pages", "PR12 must contain exactly eight final pages.");
  if (JSON.stringify(sorted(pages.filter(isObject).map((page) => page.canonical_path))) !== JSON.stringify(sorted(expectedPaths()))) add("coverage", "canonical_coverage_mismatch", "pages", "Final canonical coverage must equal PR12's eight paths.");
  ...
  pages.forEach((page, pageIndex) => {
    ...
    if (!expected || page.content_key !== expected.key || page.locale !== expected.locale || page.route_family !== expected.family || page.range_level !== expected.level || page.legacy_intent !== expected.legacyIntent) add("coverage", "range_identity_mismatch", path, "Page identity must match the frozen PR12 route matrix.");
    if (page.page_family !== "range" || page.framework !== "big_five" || page.domain_code !== "extraversion") add("framework", "range_framework_invalid", path, "Page must remain an extraversion range page.");
    ...
    if (CROSS_FRAMEWORK_PATTERN.test(serialized)) add("framework", "cross_framework_leakage", path, "A different framework appears outside PR12 authority.");
  ...
  for (const level of ["high", "middle", "low"]) {
    const en = pageByPath.get(`/en/personality/big-five/extraversion-${level === "middle" ? "mid" : level}`);
    const zh = pageByPath.get(`/zh/personality/big-five/extraversion-${level === "middle" ? "mid" : level}`);
  ...
  const v2High = pageByPath.get("/en/personality/big-five/extraversion-high");
  const legacyHigh = pageByPath.get("/en/personality/big-five/high-extraversion");
  const v2Low = pageByPath.get("/en/personality/big-five/extraversion-low");
  const legacyLow = pageByPath.get("/en/personality/big-five/low-extraversion");
...
const validateWorkflow = (raw, skeptical, repaired, final, ledger) => {
  ...
  if (skeptical.schema_version !== "big5_authority_v2_skeptical_review.v1" || skeptical.train_id !== "BIG5-AUTHORITY-V2-RANGE-EXTRAVERSION-12" || skeptical.reviewed_artifact !== raw.artifact || skeptical.review_status !== "repair_required") add("skeptical_review_invalid", "Skeptical review identity is invalid.");
  ...
  return {
    train_id: "BIG5-AUTHORITY-V2-RANGE-EXTRAVERSION-12",
