...
The operator command is `seo:council-runtime status|pause|resume`. An operator pause is recorded as `pause_source=manual`; deploy acceptance protection is recorded separately with a run-bound operation reference and generation. Old paused boolean state is `historical_unknown` and is never automatically cleared. Acceptance restores prior intent only when the generation is unchanged, so a newer operator action always wins. Pause affects Council only and retains all evidence. Shared cache failure fails closed. Pause takes effect at the next computation/commit checkpoint, bounded by the command deadline. It also stops Council notification sending. Resume rechecks the same prerequisites and never replays terminal deliveries.
...
The shipped configuration remains disabled until the deployment-owned activation evidence is installed. Development/offline tests are not production activation evidence. Production uses `seo.platform12_a08_activation.v2` and the explicit `A08_SCOPED_READ_ONLY_ACCEPTANCE` basis: exact-SHA CI, applicable focused checks, staging acceptance, production activation/smoke, production controlled acceptance, dependency fingerprint/version vector, and closed business-write guards. Nightly remains independent evidence and is never rewritten, but a full Nightly success is not an unconditional A08 activation prerequisite.

The release workflow's existing CI, staging, production and smoke receipts remain deployment authority. Runtime moves through `DISABLED -> CONTROLLED_ACCEPTANCE_ONLY -> ACTIVE_READ_ONLY`. During controlled acceptance, ordinary minute ticks cannot reserve natural slots; only the deploy-run-bound acceptance operation can submit the three allowlisted requests. Staging verifies pause/resume and sends one clearly labelled real staging notification plus an idempotent duplicate. All three production natural slots are still required before declaring A08 complete. No manual or alternate deployment route is authorized by this document.

Controlled acceptance calls use the existing scheduled command's `--acceptance` option with one allowlisted Catalog ID and the exact deploy operation reference. They are recorded separately and never count as natural slots. A HOLD caused by a genuinely connected failing source is a valid diagnosis; a missing necessary source is not completed wiring acceptance.
