...
- The Big Five zh-CN V3 authority release is restricted to the deterministic 52-page package compiled from source content SHA `056b10d3f640d0cf7da35ec7bc99b009408049e75c1e25aa8e760eb8641ea8d5` and package file SHA `83536987f7edc73d668f481942c94f6bf549abf23a0e498941f47bc56726490d`. `personality:big-five-zh-v3-content-publish` is read-only by default and may write only after both exact hashes and `admin_user:1` are confirmed; it must update the 52 existing zh-CN `PersonalityPublicContentAsset` rows in one transaction, create immutable package-bound revisions, perform transaction-wide readback, remain idempotent, preserve CMS hreflang, and leave English, media, search submission, and all non-target authority rows unchanged. The exact 10 reviewed zh-CN redirect aliases remain `content_ready`, public, `noindex,follow`, and ineligible for sitemap/llms; they must stay outside the 52 descriptors and retain identical primary rows, revisions, pointers, and fingerprints before and after release. Production execution remains a separately controlled action after code deployment and read-only readiness verification.
- The ten historical English Big Five `high-*`, `low-*`, and `emotional-stability` range identities are redirect-only aliases, never canonical content assets. Backend retirement must archive and remove public/index/sitemap/LLMS/hreflang eligibility from exactly those ten rows while preserving their immutable revisions and all 52 English canonical rows; fap-web may only consume the backend-locked map as deterministic one-hop 301 redirects. Unknown aliases, target drift, canonical-row writes, or revision writes fail closed. Production retirement remains a separately authorized database action.
