...
Approve and execute are separate transitions. Approving records evidence and never dispatches the operation. A later explicit execute action binds the freshly stepped-up execution actor, authorization timestamp, execution-attempt number, and exact approval evidence SHA into a separate hashed private authorization record before queueing the existing worker. The worker validates that authorization, performs the domain action as that execution actor, and attributes success/failure audit records to the same actor. It also recomputes and verifies the exact target fingerprint and approval evidence SHA before changing the approval to `EXECUTING`. Any payload, reason, correlation, approval actor, execution actor, mode, schema, timestamp, attempt, or evidence drift fails closed before action dispatch. Duplicate approval is idempotent only when the existing immutable evidence still matches.
