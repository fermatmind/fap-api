...
final class CareerShardedCurrentContractTest extends TestCase
{
    ...
    public function test_installed_current_inventory_is_manifest_bound_and_versionless_projection_is_deterministic(): void
    {
        ...
        self::assertSame(
            'b52b877a7aa0e05f8fc0d28c53eca5330e3a497f5ff4731f329e93c8c32fe8b0',
        ...
        $accountants = $loaded['rows']['accountants-and-auditors'];
        $pages = array_keys($accountants['page_payload_json']) === ['page']
            ? $accountants['page_payload_json']['page']
            : $accountants['page_payload_json'];
        self::assertSame(['salary'], array_keys($pages['zh']['career_snapshot_primary_locale']));
        self::assertSame('会计与审计适合什么样的人？', $pages['zh']['fermat_decision_card']['title']);
        self::assertStringContainsString('30–45 分钟', $pages['zh']['fit_decision_checklist']['how']);
        self::assertStringContainsString('两套相连但独立的工作系统', json_encode(
            $pages['zh']['career_quick_answers_block'],
            JSON_THROW_ON_ERROR | JSON_UNESCAPED_UNICODE,
        ));
        self::assertSame(
            '会计师和审计师围绕交易、账簿、财务报表和审计证据开展工作，记录、分析和解释财务信息，并判断其是否准确、完整、合规且可核验。',
            $pages['zh']['definition_block'],
        );
        self::assertNull($pages['zh']['career_quick_answers_block']['items'][0]['table']['rows'][0]['alternate_value']);
        self::assertNull($pages['zh']['career_quick_answers_block']['items'][0]['table']['rows'][1]['alternate_value']);
        self::assertSame(6, count($pages['zh']['responsibilities_block']));
        self::assertStringContainsString(
            '重大错报风险识别、评估与审计应对',
            $pages['zh']['responsibilities_block'][4],
        );
        self::assertSame(
            '关键职业判断',
            $pages['zh']['career_quick_answers_block']['items'][2]['question'],
        );
        self::assertSame(
            '会计和审计有什么区别？职责、产出与保证边界',
            $pages['zh']['career_quick_answers_block']['items'][1]['question'],
        );
        self::assertStringContainsString(
            '审计不代替管理层编制财务报表和维护必要内部控制的责任',
            $pages['zh']['career_quick_answers_block']['items'][1]['answer'],
        );
        self::assertCount(
            7,
            $pages['zh']['career_quick_answers_block']['items'][1]['table']['rows'],
        );
        self::assertCount(
            6,
            $pages['zh']['career_quick_answers_block']['items'][2]['table']['rows'],
        );
        self::assertStringContainsString(
            '充分性关注证据数量，适当性关注相关性和可靠性',
            $pages['zh']['career_quick_answers_block']['items'][2]['table']['rows'][4]['secondary_value'],
        );
        self::assertStringContainsString('提供高水平的合理保证，而非绝对保证', json_encode(
            $pages['zh']['career_quick_answers_block'],
            JSON_THROW_ON_ERROR | JSON_UNESCAPED_UNICODE,
        ));
        self::assertStringContainsString(
            '板块标题｜会计师和审计师的工作环境与工作方式',
            $pages['zh']['work_context_block'],
        );
        self::assertStringContainsString(
        ...
        $definitionRows = file(
            $currentRoot.'/definition/shard-32.jsonl',
            FILE_IGNORE_NEW_LINES,
        ) ?: [];
        $definitionAuthority = null;
        foreach ($definitionRows as $line) {
            $record = json_decode($line, true, 512, JSON_THROW_ON_ERROR);
            if (($record['canonical_slug'] ?? null) === 'accountants-and-auditors'
                && ($record['locale'] ?? null) === 'zh-CN') {
                $definitionAuthority = $record;
                break;
            }
        }
        self::assertIsArray($definitionAuthority);
        self::assertSame([
            'https://xj.mof.gov.cn/caizhengjiancha/200805/t20080524_40447.htm',
            'https://www.cicpa.org.cn/ztzl1/Professional_standards/201904/W020210421547129334232.pdf',
            'https://www.onetonline.org/link/details/13-2011.00',
            'https://www.bls.gov/ooh/business-and-financial/accountants-and-auditors.htm',
            'https://www.bls.gov/ors/factsheet/accountants-and-auditors.htm',
        ], array_column($definitionAuthority['source_bindings'], 'url'));
        self::assertSame(
            '专业依据与使用边界',
            $pages['zh']['onet_structured_fields_block']['heading'],
        );
        self::assertNotContains(
            '使用边界',
            array_column($pages['zh']['onet_structured_fields_block']['rows'], 'label'),
        );
        self::assertNull($accountants['metadata_json']['presentation_v1']['zh']['notices']['snapshot_callout']);
        foreach ($loaded['rows'] as $row) {
