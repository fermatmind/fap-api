...
final class BigFiveCmsImportDraftDryRunPlanner
{
    ...
    private function planV1Assets(array $assets, string $sourceSha256): array
    {
        ...
        foreach ($assets as $index => $asset) {
            if (! is_array($asset)) {
                $errors[] = $this->issue('assets.'.$index, 'asset_not_object', 'Each V1 asset must be an object.');
            ...
            foreach (['framework', 'entity_type', 'entity_key', 'locale', 'canonical_path', 'sections', 'faq', 'internal_links', 'robots'] as $field) {
                if (! array_key_exists($field, $asset)) {
                    $errors[] = $this->issue('assets.'.$index.'.'.$field, 'required_field_missing', 'Required V1 asset field is missing.');
                }
            }
            if (($asset['framework'] ?? '') !== PersonalityPublicContentAsset::FRAMEWORK_BIG_FIVE) {
                $errors[] = $this->issue('assets.'.$index.'.framework', 'framework_invalid', 'V1 assets must use big_five.');
            }
            $key = implode(':', [(string) ($asset['locale'] ?? ''), (string) ($asset['entity_type'] ?? ''), (string) ($asset['entity_key'] ?? '')]);
            if (isset($identity[$key])) {
                $errors[] = $this->issue('assets.'.$index, 'identity_duplicate', 'V1 asset identity must be unique.');
            }
            $identity[$key] = true;
            $isLegacyAlias = ($asset['locale'] ?? '') === 'zh-CN' && in_array((string) ($asset['entity_key'] ?? ''), $legacyAliases, true);
            if (! $isLegacyAlias) {
                foreach ((array) ($asset['sections'] ?? []) as $sectionIndex => $section) {
                    if (! is_array($section) || trim((string) ($section['body_md'] ?? '')) === '') {
                        $errors[] = $this->issue('assets.'.$index.'.sections.'.$sectionIndex.'.body_md', 'body_md_missing', 'Canonical V1 sections must use non-empty body_md.');
                    }
                }
            }
            if (str_contains((string) json_encode($asset), 'bodyMd')) {
                $errors[] = $this->issue('assets.'.$index, 'bodyMd_forbidden', 'V1 assets must use body_md, never bodyMd.');
            }
            ...
            if (($asset['index_eligible'] ?? false) && $robots !== 'index,follow') {
                $errors[] = $this->issue('assets.'.$index.'.robots', 'indexability_gate_invalid', 'Indexable assets require index,follow.');
            }
            $locale = (string) ($asset['locale'] ?? '');
            $localeCounts[$locale] = ($localeCounts[$locale] ?? 0) + 1;
            $type = (string) ($asset['entity_type'] ?? '');
            $typeCounts[$type] = ($typeCounts[$type] ?? 0) + 1;
        }
        if (count($assets) !== 124) {
            $errors[] = $this->issue('assets', 'row_count_invalid', 'V1 Big Five publish package must contain exactly 124 assets.');
        }
        if ($canonical !== 114 || $redirectOnly !== 10) {
            $errors[] = $this->issue('assets', 'topology_invalid', 'Expected 114 canonical/indexable assets and 10 noindex redirect-only aliases.');
        }
