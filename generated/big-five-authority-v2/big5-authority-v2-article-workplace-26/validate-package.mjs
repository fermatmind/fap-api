...
const locked = matrix.themes.filter((theme) => theme.batch === 26);
...
assert(finalPackage.assets.every((asset) => asset.batch === 26));
...
for (const asset of finalPackage.assets) {
  ...
  const boundaryTerms = asset.locale === 'en'
    ? ['hiring', 'screening', 'promotion', 'performance', 'suitability', 'unknown']
    : ['招聘', '筛选', '晋升', '绩效', '适配', 'unknown'];
  for (const term of boundaryTerms) assert(boundary.includes(term));
...
for (const forbidden of ['/attempt/', '/report/', '/orders/', 'clinically validated', 'guaranteed accurate', 'best fit score']) assert(!serialized.includes(forbidden));
console.log('Big Five batch 26 validation passed: exact 5 workplace themes / 10 bilingual Article candidates / hard decision-use boundary');
