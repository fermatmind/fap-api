...
final class SeoIntelCrawlerLogObserveCommand extends Command
{
    protected $signature = 'seo-intel:crawler-log-observe
        ...
        {--limit=100 : Maximum synthetic fixture lines to inspect}';

    protected $description = 'Aggregate synthetic crawler-log fixture lines without production log reads or writes.';

    public function handle(CrawlerLogAggregateDryRun $aggregateDryRun): int
    {
        $limit = max(1, min((int) $this->option('limit'), CrawlerLogAggregateDryRun::MAX_LIMIT));
        ...
        if (! (bool) $this->option('fixture')) {
            $this->emit([
                ...
                'issues' => [
                    'fixture_option_required',
                    'production_log_read_not_available_in_v1',
                ],
