...
cd "${BACKEND_DIR}"
set +e
php artisan test --filter EventPayloadLimiterTest --filter EventPayloadLimitsTest > "${ART_DIR}/phpunit.txt" 2>&1
phpunit_rc=$?
set -e
if [[ "${phpunit_rc}" -ne 0 ]]; then
  php artisan test --filter "EventPayloadLimiterTest|EventPayloadLimitsTest" > "${ART_DIR}/phpunit.txt"
fi
cd "${REPO_DIR}"
