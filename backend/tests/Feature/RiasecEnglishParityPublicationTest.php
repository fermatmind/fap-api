...
class RiasecEnglishParityPublicationTest extends TestCase
{
    private const FIXTURE_CONNECTION = 'riasec_english_parity_fixture';
    ...
    protected function setUp(): void
    {
        ...
        $connection['prefix'] = 'riasec_english_parity_'.bin2hex(random_bytes(6)).'_';
    ...
    {
        return json_decode(file_get_contents(database_path('data/assessment_riasec_parity_en_20260921.json')), true, 512, JSON_THROW_ON_ERROR);
    }

    private function baseline(): array
    {
        return json_decode(file_get_contents(database_path('data/assessment_landing_en_20260907.json')), true, 512, JSON_THROW_ON_ERROR)['scales']['RIASEC']['content'];
    }
    ...
    {
        (require database_path('migrations/2026_09_21_010000_align_riasec_english_landing_parity.php'))->up();
    }
    ...
    private function applyPackage(array $content): array
    {
        foreach ($this->package()['updates'] as $update) {
            if (isset($update['path'])) {
                data_set($content, $update['path'], $update['value']);
    ...
    public function test_updates_only_two_exact_fields_in_both_registries_and_is_idempotent(): void
    {
        $package = $this->package();
        $this->assertSame('assessment.exact-field-copy.v2', $package['schema_version']);
        $this->assertSame('RIASEC', $package['scale_code']);
        $this->assertSame('en', $package['locale']);
        $this->assertCount(2, $package['updates']);
        ...
        foreach (['scales_registry', 'scales_registry_v2'] as $table) {
            ...
            $modelFaq = collect($content['en']['faq'])->firstWhere('id', 'faq-riasec-model');
            $this->assertSame($before['faq'][0]['q'], $modelFaq['q']);
            $this->assertSame($before['faq'][0]['references'], $modelFaq['references']);
        }
    }
    ...
    public function test_rejects_cross_registry_mixed_state_without_writing(): void
    {
        ...
        $this->expectExceptionMessage('mixed state');
        $this->publish();
    }

    public function test_rejects_partial_unknown_or_missing_targets_without_writing(): void
    {
        $before = $this->baseline();
        $partial = $before;
        data_set($partial, 'why_choose.intro', $this->package()['updates'][0]['value']);
        foreach (['scales_registry', 'scales_registry_v2'] as $table) {
        ...
        foreach (['scales_registry', 'scales_registry_v2'] as $table) {
            ...
            $this->assertSame($partial, array_diff_key($actual['en'], ['unrelated' => true]));
        }
    }

    public function test_transaction_rolls_back_both_registries_when_second_update_fails(): void
    {
        $before = $this->baseline();
        foreach (['scales_registry', 'scales_registry_v2'] as $table) {
            $this->insert($table, $before);
        ...
        DB::unprepared(sprintf(
            "CREATE TRIGGER %sriasec_english_update_failure BEFORE UPDATE ON %sscales_registry_v2 BEGIN SELECT RAISE(ABORT, 'forced update failure'); END",
            $prefix,
            $prefix,
        ));
    ...
    public function test_rejects_missing_faq_target_without_writing(): void
    {
        $before = $this->baseline();
        $missing = $before;
        $missing['faq'] = array_values(array_filter(
            $missing['faq'],
            static fn (array $item): bool => $item['id'] !== 'faq-riasec-model',
        ));
        foreach (['scales_registry', 'scales_registry_v2'] as $table) {
            $this->insert($table, $missing);
        }

        try {
            $this->publish();
            $this->fail('Expected missing FAQ target rejection.');
        } catch (\RuntimeException $exception) {
            $this->assertStringContainsString('faq.faq-riasec-model is missing', $exception->getMessage());
        }

        foreach (['scales_registry', 'scales_registry_v2'] as $table) {
            $actual = json_decode(DB::table($table)->value('content_i18n_json'), true, 512, JSON_THROW_ON_ERROR);
            $this->assertSame($missing['faq'], $actual['en']['faq']);
        }
    }

    public function test_rejects_incomplete_registry_tables_or_rows_and_seeder_uses_package(): void
    {
        ...
        Schema::drop('scales_registry_v2');
        try {
            $this->publish();
            $this->fail('Expected incomplete registry table rejection.');
        } catch (\RuntimeException $exception) {
            $this->assertStringContainsString('table set is incomplete', $exception->getMessage());
        }

        $source = file_get_contents(database_path('seeders/ScaleRegistrySeeder.php'));
        $this->assertStringContainsString('assessment_riasec_parity_en_20260921.json', $source);
    }
}
