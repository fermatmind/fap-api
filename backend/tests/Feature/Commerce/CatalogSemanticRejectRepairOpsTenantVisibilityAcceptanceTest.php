...
final class CatalogSemanticRejectRepairOpsTenantVisibilityAcceptanceTest extends TestCase
{
    use RefreshDatabase;
    use SignedBillingWebhook;

    public function test_sku_not_found_repair_converges_consistently_in_lookup_ops_and_tenant(): void
    {
        ...
        DB::table('orders')
            ...
            ->update([
                'sku' => $missingSku,
                'item_sku' => $missingSku,
                'effective_sku' => $missingSku,
                'requested_sku' => $missingSku,
                'updated_at' => now()->subMinutes(9),
            ]);

        $providerEventId = 'evt_catalog_missing_'.$orderNo;
        ...
        $webhook->assertJsonPath('reject_reason', 'SKU_NOT_FOUND');
        ...
        $this->upsertSku($missingSku, 'MBTI', 'report_unlock', 'MBTI_REPORT_FULL', 199, 'CNY');
    ...
    {
        $email = 'catalog-reject-tenant@example.com';
        $userId = (int) DB::table('users')->insertGetId([
            'name' => 'Catalog Reject Tenant User',
            'email' => $email,
        ...
        $orgId = (int) DB::table('organizations')->insertGetId([
            'name' => 'Catalog Reject Tenant Org',
        ...
        $anonId = 'anon_catalog_reject_'.$userId;
