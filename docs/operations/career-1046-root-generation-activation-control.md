...
Apply additionally requires a fresh successful activation preflight receipt, its exact run/attempt and SHA-256, the same exact release SHA and SHA-256 of the active release name, the preflight database-state SHA-256, and the generated approval phrase binding every identity. Any release, database, pointer, generation, document, receipt, process, or rollback drift fails closed.
