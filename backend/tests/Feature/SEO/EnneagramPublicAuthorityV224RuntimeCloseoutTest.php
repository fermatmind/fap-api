...
final class EnneagramPublicAuthorityV224RuntimeCloseoutTest extends TestCase
{
    ...
    public function test_readback_rejects_authority_content_present_only_in_hydration_script(): void
    {
        ...
        $this->fakeRuntimeHttp($report, authorityContentOnlyInHydrationScript: true);
        ...
        try {
            ...
            $this->fail('Expected script-only authority content to fail visible readback.');
        } catch (\RuntimeException $exception) {
            $this->assertStringContainsString('html_visible_section_mismatch', $exception->getMessage());
            $this->assertStringContainsString('html_visible_faq_mismatch', $exception->getMessage());
    ...
    public function test_post_readback_rejects_missing_visible_evidence_limitations_against_current_authority(): void
    {
        $this->seedPublishedEstate();
        $report = $this->releaseReport();
        foreach (PersonalityPublicContentAsset::query()->get() as $asset) {
            $asset->forceFill(['authority_json' => [
                ...
                'limitations' => [
                    ...
                    'It does not predict an individual outcome.',
                ],
                'visible_evidence_eligible' => true,
            ]])->save();
        }
        $this->fakeRuntimeHttp($report, omitVisibleEvidenceLimitations: true);
        ...
        try {
            ...
            $this->fail('Expected missing post-readback visible-evidence limitations to fail closed.');
        } catch (\RuntimeException $exception) {
            $this->assertStringContainsString('api_visible_evidence_authority_mismatch', $exception->getMessage());
            $this->assertStringNotContainsString('api_visible_evidence_missing', $exception->getMessage());
            $this->assertStringNotContainsString('html_visible_evidence_mismatch', $exception->getMessage());
    ...
    private function fakeRuntimeHttp(
        ...
        Http::fake(function (Request $request) use ($apiCanonicalUrlOverride, $apiHreflangUrlOverride, $authorityContentHiddenByStylesheet, $authorityContentHiddenStyle, $authorityContentOnlyInHydrationScript, $baseUrlText, $canonicalUrlOverride, $discoverabilityState, $duplicateCanonicalUrl, $emitFaqSchema, $faqSchemaAnswerOverride, $hreflangUrlOverride, $omitFaqAnswerHtml, $omitFaqSchemaAnswer, $omitSectionHtml, $omitVisibleEvidence, $omitVisibleEvidenceLimitations, $partialVisibleEvidence, $privateReviewerLeak, $privateRouteLeak, $redirectSurface, $rejectRevalidation, $robotsHtmlOverride, $splitPrivateReviewerHtml, $staleHreflangPayload, $stalePublicPayload, $standardMediaLeak): \GuzzleHttp\Promise\PromiseInterface|\Illuminate\Http\Client\Response {
            ...
            if (str_starts_with($url, 'https://frontend.test/')) {
                ...
                $hydrationScript = '';
                if ($authorityContentOnlyInHydrationScript) {
                    $hydrationScript = '<script type="application/json">'
