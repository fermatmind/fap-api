...
- `module_order`：原会计师 UI 的十二个展示分组顺序（概览及十一章）；位置 ID 的首段定位组件，`sections` 定位公开章节，`entry_decisions` 定位入行组件，`hero` 定位指标。
...
- `interface.*`：原 UI 中尚未进入职业文件的标题、表头、提示文案及组合薪资格，逐项预留为空。它们不复制会计师文案，也不替换当前 UI；正式填写和展示切换属于后续工作。
...
引用 `source` 可为 `page`（仅身份、hero、seo）、`item`（稳定内容 ID）、`fact`（稳定事实 ID）或 `display`（现有展示字段）。`path` 是明确字段路径，条目数组优先以 `{ "id": "稳定条目 ID" }` 寻址。没有条目 ID 的旧数组只能在会计师已知绑定中按其原契约寻址，不能用数组下标匹配另一个职业。`parts` 记录原 UI 的分隔符解析：核心工作的 `｜`、工作现实的换行及 `｜`、双流程的 `→`、薪资来源的 `；` 和 `｜`。各标题、说明和步骤引用原字符串的明确片段，原字符串只保留一份。
