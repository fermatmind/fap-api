<?php

declare(strict_types=1);

namespace Tests\Feature\SeoIntel;

use PHPUnit\Framework\Attributes\Test;
use Tests\TestCase;

final class GlobalEnZhTopicTestLandingHumanReviewImport03Test extends TestCase
{
    #[Test]
    public function topic_test_landing_review_packet_preserves_grounding_gates(): void
    {
        $reportPath = base_path('docs/seo/global-en-zh-topic-test-landing-human-review-import-03.md');
        $generatedPath = base_path('docs/seo/generated/global-en-zh-topic-test-landing-human-review-import-03.v1.json');

        $this->assertFileExists($reportPath);
        $this->assertFileExists($generatedPath);

        $generated = json_decode((string) file_get_contents($generatedPath), true, 512, JSON_THROW_ON_ERROR);
        $this->assertSame('global-en-zh-topic-test-landing-human-review-import-03.v1', $generated['schema_version'] ?? null);
        $this->assertSame('GLOBAL-EN-ZH-TOPIC-TEST-LANDING-HUMAN-REVIEW-IMPORT-03', $generated['task'] ?? null);
        $this->assertSame('topic_test_landing_review_decision_packet_created_ready_for_human_review', $generated['final_decision'] ?? null);

        $decisions = $generated['review_decisions'] ?? [];
        $this->assertCount(21, $decisions);

        foreach ($decisions as $decision) {
            foreach ([
                'faq_visible_authority_backed',
                'cta_source_backed',
                'method_boundary_source_backed',
                'jsonld_grounded',
                'og_alt_needs_review',
                'claim_safe',
                'import_decision',
                'required_reviewer_roles',
            ] as $requiredKey) {
                $this->assertArrayHasKey($requiredKey, $decision);
            }
            $this->assertTrue($decision['human_review_required'] ?? false);
            $this->assertFalse($decision['schema_activation_allowed'] ?? true);
            $this->assertFalse($decision['faqpage_activation_allowed'] ?? true);
            $this->assertFalse($decision['publish_ready'] ?? true);
            $this->assertFalse($decision['sitemap_eligible_after_import'] ?? true);
            $this->assertFalse($decision['llms_eligible_after_import'] ?? true);
            $this->assertFalse($decision['search_channel_eligible_after_import'] ?? true);
        }

        $this->assertSame(6, $generated['summary']['deferred_missing_authority'] ?? null);
        $this->assertSame(0, $generated['summary']['schema_activation_allowed'] ?? null);
        $this->assertSame(0, $generated['summary']['publish_ready'] ?? null);
        $this->assertNotEmpty($generated['blocked_items'] ?? []);
        $this->assertNotEmpty($generated['claim_review_items'] ?? []);

        $evidence = $generated['browser_cms_read_only_evidence'] ?? [];
        $this->assertFalse($evidence['cms_mutation_performed'] ?? true);
        $this->assertFalse($evidence['browser_write_actions_performed'] ?? true);
        $this->assertFalse($evidence['private_user_data_accessed'] ?? true);

        $this->assertTrue($generated['no_cms_mutation'] ?? false);
        $this->assertTrue($generated['no_publish'] ?? false);
        $this->assertTrue($generated['no_deploy'] ?? false);
        $this->assertTrue($generated['no_search_channel_action'] ?? false);
        $this->assertTrue($generated['no_url_submission'] ?? false);
        $this->assertTrue($generated['no_pseo_generation'] ?? false);
        $this->assertTrue($generated['no_frontend_fallback_authority'] ?? false);
        $this->assertSame('GLOBAL-EN-ZH-RESEARCH-CLAIM-HUMAN-REVIEW-04', $generated['next_task'] ?? null);
    }
}
