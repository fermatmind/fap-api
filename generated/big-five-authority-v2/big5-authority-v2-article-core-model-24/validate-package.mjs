...
const locked = matrix.themes.filter((theme) => theme.batch === 24);
...
assert(finalPackage.assets.every((asset) => asset.batch === 24));
...
for (const forbidden of ['/attempt/', '/report/', '/orders/', 'clinically validated', 'guaranteed accurate']) assert(!serialized.includes(forbidden));

console.log('Big Five batch 24 validation passed: exact 5 locked themes / 10 bilingual Article candidates / full review chain');
