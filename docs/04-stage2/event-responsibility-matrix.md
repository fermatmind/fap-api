...
### 1.1 字段约定
- **event_code**：事件代码（详见 `docs/03-stage1/fap-v0.3-glossary.md`）
...
> 注：v0.3-B 允许“先约定口径，后补实现”。先把字段写进矩阵，前后端逐步补齐上报即可。

### 1.3 周报核心口径（默认）
v0.3-B 核心漏斗事件：
- `scale_view` → `test_start` → `test_submit` → `result_view` → `share_generate`

v0.3-B 用户权益最小通道事件（不一定进入漏斗，但要可审计）：
...
| **test_submit** | backend | `POST /api/v0.3/attempts` **事务成功提交后**（attempts+results 写入成功） | `attempt_id` | - | 必须后端触发，确保与 attempts/results 强一致；事务失败回滚则不记；可在 meta 带 `scale_version/profile_version/content_package_version/type_code`（如可）。 |
...
| **export_request_submit** | backend | 用户权益接口创建成功（`type=export`）后 | `request_id` | - | 同上；导出完成后续可扩展 `export_request_fulfilled`（v0.3-B 暂不要求）。 |
