...
final class WebhookPayloadSizeLimitTest extends TestCase
{
    ...
    {
        config()->set('payments.webhook_max_payload_bytes', 128);
        ...
        self::assertGreaterThan(128, strlen($rawBody));
    ...
    public function test_normal_webhook_writes_payload_forensics_without_full_payload_json(): void
    {
        ...
        config()->set('payments.webhook_max_payload_bytes', 65536);
        ...
        $this->assertSame($eventId, $summary['event_id'] ?? null);
        $this->assertSame($orderNo, $summary['order_no'] ?? null);
        $this->assertSame(4990, (int) ($summary['amount'] ?? -1));
        $this->assertSame('USD', $summary['currency'] ?? null);
        $this->assertSame('payment_intent.succeeded', $summary['type'] ?? null);
