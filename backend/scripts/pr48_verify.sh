...
ART_DIR="${ART_DIR:-${BACKEND_DIR}/artifacts/pr48}"
SERVE_PORT="${SERVE_PORT:-1848}"
HOST="127.0.0.1"
API_BASE="http://${HOST}:${SERVE_PORT}"
SCALE_CODE="${SCALE_CODE:-MBTI}"
ANON_ID="${ANON_ID:-pr48-verify-anon}"
WEBHOOK_SECRET="${INTEGRATIONS_WEBHOOK_MOCK_SECRET:-pr48_mock_secret}"
...
EVENT_TOKEN="$(cd "${BACKEND_DIR}" && ANON_ID="${ANON_ID}" php -r '
require "vendor/autoload.php";
$app=require "bootstrap/app.php";
$app->make(Illuminate\Contracts\Console\Kernel::class)->bootstrap();
$token = "fm_" . (string) Illuminate\Support\Str::uuid();
Illuminate\Support\Facades\DB::table("fm_tokens")->insert([
    "token" => $token,
    "anon_id" => (string) getenv("ANON_ID"),
    "user_id" => 2001,
...
TOO_LARGE_EVENT_PAYLOAD_JSON="${ART_DIR}/event_payload_too_large.json"
ATTEMPT_ID="${ATTEMPT_ID}" ANON_ID="${ANON_ID}" php -r '
$payload = [
    "event_code" => "pr48_payload_too_large",
    "attempt_id" => (string) getenv("ATTEMPT_ID"),
    "anon_id" => (string) getenv("ANON_ID"),
    "meta_json" => [
        "blob" => str_repeat("x", 140000),
    ],
];
echo json_encode($payload, JSON_UNESCAPED_UNICODE | JSON_UNESCAPED_SLASHES);
' > "${TOO_LARGE_EVENT_PAYLOAD_JSON}"

EVENT_TOO_LARGE_JSON="${ART_DIR}/event_payload_too_large_response.json"
http_code="$(curl -sS -o "${EVENT_TOO_LARGE_JSON}" -w "%{http_code}" \
  -X POST -H "Content-Type: application/json" -H "Accept: application/json" \
...
if [[ "${http_code}" != "413" ]]; then
  ...
  fail "event payload too large should return 413"
fi

php -r '
$j = json_decode(file_get_contents($argv[1]), true);
if (!is_array($j)) {
    fwrite(STDERR, "event_payload_too_large_response_invalid\n");
    exit(1);
}
if (($j["ok"] ?? null) !== false || (string) ($j["error"] ?? "") !== "payload_too_large") {
    fwrite(STDERR, "event_payload_too_large_response_unexpected\n");
    exit(1);
}
' "${EVENT_TOO_LARGE_JSON}" || fail "event payload too large response invalid"
...
$app=require "bootstrap/app.php";
$app->make(Illuminate\Contracts\Console\Kernel::class)->bootstrap();
$count = (int) Illuminate\Support\Facades\DB::table("events")
    ->where("event_code", "pr48_payload_too_large")
    ->count();
if ($count !== 0) {
    fwrite(STDERR, "event_payload_too_large_inserted\n");
    exit(1);
}
echo "event_payload_too_large_count=".$count.PHP_EOL;
' > "${ART_DIR}/event_payload_db_assert.txt"
cd "${REPO_DIR}"

WEBHOOK_PAYLOAD_JSON="${ART_DIR}/webhook_payload.json"
php -r '
$payload = [
    "event_id" => "evt_pr48_valid",
...
VALID_WEBHOOK_JSON="${ART_DIR}/webhook_valid.json"
...
if [[ "${http_code}" != "200" ]]; then
  ...
  fail "valid webhook signature should pass"
fi

MISSING_TS_JSON="${ART_DIR}/webhook_missing_timestamp.json"
...
if [[ "${http_code}" != "404" ]]; then
  ...
  fail "missing timestamp must return 404"
fi
...
EXPIRED_JSON="${ART_DIR}/webhook_expired_timestamp.json"
...
if [[ "${http_code}" != "404" ]]; then
  ...
  fail "expired timestamp must return 404"
fi

cat > "${ART_DIR}/webhook_summary.txt" <<TXT
valid_signature_http=200
missing_timestamp_http=404
expired_timestamp_http=404
TXT

cd "${BACKEND_DIR}"
php artisan test --filter 'EventPayloadLimitsTest|SensitiveDataRedactorTest' > "${ART_DIR}/phpunit.txt"
cd "${REPO_DIR}"
