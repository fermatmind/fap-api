...
namespace App\Filament\Ops\Resources\MethodPageResource\Support;

use App\Models\MethodPage;
use App\Models\MethodPageRevision;
use App\Models\MethodPageSeoMeta;

final class MethodPageWorkspace
{
    /**
    ...
    public static function normalizeMethodCode(?string $value): string
    {
     ...
     * @return array<string, mixed>
     */
    public static function workspaceSeoFromRecord(?MethodPage $page): array
    {
        ...
        if (! $seoMeta instanceof MethodPageSeoMeta) {
            return $state;
        }
     ...
     */
    public static function syncWorkspaceSeo(MethodPage $page, array $state): void
    {
        MethodPageSeoMeta::query()->updateOrCreate(
    ...
    public static function createRevision(MethodPage $page, ?string $note = null): void
    {
        $nextRevision = ((int) MethodPageRevision::query()
            ->where('method_page_id', (int) $page->id)
            ->max('revision_no')) + 1;

        MethodPageRevision::query()->create([
            'method_page_id' => (int) $page->id,
