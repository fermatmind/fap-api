...
> Version: API v0.3 (Stage 1 / v0.3-A)
> Related Docs:
> - docs/README.md
...
# API v0.3 规范（FAP / MBTI 主流程）— v0.3 修订版

版本：**v0.3（文档修订）**  
基础版本：v0.3  
适用范围：Fermat Assessment Platform（费马测试平台）MBTI 主流程相关接口。  
目标：支持微信小程序 & Web 端完成一次 MBTI 测试 + 结果展示 + 分享卡 + 埋点上报，并为「动态报告引擎」预留扩展位（不破坏 v0.3 路径）。
...
说明：
	•	content_package_version（v0.3 新增）：内容资产包版本号（动态报告引擎的统一入口）
...
v0.3 仍保留 profile（TypeProfile 骨架），并新增 “动态扩展位”，用于将来千人千面组装。

4.1 GET /api/v0.3/attempts/{attempt_id}/result
...
GET /api/v0.3/attempts/b4e1a8f3-0c91-4a25-a57d-abc123456789/result
...
v0.3 新增说明：
	•	dynamic（新增）：动态报告扩展位（可先返回空数组/空对象，不影响 v0.3 前端）
...
5.1 GET /api/v0.3/attempts/{attempt_id}/share
...
GET /api/v0.3/attempts/b4e1a8f3-0c91-4a25-a57d-abc123456789/share
...
事件类型请参考 docs/03-stage1/fap-v0.3-glossary.md 中 event_code 定义。

6.1 POST /api/v0.3/events
...
v0.3 提供一个只读统计接口，方便后台页面或外部工具拉取核心指标。
与 php artisan fap:weekly-report 命令使用相同口径。

7.1 GET /api/v0.3/stats/summary
...
请求参数（Query）
	...
	•	content_package_version（可选）：内容包版本过滤，如 MBTI-CN-v0.3（v0.3 新增，可选）

示例：

GET /api/v0.3/stats/summary?days=7&scale_code=MBTI
...
9.1 v0.3 的定位
	•	不改变 v0.3 的主路径与主流程：拉题 → 提交 → 出结果 → 埋点
