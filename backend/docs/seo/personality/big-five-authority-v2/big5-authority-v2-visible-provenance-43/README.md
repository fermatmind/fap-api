...
- A visible author requires a real positive `admin_user:*` identity, public label, allowed role, and a `revision-author:*` or `author-ledger:*` authority reference with a non-empty identifier. Generated, self-attested, and prefix-only identities fail closed. Article metadata must agree with the non-null matching published revision actor and any existing Article label; Personality metadata must agree with the asset's non-null `created_by_admin_user_id`; Landing surfaces have no canonical author actor and therefore fail the author gate closed.
- A visible reviewer additionally requires a real review timestamp and completed review state. Big Five `operator_approved_content_ready` and `seo_discoverability_released` are recognized completed release-review states. Article reviewer metadata must agree with the matching published revision reviewer, timestamp, state, and any existing Article label.
- Missing or fabricated reviewer evidence blocks promotion and visible-reviewer eligibility. Personality assets, Topic revisions, and Landing surfaces have no canonical reviewer actor in the current backend contract, so their reviewer projection remains fail closed even when metadata contains a matching state and timestamp; later promotion work must supply a verifiable backend review ledger/actor. The projector is read-only and never removes or overwrites existing published content.
- Article provenance is eligible only from the live `published_revision_id`, matching tenant and locale, a null-or-effective publication timestamp matching the public-read gate, and a public non-archived Article. Stale, future, foreign, archived, and soft-deleted revisions expose no provenance. Topic provenance is read from the canonical `snapshot_json.profile` payload of its current published revision, and its visible author must match that revision's non-null `created_by_admin_user_id` actor.
...
- Sources are explicitly classified as `academic_evidence`, `internal_policy`, or `product_authority`. Existing source-ledger aliases are normalized into those public categories, and authority references must use the matching `source-ledger:academic:`, `policy:`, or `product-contract:` namespace followed by a non-empty identifier; self-attested, generated, or prefix-only references fail closed.
