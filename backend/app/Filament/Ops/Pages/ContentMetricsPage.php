...
class ContentMetricsPage extends Page
{
    ...
    public function mount(): void
    {
        ...
        $articleTotal = Article::query()->whereIn('org_id', $currentOrgIds)->count();
        $articlePublishedPublic = Article::query()
            ->whereIn('org_id', $currentOrgIds)
            ->where('status', 'published')
            ->where('is_public', true)
            ->count();
        $articlePublishedPrivate = Article::query()
            ->whereIn('org_id', $currentOrgIds)
            ->where('status', 'published')
            ->where('is_public', false)
            ->count();
        $articleIndexable = Article::query()
            ->whereIn('org_id', $currentOrgIds)
            ->where('is_indexable', true)
            ->count();
        $articleDrafts = Article::query()
            ->whereIn('org_id', $currentOrgIds)
            ->where('status', 'draft')
            ->count();
        $articleStaleDrafts = Article::query()
            ->whereIn('org_id', $currentOrgIds)
            ->where('status', 'draft')
        ...
        $guidePublishedPrivate = (clone $guideBaseQuery)
            ...
            ->where('is_public', false)
            ->count();
        $guideStaleDrafts = (clone $guideBaseQuery)
            ->where('status', CareerGuide::STATUS_DRAFT)
            ->where('updated_at', '<', $staleThreshold)
            ->count();
        ...
        $jobPublishedPrivate = (clone $jobBaseQuery)
            ...
            ->where('is_public', false)
            ->count();
        $jobStaleDrafts = (clone $jobBaseQuery)
            ->where('status', CareerJob::STATUS_DRAFT)
            ->where('updated_at', '<', $staleThreshold)
            ->count();
