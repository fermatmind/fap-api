...
- `W4 / riasec`: audit-compatible through the exact RIASEC content authority, immutable package manifest, target-bound snapshots, deterministic readback and rollback.
- `W7 / eq`: audit-compatible through the compiled EQ60 authority, linted report-assets package, exact readback, idempotent publication and bounded rollback.
- `W6 / iq`: deliberately remains the sole legacy audit-incompatible adapter. The operator deferred the IQ lane; it must fail closed until explicitly reactivated and supplied with compatible revision, audit metadata, readback and rollback authority.
