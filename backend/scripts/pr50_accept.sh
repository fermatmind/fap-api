...
ART_DIR="${BACKEND_DIR}/artifacts/pr50"
SERVE_PORT="${SERVE_PORT:-1850}"
DB_PATH="/tmp/pr50.sqlite"
...
export BILLING_WEBHOOK_SECRET="${BILLING_WEBHOOK_SECRET:-pr50_billing_secret}"
export BILLING_WEBHOOK_TOLERANCE_SECONDS="${BILLING_WEBHOOK_TOLERANCE_SECONDS:-300}"
export BILLING_WEBHOOK_ALLOW_LEGACY_SIGNATURE="${BILLING_WEBHOOK_ALLOW_LEGACY_SIGNATURE:-false}"
...
SERVE_PORT="${SERVE_PORT}" ART_DIR="${ART_DIR}" bash "${BACKEND_DIR}/scripts/pr50_verify.sh"
...
PR50 Acceptance Summary
- pass_items:
  - billing_webhook_signature_timestamp_hardening: OK
  - billing_webhook_duplicate_guard: OK
...
bash "${BACKEND_DIR}/scripts/sanitize_artifacts.sh" 50

bash -n "${BACKEND_DIR}/scripts/pr50_accept.sh"
bash -n "${BACKEND_DIR}/scripts/pr50_verify.sh"
