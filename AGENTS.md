...
- Preflight is read-only and must prove zero pending `ops` jobs, no deploy lock/process, and a missing program before first install. Apply may install that one exact config and start that one worker only; it must not deploy code, move the active symlink, migrate, or change CMS/database authority, publication, sitemap, llms, search, or PR23 state.
