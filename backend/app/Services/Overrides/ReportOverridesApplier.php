...
class ReportOverridesApplier
{
    ...
    public function applyCards(string $contentPackageDir, string $typeCode, string $sectionKey, array $cards, array $ctx = []): array
    {
        ...
        if ($capture && is_callable($collector) && is_array($this->lastExplain['cards'][$sectionKey] ?? null)) {
            ...
            if (! isset($payload['context_tags']) || ! is_array($payload['context_tags'])) {
                $payload['context_tags'] = [];
            }
            if (! isset($payload['selected']) || ! is_array($payload['selected'])) {
                $payload['selected'] = [];
            }
            if (! isset($payload['rejected']) || ! is_array($payload['rejected'])) {
                $payload['rejected'] = [];
            }
        ...
        $out = array_map(function ($it) {
            ...
            if (! isset($it['tips']) || ! is_array($it['tips'])) {
                $it['tips'] = [];
            }
    ...
    private function filterRulesByTarget(?array $doc, string $target): array
    {
        ...
        foreach ($rules as $r) {
            ...
            if (is_array($ts) && in_array($target, $ts, true)) {
                $ok = true;
            }
    ...
    private function applyRulesToList(array $items, array $rules, array $context): array
    {
        ...
        foreach ($rules as $r) {
            ...
            if (! isset($r['mode']) && isset($r['action'])) {
                $mode = 'filter';
            }
        ...
        foreach ($normalRules as $rule) {
            ...
            if (isset($rule['match']) && is_array($rule['match']) && ! empty($rule['match'])) {
                $matchedBy[] = 'match';
            }
            if (isset($rule['when']) && is_array($rule['when']) && ! empty($rule['when'])) {
                $matchedBy[] = 'when';
            }
            if (empty($matchedBy)) {
                $matchedBy[] = 'default';
    ...
    private function hasTagConditions(array $rule): bool
    {
        ...
        foreach (['require_any', 'require_all', 'forbid'] as $k) {
            if (isset($rule[$k]) && is_array($rule[$k]) && $rule[$k] !== []) {
                return true;
            }
            if (isset($when[$k]) && is_array($when[$k]) && $when[$k] !== []) {
                return true;
            }
        }
    ...
    {
        $id = (string) ($rule['id'] ?? '');
        if ($id !== '' && str_contains($id, 'drop_all_default')) {
            return true;
        }
    ...
    private function explainDetailsForRuleOnTags(array $rule, array $tags): array
    {
        ...
        if (! empty($reqAny) && $detail['need_min_match'] < 1) {
            $detail['need_min_match'] = 1;
        }
        ...
        foreach ($reqAll as $t) {
            ...
            if (in_array($t, $tags, true)) {
                $detail['hit_require_all'][] = $t;
            } else {
                $detail['miss_require_all'][] = $t;
            }
        }
    ...
    private function ruleMatchesContext(array $rule, array $context): bool
    {
        ...
        if (array_key_exists('type_code', $match)) {
            ...
            if (is_string($want)) {
                ...
                if (! in_array($typeCode, $want, true)) {
                    return false;
                }
            }
        }
        ...
        if (isset($match['type_codes']) && is_array($match['type_codes'])) {
            if (! in_array($typeCode, $match['type_codes'], true)) {
                return false;
            }
        }
        ...
        if (array_key_exists('section', $match)) {
            ...
            if (is_string($want)) {
                ...
                if (! in_array($sectionKey, $want, true)) {
                    return false;
                }
            }
        }
        ...
        if (isset($match['section_key']) && is_string($match['section_key'])) {
            if ($sectionKey !== $match['section_key']) {
                return false;
            }
        }
        if (isset($match['sections']) && is_array($match['sections'])) {
            if (! in_array($sectionKey, $match['sections'], true)) {
                return false;
            }
        }
    ...
    private function selectIndexes(array $list, $selector): array
    {
        ...
        // by id(s)
        $wantIds = [];
        if (isset($selector['id']) && is_string($selector['id'])) {
            $wantIds[] = $selector['id'];
        }
        ...
        if (! empty($wantIds)) {
            ...
            foreach ($list as $idx => $it) {
                ...
                $id = $it['id'] ?? null;
                if (is_string($id) && in_array($id, $wantIds, true)) {
                    $out[] = (int) $idx;
                }
            }
    ...
    private function shouldExplain(array $ctx = []): bool
    {
        ...
        if ((bool) ($ctx['overrides_debug'] ?? false)) {
            return true;
        }

        // ✅ 你现在验证时已经在用 RE_EXPLAIN=1，就复用它
        if ((bool) \App\Support\RuntimeConfig::value('RE_EXPLAIN', false)) {
            return true;
        }

        // 也允许单独开关
        if ((bool) \App\Support\RuntimeConfig::value('OVR_EXPLAIN', false)) {
            return true;
        }
