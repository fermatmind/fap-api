...
Career list/detail review state defaults to `unknown` with no review timestamp. Rejected, exception, partial, duplicate, malformed, missing, cold, and stale evidence cannot inherit an older trust status. The visible-claims target binds the complete recursive public detail projection after removing only derived review fields and private/internal evidence keys. Any public field or exact index-entry drift changes the target/package SHA and invalidates the whole batch.
