...
final class MigrationNoSilentCatchTest extends TestCase
{
    #[Test]
    ...
    #[Test]
    public function migration_catch_blocks_must_rethrow_or_handle_explicitly(): void
    {
        foreach ($this->migrationFiles() as $filePath) {
            $source = (string) file_get_contents($filePath);
            foreach ($this->extractCatchBodies($source) as $catchBody) {
                ...
                $hasExplicitHandling = preg_match(
                    '/\b(if|return|continue|break|SchemaIndex::|Log::|report\s*\()/',
                    $bodyWithoutComments
                ) === 1;

                $this->assertTrue(
                    $hasThrow || $hasExplicitHandling,
                    "Catch block must rethrow or handle explicitly in {$filePath}"
                );
            }
        }
    }
