...
- 2026-09-24 新增 URL Inspection 抽查：中文 `infp-t` 人格页、`big-five-30-facets-explained-with-model-caveats` 文章页和 `actors` 职业页也显示 **Indexed successfully**。`infp-t` 的最近抓取为当日 02:15，允许抓取、抓取成功、允许索引。
- 2026-09-25 再查中文 Big Five 与九型人格核心测评页：Bing Index 均显示 **Indexed successfully / URL can appear on Bing**，无 SEO/GEO 问题；均允许抓取、抓取成功、允许索引。Big Five 显示 2026-02-25 发现、最近抓取“Yesterday at 14:39”；九型显示 2026-04-22 发现、最近抓取“Yesterday at 01:01”（时间为站长工具原样显示，未换算时区）。两页检查详情的 canonical 字段为 `-`，仍应以线上自指 canonical 单独核验。十个已索引样本不能外推为全站覆盖率。
...
- 分层：sitemap 证明 **可发现候选**；十个受检 URL 可升至 **Bing 已索引**；上述两个职业页为 **当前可索引、Bing 快照未索引**；**有 Web 曝光、有 Web 点击**仍待 Search Performance 报告。Site Explorer 当前显示 `No data available`，Bingbot 的全站 4xx/5xx、robots 命中及抓取量尚不能汇总，样本不能外推至 579 页。
...
- `909c5cda5` 已完成同 SHA CI、预发、生产激活、线上 smoke 与 A08 验收，新增独立文章 IndexNow 自动通道：按发布决定的搜索表面指纹识别首次发布、重新发布、URL 或摘要变化；只允许公开文章、精确 URL Truth 和单一 IndexNow URL；提交前读取公开 HTML，核验 HTTP 200、自指 canonical、`index, follow`、现行 SEO 标题与描述及非空正文；复用 Search Channel Queue 的规划、入队、审批、原子提交和脱敏响应日志。生产调度每 10 分钟、单实例运行；同一来源版本已入队/提交则停止，正文单独变化不触发搜索提交。聚焦测试覆盖一次提交与重复运行、提供方接受记录、错误页面拦截。**本次验收证明代码已部署且线上 smoke 通过，尚未取得该新通道的真实 IndexNow provider 回执，更不能推断 Bing 已收录。**旧的四个全局开关仍关闭，新通道仅限经过上述验收的文章。
- 对上述中文 RIASEC 文章运行生产只读 `seo-intel:search-channel-queue --dry-run --no-write`，IndexNow planner 返回候选 1、合格 1、计划入队 1、无重复或问题，且写入与外部调用均未尝试。该 URL 的可入队性已有实证；这是自动通道部署**之前**的诊断，不代表部署后已入队或已调用 IndexNow。历史文章的旧发布决定缺少新的搜索表面指纹，新通道不追溯提交旧 URL；Ops 中该篇文章当前仍显示 IndexNow 队列缺口，应在后续合格发布决定出现时核对队列和 provider 回执，不因旧缺口批量重提全站 URL。
...
2026-09-25 同一窗口、China 筛选下，`审计师`约 1.2K **全网关键词曝光**。前十结果中除职业百科外，多为审计师考试、报名、资格规定及备考内容；相关词也集中于初中级考试与报名。`accountants-and-auditors` 的中文 Current 页面确有增强正文、20 条 fact register 事实，含中国大陆工资资料，但“中国大陆经济和金融专业人员年工资中位数”不是审计师专属薪资，不能替代职业精确口径。故“审计师”虽有需求，其主要 SERP 意图与职业介绍页只部分相合；在得到本站 Bing Web 查询×页面证据前，不以该词单独决定试点优先级，也不把考试流量当成本站机会。
...
`项目经理`在同一 2026-06-24 至 09-21 窗口显示全球约 4.2K、中国约 4K 次全网关键词曝光；相关词 `项目经理职责`约 1.3K、`项目经理岗位职责`约 1.1K。Bing 展示的前十名以百科、知乎、项目管理文章和职业职责说明为主，存在职业介绍意图，但竞争强。Current manifest 中 `project-management-specialists` 与 `information-technology-project-managers` 的中文页均标记 `has_public_body: false`，不能作为已上线可索引职业正文纳入本轮试点；先完成原有内容权威选中迭代与上线验收，再考虑该需求。上述数字不代表本站曝光或点击。
