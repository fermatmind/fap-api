...
If that complete diagnostic reports
`PASS_DIAGNOSTIC_COMPLETE_STATE_DRIFT`, the old recovery payload-set is no
longer an eligible resume baseline. After a separate design authorization, the
existing resume workflow may accept the exact diagnostic run/attempt, immutable
receipt SHA, diagnostic state SHA, and observed payload-set SHA as a rebase
chain. It must independently revalidate the original failed execute, recovery,
failed resume-preflight, and diagnostic receipts; require the exact latest
control plane and unchanged active release; and reject any later refresh,
recovery, diagnostic, or resume run.

The diagnostic-rebased `preflight` requires a new exact operator phrase. It
performs a fresh complete 100-URL read, requires the payload-set and issue
counts to match the diagnostic receipt exactly, and only then calculates the
residual target set and batches. It performs no write. Its receipt identifies
`baseline_source=diagnostic` and carries the complete receipt chain. The
subsequent `execute` remains a separate exact phrase generated by that
preflight and revalidates the same state immediately before the first cache
publication. The existing maximum five slugs / ten URLs per batch, 5,000ms
offline publisher, zero per-target retry, database write guard, exact quality
package, complete 100-URL readback, and all prohibited-write boundaries remain
unchanged. Merging the rebase control does not authorize either dispatch.
