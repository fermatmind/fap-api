...
final class PacksPublishSds20Test extends TestCase
{
    use RefreshDatabase;

    private const DIR_ALIAS = 'SDS-20-CI-TEST';
    ...
    public function test_packs_publish_creates_release_and_audit_for_sds20(): void
    {
        $target = base_path('../content_packages/default/CN_MAINLAND/zh-CN/'.self::DIR_ALIAS);
        if (File::isDirectory($target)) {
            File::deleteDirectory($target);
        ...
        $this->assertSame('SDS_20', (string) $release->to_pack_id);
