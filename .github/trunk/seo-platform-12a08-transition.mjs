...
if (!['prepare','enable'].includes(mode) || !['0','1'].includes(value)) throw new Error('A08_TRANSITION_SCOPE_HOLD');
...
if (mode === 'enable') {
  ...
  ready=ready && ((state.paused === true && state.selected_missions.length === 0
    ...
    || (state.paused === false && state.selected_missions.includes(id)
      && !state.selected_missions.includes(MISSIONS[2])));
  generation=state.generation;
} else {
  const state=read('a08-install-after.json');
  const retained=state.paused === false && state.selected_missions.includes(MISSIONS[0])
    && manifest.missions[MISSIONS[0]].end_to_end_acceptance?.status === 'pass';
  ready=ready && (existsSync('enabled-0.json') || retained);
  generation=existsSync('enabled-0.json') ? read('enabled-0.json').generation : state.generation;
}
