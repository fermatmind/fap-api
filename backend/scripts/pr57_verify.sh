...
cd "${BACKEND_DIR}"
php artisan route:list --path=api/v0.3/webhooks/payment --json > "${ART_DIR}/route_webhook.json"
cd "${REPO_DIR}"
grep -E "v0\.3\.webhooks\.payment|PaymentWebhookController@handle" "${ART_DIR}/route_webhook.json" >/dev/null || fail "route wiring missing"
...
if ($configPackId === "" || strtolower($configPackId) === "default") {
    fwrite(STDERR, "invalid_default_pack_id\n");
    exit(1);
}
if ($configDirVersion === "") {
    fwrite(STDERR, "invalid_default_dir_version\n");
    exit(1);
}
...
if ($registryPackId !== $configPackId || $registryDirVersion !== $configDirVersion) {
...
q_code="$(curl -sS -o "${QUESTIONS_JSON}" -w "%{http_code}" \
  -H "Accept: application/json" \
  "${API_BASE}/api/v0.3/scales/${SCALE_CODE}/questions" || true)"
if [[ "${q_code}" != "200" ]]; then
  echo "questions_failed http=${q_code}" >&2
...
echo (string) (is_array($data) ? count($data) : 0);
' "${ANSWERS_JSON}")"
...
START_PAYLOAD="$(SCALE_CODE="${SCALE_CODE}" ANON_ID="${ANON_ID}" php -r '
$payload = ["scale_code" => getenv("SCALE_CODE"), "anon_id" => getenv("ANON_ID")];
echo json_encode($payload, JSON_UNESCAPED_UNICODE);
')"
start_code="$(curl -sS -o "${ATTEMPT_START_JSON}" -w "%{http_code}" \
  -X POST -H "Content-Type: application/json" -H "Accept: application/json" \
  --data "${START_PAYLOAD}" \
  "${API_BASE}/api/v0.3/attempts/start" || true)"
if [[ "${start_code}" != "200" ]]; then
  echo "attempt_start_failed http=${start_code}" >&2
...
submit_code="$(curl -sS -o "${ATTEMPT_SUBMIT_JSON}" -w "%{http_code}" \
  -X POST -H "Content-Type: application/json" -H "Accept: application/json" \
  --data @"${SUBMIT_PAYLOAD}" \
  "${API_BASE}/api/v0.3/attempts/submit" || true)"
if [[ "${submit_code}" != "200" ]]; then
  echo "attempt_submit_failed http=${submit_code}" >&2
...
cd "${BACKEND_DIR}"
php artisan test --filter BillingWebhookSignatureTest > "${ART_DIR}/phpunit_billing_signature.txt" 2>&1
php artisan test --filter PaymentWebhookRouteWiringTest > "${ART_DIR}/phpunit_route_wiring.txt" 2>&1
php artisan test --filter BillingWebhookMisconfiguredSecretTest > "${ART_DIR}/phpunit_misconfigured_secret.txt" 2>&1
cd "${REPO_DIR}"
