...
class BigFiveAuthorityV226Test extends TestCase
{
    ...
    {
        parent::setUp();
        $this->packagePath = base_path('../generated/big-five-authority-v2/big5-authority-v2-article-workplace-26');
    }

    public function test_final_package_has_exact_batch_26_bilingual_inventory(): void
    {
        ...
        $this->assertSame([26], array_values(array_unique(array_column($package['assets'], 'batch'))));
        $this->assertCount(5, array_unique(array_column($package['assets'], 'unique_intent_key')));
    }

    public function test_every_asset_has_complete_structure_and_workplace_boundary(): void
    {
        $package = $this->readJson('final-package.json');
        ...
        foreach ($package['assets'] as $asset) {
            ...
            $boundaryTerms = $asset['locale'] === 'en'
                ? ['hiring', 'screening', 'promotion', 'performance', 'suitability', 'unknown']
                : ['招聘', '筛选', '晋升', '绩效', '适配', 'unknown'];
            foreach ($boundaryTerms as $term) {
    ...
    public function test_qa_proves_locked_scope_and_zero_release_mutations(): void
    {
        ...
        $this->assertTrue($qa['checks']['consumes_only_pr21_batch_26']);
        $this->assertTrue($qa['checks']['workplace_decision_use_prohibited']);
