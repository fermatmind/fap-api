...
class BigFiveAuthorityV217Test extends TestCase
{
    private const DIR = 'generated/big-five-authority-v2/big5-authority-v2-facets-extraversion-17';
    ...
    /** @var list<string> */
    private const FACETS = ['warmth', 'gregariousness', 'assertiveness', 'activity', 'excitement-seeking', 'positive-emotions'];
    ...
    public function test_package_workflow_passes_with_twelve_pages_and_closed_release_gates(): void
    {
        ...
        $this->assertSame([
            '/en/personality/big-five/facets/activity',
            '/en/personality/big-five/facets/assertiveness',
            '/en/personality/big-five/facets/excitement-seeking',
            '/en/personality/big-five/facets/gregariousness',
            '/en/personality/big-five/facets/positive-emotions',
            '/en/personality/big-five/facets/warmth',
            '/zh/personality/big-five/facets/activity',
            '/zh/personality/big-five/facets/assertiveness',
            '/zh/personality/big-five/facets/excitement-seeking',
            '/zh/personality/big-five/facets/gregariousness',
            '/zh/personality/big-five/facets/positive-emotions',
            '/zh/personality/big-five/facets/warmth',
    ...
    public function test_final_package_preserves_exact_facet_pairs_taxonomy_and_content_contract(): void
    {
        ...
        foreach ($package['pages'] as $page) {
            ...
            $this->assertSame('extraversion', $page['domain_code']);
            $this->assertSame('ipip_neo_30_facet_navigation', $page['taxonomy']);
            $this->assertSame('facet:extraversion:'.$page['facet_code'], $page['content_key']);
        ...
        $this->assertStringContainsString('not empathy', $pairs['warmth']['en']['summary']);
        $this->assertStringContainsString('不等于社交技能', $pairs['gregariousness']['zh-CN']['summary']);
        $this->assertStringContainsString('not correctness', $pairs['assertiveness']['en']['summary']);
        $this->assertStringContainsString('恢复间隔', $pairs['activity']['zh-CN']['low_risk_action']);
        $this->assertStringContainsString('not courage', $pairs['excitement-seeking']['en']['summary']);
        $this->assertStringContainsString('不等于心理健康', $pairs['positive-emotions']['zh-CN']['summary']);
    ...
    {
        $package = $this->readJson(self::DIR.'/final-package.json');
        $package['pages'][0]['facet_code'] = 'gregariousness';
    ...
    {
        $path = tempnam(sys_get_temp_dir(), 'big5-authority-v2-facets-extraversion-');
