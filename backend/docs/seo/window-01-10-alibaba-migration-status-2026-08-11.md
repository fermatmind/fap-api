...
3. 验证 active revision、migration status、queue/scheduler、Redis/RDS；公开 `/up` 必须为 `200`；target-node loopback `/api/healthz` 必须为 `200` 且 `ok=true`，非 allowlisted 公网同路径必须严格为 `404`；flags 与 zh-CN Big Five Hub Personality API 必须为 `200`，后者还必须有 64 位小写十六进制 `source_hash`；再读回 Career directory/detail 与 sitemap source。
