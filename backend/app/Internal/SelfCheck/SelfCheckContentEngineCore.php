...
class SelfCheckContentEngineCore
{
    ...
    public function findManifestByPackId(string $packId): ?string
    {
        ...
        foreach ($it as $file) {
            ...
            if (str_contains($norm, '/_deprecated/')) {
                continue;
            }
            if (! str_contains($norm, '/default/')) {
                continue;
            }
    ...
    public function checkAssetsSchemasAgainstManifest(array $manifest, string $baseDir, string $packId): array
    {
        ...
        if (! is_array($schemas) || ! is_array($assets)) {
            return $errs;
        }
        ...
        $assertOne = function (string $assetLabel, string $assetKey, string $rel) use (&$errs, &$usedSchemaKeys, $manifest, $readSchema, $packId) {
            if (! is_string($rel) || trim($rel) === '') {
                return;
            }
            ...
            if ($assetKey === 'identity') {
                $bn = basename($rel);
                if (str_contains($bn, 'identity_cards')) {
                    $label = 'identity_cards';
                } elseif (str_contains($bn, 'identity_layers')) {
                    $label = 'identity_layers';
                } elseif (str_contains($bn, 'roles')) {
                    $label = 'roles';
                } elseif (str_contains($bn, 'strategies')) {
                    $label = 'strategies';
                }
            }
            ...
            if ($expected === null) {
                $errs[] = "pack={$packId} file={$abs} :: missing manifest.schemas mapping (asset={$label})";
            ...
            if (! is_string($got) || trim((string) $got) === '') {
                $errs[] = "pack={$packId} file={$abs} path=$.schema :: missing schema field (asset={$label}) want=".var_export($expected, true);
    ...
    ." (asset={$label})";
        ...
        foreach ($assets as $assetKey => $paths) {
            ...
            if ($assetKey === 'overrides' && is_array($paths) && $this->isAssocArray($paths)) {
                foreach ($paths as $bucket => $list) {
                    ...
                    $bucketKey = null;
                    if ($bucket === 'unified') {
                        $bucketKey = 'overrides_unified';
                    }
                    if ($bucket === 'highlights_legacy') {
                        $bucketKey = 'overrides_highlights_legacy';
                    }
        ...
        $collectSchemaKey = function (string $assetKey, string $file) use (&$used) {
            ...
            if ($assetKey === 'borderline') {
                if (str_contains($file, 'borderline_templates')) {
                    $used['borderline_templates'] = true;
                }
                if (str_contains($file, 'borderline_notes')) {
                    $used['borderline_notes'] = true;
                }
            ...
            if ($assetKey === 'identity') {
                if (str_contains($file, 'identity_cards')) {
                    $used['identity_cards'] = true;
                }
                if (str_contains($file, 'identity_layers')) {
                    $used['identity_layers'] = true;
                }
                if (str_contains($file, 'roles')) {
                    $used['roles'] = true;
                }
                if (str_contains($file, 'strategies')) {
                    $used['strategies'] = true;
                }
            ...
            if ($assetKey === 'overrides_unified') {
                $used['overrides_unified'] = true;
            }
            if ($assetKey === 'overrides_highlights_legacy') {
                $used['overrides_highlights_legacy'] = true;
            }
        };
        ...
        if (! is_array($faq) || $this->isAssocArray($faq)) {
            ...
            foreach ($faq as $i => $it) {
                ...
                if (! is_string($q) || trim($q) === '') {
                    $errors[] = "pack={$packId} file={$path} path={$bp}.question :: missing/invalid question";
                }
                if (! is_string($a) || trim($a) === '') {
                    $errors[] = "pack={$packId} file={$path} path={$bp}.answer :: missing/invalid answer";
                }
            }
        }
    ...
    public function landingMetaNode(array $doc): array
    {
        ...
        if (is_array($landing) && $this->isAssocArray($landing)) {
            return $landing;
        }
    ...
    public function checkShareTemplatesGate(array $manifest, string $manifestPath, string $packId): array
    {
        ...
        if ($warnings) {
            $notes = array_merge($notes, ['-- warnings --'], array_slice($warnings, 0, 120));
        }
    ...
    public function checkReportOverrides(string $path, array $manifest, string $packId, ?string $expectedSchema = null): array
    {
        ...
        $listKey = null;
        $rules = null;
        if (is_array($doc['overrides'] ?? null)) {
            $listKey = 'overrides';
            $rules = $doc['overrides'];
        } elseif (is_array($doc['rules'] ?? null)) {
            $listKey = 'rules';
            $rules = $doc['rules'];
        }
        ...
        foreach ($rules as $i => $r) {
            ...
            if (array_key_exists('replace_fields', $r)) {
                ...
                if ($mode === 'replace') {
                    ...
                    if ($hasReplace && is_array($r['replace']) && $r['replace'] !== []) {
                        $okCarrier = true;
                    }
    ...
    public function checkReportRules(string $path, array $manifest, string $packId, ?string $expectedSchema = null): array
    {
        ...
        $isIntLike = function ($v): bool {
            ...
            if (is_string($v) && trim($v) !== '' && preg_match('/^-?\d+$/', $v)) {
                return true;
            }
    ...
    public function checkFallbackCardsAgainstSectionPolicies(array $manifest, string $baseDir, string $packId): array
    {
        ...
        foreach ($items as $secKey => $pol) {
            if (! is_string($secKey) || trim($secKey) === '') {
                continue;
            }
            if (! is_array($pol)) {
                $pol = [];
            ...
            if (is_string($pol['fallback_file'] ?? null) && trim((string) $pol['fallback_file']) !== '') {
                ...
                $score = function (string $r) use ($secKey): int {
                    ...
                    $sk = strtolower($secKey);
                    $s = 0;
                    if ($bn === "fallback_cards_{$sk}.json") {
                        $s += 100;
                    }
                    if ($bn === "report_fallback_cards_{$sk}.json") {
                        $s += 95;
                    }
                    if ($bn === "report_cards_fallback_{$sk}.json") {
                        $s += 90;
                    }
                    if (str_contains($bn, "fallback_cards_{$sk}")) {
                        $s += 80;
                    }
                    if (str_contains($bn, "{$sk}_fallback")) {
                        $s += 60;
                    }
                    if (str_contains($bn, "fallback_{$sk}")) {
                        $s += 60;
                    }
    ...
    public function checkQuestions(string $path, string $packId, ?string $expectedSchema = null, ?string $scaleCode = null): array
    {
        ...
        foreach ($items as $i => $q) {
            ...
            if ($qid === null || $qid === '') {
                $errors[] = "pack={$packId} file={$path} path=$.items[{$i}].question_id :: missing question_id";
            }
            if ($qid !== null && $qid !== '' && isset($seenQid[$qidLabel])) {
                $errors[] = "pack={$packId} file={$path} :: duplicate question_id {$qidLabel}";
            }
            if ($qid !== null && $qid !== '') {
                $seenQid[$qidLabel] = true;
            }

            $order = $q['order'] ?? null;
            if (! is_int($order) && ! is_numeric($order)) {
                $errors[] = "pack={$packId} file={$path} path=$.items[{$i}].order :: invalid order";
            }
            if (is_numeric($order)) {
                $orders[] = (int) $order;
            }
            ...
            if ($isMbti) {
                ...
                if (! in_array($dim, $validDims, true)) {
                    $errors[] = "pack={$packId} file={$path} :: invalid dimension {$dim} (qid={$qidLabel})";
                }

                $text = $q['text'] ?? null;
                if (! is_string($text) || trim($text) === '') {
                    $errors[] = "pack={$packId} file={$path} :: missing text (qid={$qidLabel})";
                }

                $keyPole = $q['key_pole'] ?? null;
                if (! is_string($keyPole) || $keyPole === '') {
                    $errors[] = "pack={$packId} file={$path} :: missing key_pole (qid={$qidLabel})";
                }

                $direction = $q['direction'] ?? null;
                if (! in_array((int) $direction, [1, -1], true)) {
                    $errors[] = "pack={$packId} file={$path} :: direction must be 1 or -1 (qid={$qidLabel})";
                }
                ...
                foreach ($needCodes as $c) {
                    ...
                    if (! is_string($t) || trim($t) === '') {
                        $errors[] = "pack={$packId} file={$path} :: option {$c} missing text (qid={$qidLabel})";
                    }
        ...
        if ($isMbti && count($orders) > 0) {
            ...
            $max = max($orders);
            if ($min !== 1 || $max !== 144) {
                $errors[] = "pack={$packId} file={$path} :: order range should be 1..144, got {$min}..{$max}";
            }
        }
    ...
    public function checkTypeProfiles(string $path, string $packId, ?string $expectedSchema = null): array
    {
        ...
        if ($missing) {
            $errors[] = "pack={$packId} file={$path} :: Missing types: ".implode(', ', $missing);
        }
        if ($extra) {
            $errors[] = "pack={$packId} file={$path} :: Extra types: ".implode(', ', $extra);
        }
        if (count($items) !== 32) {
            $errors[] = "pack={$packId} file={$path} :: items count must be 32, got ".count($items);
        }
        ...
        foreach ($expected as $code) {
            ...
            if (($p['type_code'] ?? null) !== $code) {
                $errors[] = "pack={$packId} file={$path} :: {$code} type_code mismatch";
            }
            if (! isset($p['type_name']) || ! is_string($p['type_name']) || trim($p['type_name']) === '') {
                $errors[] = "pack={$packId} file={$path} :: {$code} missing type_name";
            }
            if (! isset($p['tagline']) || ! is_string($p['tagline']) || trim($p['tagline']) === '') {
                $errors[] = "pack={$packId} file={$path} :: {$code} missing tagline";
            }
            if (isset($p['keywords']) && ! is_array($p['keywords'])) {
                $errors[] = "pack={$packId} file={$path} :: {$code} keywords must be array";
            }
        }
    ...
    public function checkHighlightsTemplates(string $path, string $packId, ?string $expectedSchema = null): array
    {
        ...
        if (! is_array($tpl)) {
            return [false, ["pack={$packId} file={$path} path=$.templates :: Missing/invalid templates"]];
        }
    ...
    public function parseHighlightsPoolsDoc(array $doc, string $packId, string $path, array &$errors): array
    {
        ...
        if (isset($doc['pools']) && is_array($doc['pools'])) {
            $root = $doc['pools'];
    ...
    public function collectTemplateRefsFromRules($node, array &$out): void
    {
        ...
        foreach ($node as $k => $v) {
            ...
            if (in_array($key, ['template_id', 'tpl_id', 'tpl', 'template'], true)) {
                ...
                if (is_array($v)) {
                    $this->collectTemplateRefsFromRules($v, $out);
                }
            }
        }
    }
    ...
    public function validateHighlightsPolicy(array $policy, array $countsByPool, string $packId, string $path, array &$errors): void
    {
        ...
        if ($minTotal === null) {
            $errors[] = "pack={$packId} file={$path} path=$.policy.min_total :: must be integer";
        }
        if ($maxTotal === null) {
            $errors[] = "pack={$packId} file={$path} path=$.policy.max_total :: must be integer";
        }
        if ($maxStrength === null) {
            $errors[] = "pack={$packId} file={$path} path=$.policy.max_strength :: must be integer";
        }

        if ($minTotal !== null && $minTotal < 0) {
            $errors[] = "pack={$packId} file={$path} path=$.policy.min_total :: must be >= 0";
        }
        if ($maxTotal !== null && $maxTotal < 0) {
            $errors[] = "pack={$packId} file={$path} path=$.policy.max_total :: must be >= 0";
        }
    ...
    public function checkHighlightsOverrides(string $path, string $packId, ?string $expectedSchema = null): array
    {
        ...
        $items = $json['items'] ?? null;
        if (! is_array($items)) {
            return [false, ["pack={$packId} file={$path} path=$.items :: Missing/invalid items"]];
        }
    ...
    public function checkBorderlineTemplates(string $path, string $packId, ?string $expectedSchema = null): array
    {
        ...
        foreach ($dims as $dim) {
            $t = $items[$dim] ?? null;
            if (! is_array($t)) {
                $errors[] = "pack={$packId} file={$path} path=$.items.{$dim} :: missing/invalid";
            ...
            if (! is_string($title) || trim($title) === '') {
                $errors[] = "pack={$packId} file={$path} path=$.items.{$dim}.title :: must be non-empty string";
            }
            if (! is_string($text) || trim($text) === '') {
                $errors[] = "pack={$packId} file={$path} path=$.items.{$dim}.text :: must be non-empty string";
            }

            if (! array_key_exists('examples', $t) || ! is_array($t['examples'])) {
                $errors[] = "pack={$packId} file={$path} path=$.items.{$dim}.examples :: must be array";
            }
            if (! array_key_exists('suggestions', $t) || ! is_array($t['suggestions'])) {
                $errors[] = "pack={$packId} file={$path} path=$.items.{$dim}.suggestions :: must be array";
            }
        }

        if ($errors) {
            return [false, array_merge(['Borderline templates invalid: '.count($errors)], array_slice($errors, 0, 120))];
        }
    ...
    public function checkReportRoles(string $path, string $packId, ?string $expectedSchema = null): array
    {
        ...
        foreach ($expected as $k) {
            $it = $items[$k] ?? null;
            if (! is_array($it)) {
                $errors[] = "pack={$packId} file={$path} :: Missing item {$k}";

                continue;
            }
            if (($it['code'] ?? null) !== $k) {
                $errors[] = "pack={$packId} file={$path} :: {$k} code mismatch";
            }

            foreach (['title', 'subtitle', 'desc'] as $f) {
                if (! isset($it[$f]) || ! is_string($it[$f]) || trim($it[$f]) === '') {
                    $errors[] = "pack={$packId} file={$path} :: {$k} missing {$f}";
                }
            }
            ...
            if (isset($it['tags']) && ! is_array($it['tags'])) {
                $errors[] = "pack={$packId} file={$path} :: {$k} tags must be array";
            }
        }
    ...
    public function checkRecommendedReads(string $path, string $packId, ?string $expectedSchema = null): array
    {
        ...
        foreach ($requiredBuckets as $k) {
            ...
            if ($k === 'fallback') {
                if (! is_array($items[$k])) {
                    $errors[] = "pack={$packId} file={$path} path=$.items.{$k} :: must be array(list)";
                }
            } else {
                if (! is_array($items[$k])) {
                    $errors[] = "pack={$packId} file={$path} path=$.items.{$k} :: must be object(map)";
                }
            }
        }
        ...
        foreach (($items['by_type'] ?? []) as $typeCode => $list) {
            if (! is_array($list)) {
                $rowErrors[] = "pack={$packId} file={$path} path=$.items.by_type.{$typeCode} :: must be array(list)";

                continue;
            }
            foreach ($list as $i => $it) {
                if (is_array($it)) {
                    $validateRead($it, "path=$.items.by_type.{$typeCode}[{$i}]");
                } else {
                    $rowErrors[] = "pack={$packId} file={$path} path=$.items.by_type.{$typeCode}[{$i}] :: must be object";
                }
            }
        }
        foreach (($items['by_role'] ?? []) as $role => $list) {
            if (! is_array($list)) {
                $rowErrors[] = "pack={$packId} file={$path} path=$.items.by_role.{$role} :: must be array(list)";

                continue;
            }
            foreach ($list as $i => $it) {
                if (is_array($it)) {
                    $validateRead($it, "path=$.items.by_role.{$role}[{$i}]");
                } else {
                    $rowErrors[] = "pack={$packId} file={$path} path=$.items.by_role.{$role}[{$i}] :: must be object";
                }
            }
        }
        foreach (($items['by_strategy'] ?? []) as $st => $list) {
            if (! is_array($list)) {
                $rowErrors[] = "pack={$packId} file={$path} path=$.items.by_strategy.{$st} :: must be array(list)";

                continue;
            }
            foreach ($list as $i => $it) {
                if (is_array($it)) {
                    $validateRead($it, "path=$.items.by_strategy.{$st}[{$i}]");
                } else {
                    $rowErrors[] = "pack={$packId} file={$path} path=$.items.by_strategy.{$st}[{$i}] :: must be object";
                }
            }
        }
        foreach (($items['by_top_axis'] ?? []) as $axisKey => $list) {
            if (! is_array($list)) {
                $rowErrors[] = "pack={$packId} file={$path} path=$.items.by_top_axis.{$axisKey} :: must be array(list)";

                continue;
            }
            foreach ($list as $i => $it) {
                if (is_array($it)) {
                    $validateRead($it, "path=$.items.by_top_axis.{$axisKey}[{$i}]");
                } else {
                    $rowErrors[] = "pack={$packId} file={$path} path=$.items.by_top_axis.{$axisKey}[{$i}] :: must be object";
                }
            }
        }
        foreach (($items['fallback'] ?? []) as $i => $it) {
            if (is_array($it)) {
                $validateRead($it, "path=$.items.fallback[{$i}]");
            } else {
                $rowErrors[] = "pack={$packId} file={$path} path=$.items.fallback[{$i}] :: must be object";
            }
        }
    ...
    public function checkIdentityLayers(string $path, string $packId, ?string $expectedSchema = null): array
    {
        ...
        if ($this->isAssocArray($items)) {
            ...
            if ($errs) {
                return [false, array_merge(['Identity layers invalid: '.count($errs)], array_slice($errs, 0, 120))];
            }
        ...
        foreach ($items as $i => $q) {
            ...
            if (is_array($opts)) {
                foreach ($opts as $j => $opt) {
                    if (! is_array($opt) || ! $this->isAssocArray($opt)) {
                        continue;
                    }
                    if (! array_key_exists('assets', $opt)) {
                        continue;
                    }
    ...
    public function checkStrictAssets(string $baseDir, array $declaredBasenames, string $packId): array
    {
        ...
        if (! empty($errors)) {
            return [false, array_merge(['Undeclared known files found: '.count($errors)], $errors)];
        }
    ...
    public function expectedSchemaFor(array $manifest, string $assetKey, string $file): ?string
    {
        ...
        if ($assetKey === 'identity') {
            if (str_contains($file, 'identity_cards')) {
                return $schemas['identity_cards'] ?? null;
            }
            if (str_contains($file, 'identity_layers')) {
                return $schemas['identity_layers'] ?? null;
            }
            if (str_contains($file, 'roles')) {
                return $schemas['roles'] ?? null;
            }
            if (str_contains($file, 'strategies')) {
                return $schemas['strategies'] ?? null;
            }
