...
目的：明确每个 `event_code` 由谁触发、何时触发、如何去重，保证埋点口径稳定、周报可复盘、后续迭代不扯皮。  
适用范围：FAP v0.2.1（MBTI 主流程 + 用户权益最小通道）
...
为确保可去重、可追踪，建议前端/后端在 `POST /api/v0.2/events` 的 `meta_json` 中尽量携带：
...
> 注：v0.2.1 允许“先约定口径，后补实现”。先把字段写进矩阵，前后端逐步补齐上报即可。

### 1.3 周报核心口径（默认）
v0.2 核心 5 事件用于最小漏斗：
- `scale_view` → `test_start` → `test_submit` → `result_view` → `share_generate`

v0.2.1 新增用户权益最小通道事件（不一定进入漏斗，但要可审计）：
...
| event_code | trigger_side | trigger_time | dedup_key | notes |
|---|---|---|---|---|
...
| **result_view** | frontend | 结果页 `GET /attempts/{id}/result` **成功返回** 且页面 **完成渲染**（首屏关键模块 ready） | `anon_id + attempt_id + page_session_id` | 同一次打开结果页只记一次；刷新不重复记；再次进入结果页（新 page_session_id）可再记；用于衡量结果曝光；可在 meta 带 `profile_version/content_package_version/type_code`。 |
| **share_generate** | frontend | 用户点击「生成分享卡/保存图片」并且 **图片生成成功**（canvas 导出成功/保存成功） | `anon_id + attempt_id + share_style + page_session_id` | 只记“生成成功”；生成失败不记；share_style 用于区分模板（朋友圈/群聊）；用于衡量分享资产产出；可在 meta 带 `share_style/template_version`。 |
| **delete_request_submit** | backend | 用户权益接口 `POST /api/v0.2/user-requests` 创建成功（`type=delete`）后 | `request_id` | 合规审计链路，必须后端触发；meta 建议带 `anon_id`（或其哈希）与请求范围；避免存敏感明文；后续可扩展 `delete_request_fulfilled`。 |
| **export_request_submit** | backend | 用户权益接口 `POST /api/v0.2/user-requests` 创建成功（`type=export`）后 | `request_id` | 同上；导出完成后续可扩展 `export_request_fulfilled`（v0.2.1 暂不要求）。 |
...
### 3.2 “体验型”事件（前端触发更合理）
- `scale_view` / `test_start` / `result_view` / `share_generate`：以用户真实交互与页面完成渲染为准
...
4) 打开结果页渲染完成 → 触发 `result_view`  
5) 生成分享卡成功 → 触发 `share_generate`  

### 4.2 去重验证（关键）
- 同一次结果页打开：`result_view` 只出现 1 次（同 `page_session_id`）
- 同一次生成分享卡：`share_generate` 只出现 1 次（同 `share_style + page_session_id`）
- 重进结果页（新 `page_session_id`）：允许出现新的 `result_view`

---

## 5. 未来扩展（非 v0.2.1 必做，仅占位）
- `share_click`（用户点击“分享”按钮但未必生成成功）
- `share_success`（平台分享成功回调，微信对这块有限制，可谨慎）
- `delete_request_fulfilled` / `export_request_fulfilled`（权益请求处理完成）
- `report_open`（若将“报告页/深读页”拆分多页面）
