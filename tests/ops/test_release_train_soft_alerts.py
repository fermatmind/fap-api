...
class ReleaseTrainSoftAlertTest(unittest.TestCase):
    ...
    def test_soft_alert_failure_does_not_skip_private_content_scan(self):
        original = module.run_smoke_checks
        module.run_smoke_checks = lambda checks: [
            {
                "name": "llms-full",
            ...
            },
            {
                ...
                "hard_block": True,
                "core_smoke": True,
                "private_or_held_exposure_guard": True,
                "search_channel_or_staging_guard": False,
                "owner": "seo-ops",
                "recommended_followup": "Remove private URLs.",
                "response_snippet": "https://fermatmind.com/results/private-attempt",
            },
        ]
        try:
            result = module.evaluate_item(
        ...
        self.assertEqual(result["status"], "blocked")
        self.assertIn("forbidden_content_detected", result["failures"])
        private_scan = [
            row for row in result["sidecars"]
            if row["check_name"] == "smoke:https://fermatmind.com/sitemap.xml"
        ]
        self.assertEqual(len(private_scan), 1)
        self.assertFalse(private_scan[0]["allow_nonblocking"])
