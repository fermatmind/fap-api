...
- Residue from a failed shared-layout migration may be inspected and removed only through the separately versioned `Backend Production Ops Shared Residue Recovery` workflow. Its read-only preflight must bind the exact failed run, a fresh successful v5 queue-control receipt, unchanged shared source and absent dedicated target, the preserved exact backup, foreign runtime fingerprint, one current ops worker, zero pending ops jobs, an absent temporary validation process, and the hashes/count of only the five allowlisted failed-run `/tmp` files. Apply requires the workflow-defined exact receipt-bound recovery authorization, may delete only that attested file set, and must prove the shared source, absent target, backup, foreign programs/PIDs, ops worker PID, and queue state remain unchanged. A live validation process, unknown suffix, changed residue set, or any runtime/config drift fails closed; this file-only recovery never kills a process, restarts a worker, deploys code, moves a symlink, migrates the application, or changes CMS/database authority, publication, sitemap, llms, search, or PR23 state.
