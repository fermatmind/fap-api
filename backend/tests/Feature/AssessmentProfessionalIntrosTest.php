...
class AssessmentProfessionalIntrosTest extends TestCase
{
    private const FIXTURE_CONNECTION = 'professional_intros_fixture';
    ...
    protected function setUp(): void
    {
        ...
        $connection['prefix'] = 'professional_intros_'.bin2hex(random_bytes(6)).'_';
    ...
    {
        return json_decode(file_get_contents(database_path('data/assessment_professional_intros_zh_20260908.json')), true, 512, JSON_THROW_ON_ERROR);
    }

    private function baseline(string $code): array
    {
        $baseline = json_decode(file_get_contents(database_path('data/assessment_methods_zh_20260906.json')), true, 512, JSON_THROW_ON_ERROR)['scales'][$code];
        $mbti = json_decode(file_get_contents(database_path('data/assessment_mbti_copy_zh_20260908.json')), true, 512, JSON_THROW_ON_ERROR);
        $other = json_decode(file_get_contents(database_path('data/assessment_riasec_enneagram_copy_zh_20260908.json')), true, 512, JSON_THROW_ON_ERROR);
        $updates = $code === 'MBTI' ? $mbti['updates'] : ($other['scales'][$code]['updates'] ?? []);
        foreach ($updates as $update) {
            if (isset($update['path'])) {
                data_set($baseline, $update['path'], $update['value']);
            } else {
                $items = data_get($baseline, $update['collection']);
                $index = array_search($update['id'], array_column($items, 'id'), true);
                $items[$index][$update['field']] = $update['value'];
                data_set($baseline, $update['collection'], $items);
            }
        }
    ...
    {
        (require database_path('migrations/2026_09_08_120000_refine_assessment_professional_intros_zh.php'))->up();
    }
    ...
    public function test_one_conflict_rolls_back_all_scales_and_tables(): void
    {
        foreach (['scales_registry', 'scales_registry_v2'] as $table) {
            foreach (array_keys($this->package()['scales']) as $code) {
                $baseline = $this->baseline($code);
                if ($table === 'scales_registry_v2' && $code === 'EQ_60') {
