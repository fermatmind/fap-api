...
class BigFiveAuthorityV232Test extends TestCase
{
    ...
    {
        parent::setUp();
        $this->packagePath = base_path('../generated/big-five-authority-v2/big5-authority-v2-article-comparisons-32');
    }

    public function test_final_package_has_exact_batch_32_bilingual_inventory(): void
    {
        ...
        $this->assertSame([32], array_values(array_unique(array_column($package['assets'], 'batch'))));
        $this->assertCount(5, array_unique(array_column($package['assets'], 'unique_intent_key')));
    }

    public function test_every_asset_has_complete_structure_and_crosswalk_boundary(): void
    {
        $package = $this->readJson('final-package.json');
        ...
        foreach ($package['assets'] as $asset) {
            ...
            $terms = $asset['locale'] === 'en' ? ['not mbti', 'no deterministic crosswalk', 'score conversion', 'unknown'] : ['不是 mbti', '确定性对照', '分数换算', 'unknown'];
    ...
    public function test_qa_proves_no_crosswalk_and_zero_release_mutations(): void
    {
        ...
        $this->assertTrue($qa['checks']['consumes_only_pr21_batch_32']);
        $this->assertTrue($qa['checks']['deterministic_crosswalks_prohibited']);
        $this->assertTrue($qa['checks']['score_and_type_conversion_prohibited']);
        $this->assertTrue($qa['checks']['high_stakes_model_use_prohibited']);
