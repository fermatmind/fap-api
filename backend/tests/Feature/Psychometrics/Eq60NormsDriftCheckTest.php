...
final class Eq60NormsDriftCheckTest extends TestCase
{
    use RefreshDatabase;

    public function test_drift_check_passes_when_versions_are_identical(): void
    {
        $this->seedVersion('bootstrap_v1', 'zh-CN_all_18-60', 0.0);

        $this->artisan('norms:eq60:drift-check --from=bootstrap_v1 --to=bootstrap_v1 --group_id=zh-CN_all_18-60')
            ->assertExitCode(0);
    }
    ...
    {
        $this->seedVersion('bootstrap_v1', 'zh-CN_all_18-60', 0.0);
        $this->seedVersion('2026Q2_v1', 'zh-CN_all_18-60', 4.0);

        $this->artisan('norms:eq60:drift-check --from=bootstrap_v1 --to=2026Q2_v1 --group_id=zh-CN_all_18-60 --threshold_mean=1 --threshold_sd=1')
            ->assertExitCode(1);
    }
    ...
    private function seedVersion(string $version, string $groupId, float $shift): void
    {
        ...
        DB::table('scale_norms_versions')->insert([
            ...
            'age_min' => 18,
            'age_max' => 60,
            'source_id' => 'FERMATMIND_EQ60_PROD_ROLLING',
