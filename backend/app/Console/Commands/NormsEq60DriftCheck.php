...
final class NormsEq60DriftCheck extends Command
{
    protected $signature = 'norms:eq60:drift-check
    ...
    protected $description = 'Check EQ_60 norms drift between two norms versions and fail on threshold breach.';
    ...
    public function handle(): int
    {
        ...
        $thresholdMean = $this->resolveThreshold(
            ...
            (float) config('eq60_norms.drift.threshold_mean', 3.0)
        );
        $thresholdSd = $this->resolveThreshold(
            $this->option('threshold_sd'),
            (float) config('eq60_norms.drift.threshold_sd', 3.0)
        );
