...
use App\Services\Cms\MethodPageService;
...
final class MethodPageController extends Controller
{
    use RespondsWithNotFound;

    public function __construct(
        private readonly MethodPageService $methodPageService,
    ...
    public function index(Request $request): JsonResponse
    {
        ...
        $paginator = $this->methodPageService->listPublicMethods(
            $validated['org_id'],
            $validated['locale'],
            $validated['page'],
            $validated['per_page'],
        ...
        foreach ($paginator->items() as $page) {
            if (! $page instanceof MethodPage) {
                continue;
            }

            $items[] = $this->methodPageService->listPayload($page);
        }
    ...
    public function show(Request $request, string $slug): JsonResponse
    {
        ...
        $page = $this->methodPageService->getPublicMethodBySlug($slug, $validated['org_id'], $validated['locale']);
        if (! $page instanceof MethodPage) {
            return $this->notFoundResponse('method page not found.');
        }

        $meta = $this->methodPageSeoService->buildMeta($page, $validated['locale']);
        $jsonLd = $this->methodPageSeoService->buildJsonLd($page, $validated['locale']);
        $seoSurface = $this->buildSeoSurface($meta, $jsonLd, 'method_public_detail');
        $landingSurface = $this->buildDetailLandingSurface($page, $validated['locale']);

        return response()->json([
            'ok' => true,
            'page' => $this->methodPageService->detailPayload($page),
            'seo_meta' => $this->methodPageService->seoMetaPayload($page),
    ...
    public function seo(Request $request, string $slug): JsonResponse
    {
        ...
        return response()->json([
            'meta' => $meta,
            'jsonld' => $jsonLd,
            'seo_surface_v1' => $this->buildSeoSurface($meta, $jsonLd, 'method_public_detail'),
        ]);
    }
    ...
    private function buildIndexLandingSurface(string $locale): array
    {
        ...
        return $this->landingSurfaceContractService->build([
            ...
            'entry_surface' => 'method_index',
            'entry_type' => 'method_page',
            'summary_blocks' => [[
                'key' => 'methods_summary',
                'title' => $locale === 'zh-CN' ? '方法页中心' : 'Methods hub',
                'body' => $locale === 'zh-CN'
                    ? '集中解释费马测试的方法定义、解释边界和测评口径。'
                    : 'A public hub for method definitions, interpretation boundaries, and assessment framing.',
                'kind' => 'answer_first',
            ]],
            'discoverability_keys' => ['method_page', 'topic', 'test'],
            'continue_reading_keys' => ['method_page', 'topic'],
            ...
            'attribution_scope' => 'public_method_index_landing',
            'surface_family' => 'method',
            'primary_content_ref' => 'method-index',
            'related_surface_keys' => ['topic', 'test'],
            'fingerprint_seed' => ['locale' => $locale, 'surface' => 'method_index'],
        ]);
    }
     ...
     * @return array<string, mixed>
     */
    private function buildDetailLandingSurface(MethodPage $page, string $locale): array
    {
        $segment = $this->frontendLocaleSegment($locale);

        return $this->landingSurfaceContractService->build([
            'landing_scope' => 'public_indexable_detail',
            'entry_surface' => 'method_detail',
            'entry_type' => 'method_page',
            'summary_blocks' => [[
                'key' => 'hero',
                'title' => (string) $page->title,
                'body' => trim((string) ($page->excerpt ?? $page->subtitle ?? '')),
                'kind' => 'answer_first',
            ]],
            'discoverability_keys' => ['method_page', 'topic', 'test', 'data_page'],
            'continue_reading_keys' => ['topic', 'test', 'data_page'],
            'start_test_target' => '/'.$segment.'/tests/mbti-personality-test-16-personality-types',
            'content_continue_target' => '/'.$segment.'/data',
            'cta_bundle' => [
                [
                    'key' => 'data_pages',
                    'label' => $locale === 'zh-CN' ? '查看数据页' : 'View data pages',
                    'href' => '/'.$segment.'/data',
            ...
            'attribution_scope' => 'public_method_detail_landing',
            'surface_family' => 'method',
            'primary_content_ref' => (string) $page->slug,
            'related_surface_keys' => ['topic', 'test', 'data_page'],
     ...
     * @return array<string, mixed>
     */
    private function buildDetailAnswerSurface(MethodPage $page, array $seoSurface, array $landingSurface, string $locale): array
    {
        return $this->answerSurfaceContractService->build([
            'surface_scope' => 'public_indexable_detail',
            'surface_type' => 'method_detail',
            'summary_blocks' => [[
                'key' => 'method_summary',
                'title' => (string) $page->title,
                'body' => trim((string) ($page->excerpt ?? $page->subtitle ?? '')),
            ...
            'attribution_scope' => 'public_method_answer',
            'locale' => $locale,
            'primary_content_ref' => (string) $page->slug,
        ]);
    }
