...
final class DataPageSeoService
{
    /**
     * @return array<string, mixed>
     */
    public function buildMeta(DataPage $page, string $locale): array
    {
        ...
        $description = $this->fallbackText(
            ...
            (string) ($page->summary_statement_md ?? null)
        );
     ...
     * @return array<string, mixed>
     */
    public function buildJsonLd(DataPage $page, string $locale): array
    {
        ...
        return SeoSchemaPolicyService::finalize($page, [
            ...
            'overrides' => $seoMeta instanceof DataPageSeoMeta && is_array($seoMeta->jsonld_overrides_json)
                ? SeoSchemaPolicyService::sanitizeStoredOverrides($seoMeta->jsonld_overrides_json) ?? []
                : [],
        ]);
    }

    public function buildCanonicalUrl(DataPage $page, string $locale): ?string
    {
    ...
    private function resolveSeoMeta(DataPage $page): ?DataPageSeoMeta
    {
        if ($page->relationLoaded('seoMeta') && $page->seoMeta instanceof DataPageSeoMeta) {
            return $page->seoMeta;
        }

        return DataPageSeoMeta::query()
