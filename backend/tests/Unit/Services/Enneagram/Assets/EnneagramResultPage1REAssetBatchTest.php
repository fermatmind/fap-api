...
final class EnneagramResultPage1REAssetBatchTest extends TestCase
{
    public function test_1r_e_batch_assets_are_runner_safe_and_not_runtime(): void
    {
        $root = base_path('content_assets/enneagram/result_page/batch_1r_e/v0_1');
        $manifest = $this->readJson($root.'/asset_batch_manifest.json');
        $assetsDoc = $this->readJson($root.'/assets.json');

        $this->assertSame('fap.enneagram.result_page.batch_1r_e.manifest.v0.1', $manifest['schema_version'] ?? null);
        ...
        $this->assertSame('c88683363e40715425f3b7377c26a59379e2fc4036f0dd22c58ff2771760d9d9', $manifest['source_asset_sha256'] ?? null);
        ...
        $this->assertSame([
            'behavior_vs_motivation' => 9,
            'broad_distribution' => 9,
            'center_clustered_body' => 9,
            'center_clustered_head' => 9,
            'center_clustered_heart' => 9,
            'contradictory_pattern' => 9,
            'cross_center_distribution' => 9,
            'low_signal_top3' => 9,
            'next_step_convergence' => 9,
            'scene_specific_top3' => 9,
            'three_center_spread' => 9,
            'top3_flat' => 9,
        ], data_get($manifest, 'coverage_counts.by_diffuse_axis'));
        ...
        $sourceLedgerRow = $this->sourceLedgerRow('batch_1r_e_asset_stream');
        $assetKeys = [];
        ...
        foreach ((array) ($assetsDoc['assets'] ?? []) as $asset) {
            ...
            $this->assertSame('1R-E', $asset['batch_scope'] ?? null);
            $this->assertSame('diffuse_convergence_response', $asset['category'] ?? null);
            $this->assertNotSame('', (string) data_get($asset, 'routing_metadata.branch_metadata.diffuse_axis'));
            $this->assertSame('batch_1r_e_asset_stream', data_get($asset, 'source_trace.primary_source_id'));
