...
final class PersonalityDesktopCloneBaselineImportTest extends TestCase
{
    ...
    public function test_import_publishes_32_full_code_zh_clone_content_with_compatibility_fields_and_is_idempotent(): void
    {
        ...
        $this->assertSame('允许先做一个不成熟的尝试', data_get($enfpTContent, 'content_json.chapters.growth.what_energizes.items.0.title'));
        ...
        $this->assertSame('把听到的感受交还对方确认', data_get($infjAContent, 'content_json.chapters.relationships.superpowers.items.0.title'));
        $this->assertCount(1, (array) data_get($infjAContent, 'content_json.chapters.relationships.pitfalls.items.0.signals'));
        $this->assertSame(
    ...
    private function assertInsightListModuleShape(array $content, string $modulePath, string $fullCode): void
    {
        ...
        $this->assertCount(3, $items, sprintf('%s invalid scenario count in %s.items', $fullCode, $modulePath));
        ...
        foreach ($items as $index => $item) {
            ...
            $this->assertCount(1, (array) data_get($item, 'signals'), sprintf('%s invalid %s.items[%d].signals', $fullCode, $modulePath, $index));
