...
- Big Five ZH6 readiness may use `solo_operator` only for exact `admin_user:1`, explicit self-review, immutable cohort/package hashes, and complete external-human audit evidence. It does not relax global reviewer separation. Personality public content media readiness is retired: Big Five and Enneagram `PersonalityPublicContentAsset` records permanently support no hero, inline, OG, or Twitter image fields.
