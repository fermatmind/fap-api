...
- The inactive Career candidate bootstrap uses only the candidate's own code, a 10-row offline batch, a 5,000ms build budget, and at most one 500ms retry for an explicit build-budget or transient database-read classification. The 10-row boundary limits high-density batches to at most five bilingual slugs per conversion-closure precompute. The public HTTP warmer remains on its separate 2,000ms budget.
- The candidate's full coverage inspection before each offline batch follows the same read-only retry boundary: retry exactly once only for an explicitly classified transient database read, preserve a safe pre-batch stage/category/attempt receipt, and stop without a write on permanent or unclassified failure. Candidate-runtime initialization is split into safe stages. Only an explicitly classified transient database read may start one fresh PHP process after 500ms and an exact identity recheck; a permanent or unclassified initialization failure is never retried, and the failed process must report zero cache/queue/database writes.
