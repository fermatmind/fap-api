...
class LegacyMbtiAttemptLifecycleService
{
    ...
    public function storeAttempt(Request $request): array
    {
        ...
        if ($expectedQuestionCount <= 0) {
            throw new ApiProblemException(500, 'QUESTIONS_NOT_READY', 'Questions index is empty.');
        }
        ...
        if (! empty($unknown)) {
            throw new ApiProblemException(422, 'VALIDATION_FAILED', 'Unknown question_id in answers.', [
        ...
        if (count($unique) !== $expectedQuestionCount) {
            ...
            throw new ApiProblemException(422, 'VALIDATION_FAILED', "Answers must contain {$expectedQuestionCount} unique question_id.", [
        ...
        if (count($payload['answers']) !== $expectedQuestionCount) {
            throw new ApiProblemException(422, 'VALIDATION_FAILED', "Invalid answers count. expected={$expectedQuestionCount}", [
        ...
        if ($typeCode === '' || empty($scoresPct)) {
            throw new ApiProblemException(500, 'SCORING_FAILED', 'Scoring output incomplete.');
        }
        ...
        if (! $canStoreJson && (! $storeToStorage || ! $canStorePath)) {
            throw new ApiProblemException(
                ...
                'Attempt answers persistence is required for audit, but attempts.answers_json / answers_storage_path is not available.',
                [
        ...
        $tx = DB::transaction(function () use (
            ...
            if ($existingAttempt) {
                if ((string) $existingAttempt->anon_id !== (string) $payload['anon_id']
                    ...
                    throw new ApiProblemException(409, 'ATTEMPT_MISMATCH', 'attempt_id does not match anon_id/scale.');
                }
            }
