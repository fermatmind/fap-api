...
final class SchedulerEvidenceMonitorTest extends TestCase
{
    ...
    {
        $now = CarbonImmutable::parse('2026-08-27T13:50:00Z');
        ...
        $this->assertSame('2026-08-27T14:00:00Z', $receipt['weekly']['due_at']);
    }

    #[Test]
    public function grace_expiry_accepts_only_the_exact_natural_receipt_contract(): void
    {
        $now = CarbonImmutable::parse('2026-08-27T14:01:00Z');
        $heartbeat = new SchedulerHeartbeatService($this->heartbeatPath);
        $heartbeat->record('completed', 0, $now);
        $expected = $this->insertNaturalReceipt(CarbonImmutable::parse('2026-08-27T13:45:00Z'));
    ...
    public function receipt_hash_or_manual_exclusion_mismatch_fails_closed(): void
    {
        ...
        $heartbeat = new SchedulerHeartbeatService($this->heartbeatPath);
        $heartbeat->record('completed', 0, $now);
        $this->insertNaturalReceipt(CarbonImmutable::parse('2026-08-27T13:45:00Z'));
        ...
        $this->insertNaturalReceipt(
            CarbonImmutable::parse('2026-08-27T13:45:00Z'),
    ...
    {
        Http::fake();
        $healthyAt = CarbonImmutable::parse('2026-08-27T13:50:00Z');
        $heartbeat = new SchedulerHeartbeatService($this->heartbeatPath);
    ...
    private function insertNaturalReceipt(
        ...
        DB::connection('seo_intel')->table('seo_weekly_decision_receipts')->insert([
            ...
            'receipt_hash' => hash('sha256', $selectionJson),
            'created_at' => $slot,
        ]);
        ...
        $receiptHash = hash('sha256', $capabilityJson);
