...
class BigFiveAuthorityV219Test extends TestCase
{
    private const DIR = 'generated/big-five-authority-v2/big5-authority-v2-facets-neuroticism-19';
    ...
    /** @var list<string> */
    private const FACETS = ['anxiety', 'anger', 'depression', 'self-consciousness', 'impulsiveness', 'vulnerability'];
    ...
    public function test_package_workflow_passes_with_twelve_pages_and_closed_release_gates(): void
    {
        ...
        $this->assertSame([
            '/en/personality/big-five/facets/anger',
            '/en/personality/big-five/facets/anxiety',
            '/en/personality/big-five/facets/depression',
            '/en/personality/big-five/facets/impulsiveness',
            '/en/personality/big-five/facets/self-consciousness',
            '/en/personality/big-five/facets/vulnerability',
            '/zh/personality/big-five/facets/anger',
            '/zh/personality/big-five/facets/anxiety',
            '/zh/personality/big-five/facets/depression',
            '/zh/personality/big-five/facets/impulsiveness',
            '/zh/personality/big-five/facets/self-consciousness',
            '/zh/personality/big-five/facets/vulnerability',
    ...
    public function test_final_package_preserves_exact_facet_pairs_taxonomy_and_content_contract(): void
    {
        ...
        foreach ($package['pages'] as $page) {
            ...
            $this->assertSame('neuroticism', $page['domain_code']);
            $this->assertSame('ipip_neo_30_facet_navigation', $page['taxonomy']);
            $this->assertSame('facet:neuroticism:'.$page['facet_code'], $page['content_key']);
        ...
        $this->assertStringContainsString('not an anxiety-disorder diagnosis', $pairs['anxiety']['en']['summary']);
        $this->assertStringContainsString('不衡量攻击性', $pairs['anger']['zh-CN']['summary']);
        $this->assertStringContainsString('not a diagnosis of depressive disorder', $pairs['depression']['en']['summary']);
        $this->assertStringContainsString('不是社交焦虑诊断', $pairs['self-consciousness']['zh-CN']['summary']);
        $this->assertStringContainsString('does not diagnose an impulse-control condition', $pairs['impulsiveness']['en']['not_meaning']);
        $this->assertStringContainsString('不能预测实际表现', $pairs['vulnerability']['zh-CN']['summary']);
    ...
    {
        $package = $this->readJson(self::DIR.'/final-package.json');
        $package['pages'][0]['facet_code'] = 'anger';
    ...
    {
        $path = tempnam(sys_get_temp_dir(), 'big5-authority-v2-facets-neuroticism-');
