...
> Version: Glossary v0.3 (Stage 1 / v0.3-A)
> Related Docs:
> - docs/README.md
...
# FAP v0.3 领域词典（Glossary）— v0.3（对齐 API / 合规 / 发布）

> FAP = Fermat Assessment Platform  
> 适用范围：`v0.3.x`（MBTI 主流程）所有后端 / 前端 / 数据分析文档 & 代码。  
> 本版本：**v0.3**（新增：动态报告相关字段、分享接口口径、用户权益相关术语）
...
- **典型用法**：`FAP v0.3.x`、域名 `fermatmind.com`、微信小程序「费马测试」。
...
- **说明**：注册或绑定账号的真实用户主体（v0.3 允许未登录流程）。
...
- **字段关键点**：
  ...
  - `scores_pct`（v0.3 新增，见 6.1）
  - `axis_states`（v0.3 新增，见 6.2）
  - `profile_version`：结果文案版本（类型骨架/长文结构版本）
  - `content_package_version`（v0.3 新增，内容包版本）
  - `computed_at`
  - `is_valid`
...
v0.3 新增：`content_package_version`（内容包版本），用于动态报告与分享资产的版本化发布/回滚。
...
### 5.2 content_package_version（内容包版本）— v0.3 新增

- **字段名**：`content_package_version`
- **类型**：`string(32)`
- **示例**：
  - `MBTI-CN-v0.3`
  - `MBTI-CN-v0.3-hotfix1`
- **说明**：
...
## 6. 动态报告字段（Dynamic Report Fields）— v0.3 新增

v0.3 的核心变化：结果返回除了 `scores_json`，新增面向前端渲染的“稳定结构字段”，用于实现同型不同百分比差异化。
...
## 10. 分享与追踪（Share & Attribution）— v0.3 新增
...
## 11. API 端点术语索引（Endpoints Index）— v0.3 补齐

这里仅登记“端点名称与用途口径”，详细请求/响应以 `docs/03-stage1/api-v0.3-spec.md`（v0.3）为准。

### 11.1 Ping

- `GET /api/v0.3/ping`：健康检查

### 11.2 Scale / Questions

- `GET /api/v0.3/scales/mbti`（或统一 `GET /scales/{code}`）：拉取量表配置与题目列表

### 11.3 Attempts

- `POST /api/v0.3/attempts`：提交一次作答（写入 attempts + results，事务一致性）
- `GET /api/v0.3/attempts/{attempt_id}/result`：读取结果（只读，不重复写 result）

### 11.4 Share — v0.3 新增

- `GET /api/v0.3/attempts/{attempt_id}/share`：获取分享模板数据（含 share_id、content_package_version）

### 11.5 Events

- `POST /api/v0.3/events`：上报单条埋点事件（写入 events）

### 11.6 Stats

- `GET /api/v0.3/stats/summary`：最近 N 天游标口径汇总（对齐周报/命令）

### 11.7 User Rights（用户权益）— v0.3 新增/补齐口径

- `GET /api/v0.3/user-rights`（或 `GET /api/v0.3/privacy/summary`）：返回“我们记录什么/用途/如何删除/如何导出/联系渠道”
- `POST /api/v0.3/user-requests`：提交用户请求（删除/导出等），写入请求记录并可触发事件

---

## 12. 合规与用户权益（Compliance & User Rights）— v0.3 补齐
