...
final class PromotionContextFactory
{
    public function __construct(
        private readonly ExactPackagePathGuard $pathGuard,
    ...
    public function make(
        ...
        // Bootstrap captures per-run workflow values before cached configuration can mask them.
        $sourceCommit = strtolower(trim($this->execution->value('content_promotion.execution.source_commit')));
        $workflowRunId = trim($this->execution->value('content_promotion.execution.workflow_run_id'));
        $workflowRunAttempt = (int) $this->execution->value('content_promotion.execution.workflow_run_attempt', '0');
        $expectedRowCount = (int) $this->execution->value('content_promotion.execution.expected_row_count', '0');
        $executorReleaseSha256 = strtolower(trim($this->execution->value('content_promotion.execution.executor_release_sha256')));
        $releasePolicySha256 = strtolower(trim($this->execution->value('content_promotion.execution.release_policy_sha256')));
        $workflowSignature = strtolower(trim($this->execution->value('content_promotion.execution.workflow_signature')));
