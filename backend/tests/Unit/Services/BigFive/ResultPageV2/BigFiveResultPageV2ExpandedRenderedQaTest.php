...
final class BigFiveResultPageV2ExpandedRenderedQaTest extends TestCase
{
    private const BASE_PATH = 'content_assets/big5/result_page_v2/qa/rendered_preview/expanded_surfaces/v0_1';

    private const O59_PAYLOAD_PATH = 'tests/Fixtures/big5_result_page_v2/canonical_o59_core_body_preview.payload.json';

    public function test_expanded_rendered_qa_package_is_staging_only_and_not_runtime(): void
    {
        ...
        $this->assertSame('staging_preview_qa_advisory', $matrix['mode'] ?? null);
        $this->assertSame('staging_preview_qa_advisory', $report['mode'] ?? null);
        $this->assertTrue((bool) data_get($report, 'staging_preview_readiness.staging_preview_can_remain_non_runtime'));
        $this->assertTrue((bool) data_get($report, 'staging_preview_readiness.pilot_runtime_blocked'));
    ...
    public function test_public_payload_visible_fields_do_not_leak_banned_terms(): void
    {
        $matrix = $this->jsonFile('big5_o59_expanded_rendered_qa_surface_matrix_v0_1.json');
        $report = $this->jsonFile('big5_o59_expanded_rendered_qa_report_v0_1.json');
        $visibleText = $this->visibleText($this->o59Payload());
        $terms = (array) ($matrix['banned_rendered_terms_union'] ?? []);

        $this->assertSame('pass', data_get($report, 'public_payload_banned_scan.status'));
        $this->assertContains('frontend_fallback', $terms);
     ...
     */
    private function o59Payload(): array
    {
        $envelope = $this->decodeJson(self::O59_PAYLOAD_PATH);
